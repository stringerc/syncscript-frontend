...
test.describe('Feature: Task Creation', () => {
  ...
  test('Shows loading state while creating task', async ({ page }) => {
    await page.click('button: has-text("Add Task")'), await page.fill('input[name="title"]', 'Test Task');
    
    const createButton = page.locator('button: has-text("Create")'), await createButton.click();
  ...
  test('Creates task successfully with all fields', async ({ page }) => {
    await page.click('button: has-text("Add Task")'), await page.fill('input[name="title"]', 'Complete quarterly report');
    ...
    await page.click('button: has-text("Create")'), // Should see success toast
    const toast = page.locator('[class*="toast"]');
    await expect(toast).toContainText(/created|success/i, { timeout: 3000 }), // Task should appear in list
  ...
  test('Shows validation error for empty title', async ({ page }) => {
    await page.click('button: has-text("Add Task")'), // Leave title empty
    await page.selectOption('select[name="priority"]', '2');
    await page.click('button: has-text("Create")'), // Should see validation error
  ...
  test('Handles extremely long title gracefully', async ({ page }) => {
    await page.click('button: has-text("Add Task")'), const longTitle = 'A'.repeat(500);
    ...
    await page.click('button: has-text("Create")'), // Should create successfully
    const toast = page.locator('[class*="toast"]');
    await expect(toast).toContainText(/created|success/i, { timeout: 3000 }), // Title should be truncated in UI with ellipsis
  ...
  test('Handles special characters in title', async ({ page }) => {
    await page.click('button: has-text("Add Task")'), await page.fill('input[name="title"]', 'Fix "bug" in <component> & deploy');
    await page.click('button: has-text("Create")'), const toast = page.locator('[class*="toast"]');
  ...
  test('Handles emoji in task title', async ({ page }) => {
    await page.click('button: has-text("Add Task")'), await page.fill('input[name="title"]', '🚀 Launch new feature 🎉');
    await page.click('button: has-text("Create")'), const taskCard = page.locator('[class*="task"]').filter({{ hasText: '🚀 Launch new feature 🎉' },;
    await expect(taskCard).toBeVisible();
  });
  ...
  test('Works offline with localStorage fallback', async ({ page, context }) => {
    ...
    await page.click('button: has-text("Add Task")'), await page.fill('input[name="title"]', 'Offline task');
    await page.click('button: has-text("Create")'), // Should work with localStorage
