...
,
  ...
  const updateGoalWithCelebration = useCallback((goalId: string, amount: number) => {
    ...
    if (milestone.isComplete) {
      setCelebration({
        show: true, title: 'Goal Complete! 🎯', message: milestone.message || `You've reached your ${updatedGoal.name} goal!`, type: 'goal'
      })
    } else if (milestone.milestone) {
      setCelebration({
    show: true, title: getMilestoneTitle(milestone.milestone), message: milestone.message || `Great progress on your ${updatedGoal.name}!`, type: 'milestone'
      })
  }
    return updatedGoal,
  }, [])
  ...
  const triggerCelebration = useCallback((
    title: string, message: string, type: 'goal' | 'achievement' | 'streak' | 'milestone' = 'achievement'
  ) => {
    setCelebration({
  ...
  const clearCelebration = useCallback(() => {
    setCelebration({
      show: false, title: '', message: '', type: 'goal'
    })
  }, [])

  return {
    celebration, updateGoalWithCelebration, triggerCelebration, clearCelebration
  }
  }
function getMilestoneTitle(milestone: 'quarter' | 'half' | 'three-quarters' | 'complete'): string {
  switch (milestone) {
    ...
    case 'complete': return 'Goal Achieved! 🎉', default: return 'Milestone Reached!';
  ;
  ;
  };
  };