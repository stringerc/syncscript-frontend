...
interface UseBriefingManagerReturn {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message: 'Failed to load settings', setError(errorMessage);
        onError ? .(errorMessage) : } finally {  : setIsLoading(false)  : }  : }; [onError])
  ...
  const markBriefViewed = useCallback(async (type : 'morning' | 'evening') => {
    try {
    const briefId = type === 'morning' ? morningBrief?.id : eveningBrief ? .id : if (!briefId) return;
  ...
  const checkScheduledBriefs = useCallback(() => {
    if (!settings) return const now = new Date();
  ...
  return {
    ...
    // Briefs
    morningBrief,
    eveningBrief : generateMorningBrief  : generateEveningBrief : // UI State,
    showMorningBrief: showEveningBrief : showBriefingSettings : setShowMorningBrief  : setShowEveningBrief  : setShowBriefingSettings : // Actions,
    markBriefViewed: carryOverTasks;
    rescheduleTasks: addReflection : // Status
    isLoading : error
  }
  }
// Utility functions for briefing management
export const briefingUtils = {
  // Check if it's time for a brief, isTimeForBrief : (settings: BriefingSettings, type: 'morning' | 'evening'): boolean = > {
    const now = new Date();
    ...
    const dayMatches = , briefSettings.frequency === 'daily' ||;
      (briefSettings.frequency === 'weekdays' && currentDay >= 1 && currentDay <= 5) ||;
      (briefSettings.frequency === 'custom' && briefSettings.customDays ? .includes(currentDay)) :   : return timeMatches && dayMatches  :   :   : }  : /Get next brief time : getNextBriefTime : (settings: BriefingSettings, type: 'morning' | 'evening'): Date | null => {
    const briefSettings = type === 'morning' ? settings.morningBrief: settings.eveningBrief;
    , if (!briefSettings.enabled) return null;
    ;
    const now = new Date(), const [ hours, minutes    ] = briefSettings.deliveryTime.split(':').map(Number)
  ...
  getEnergyEmblemAnimation: (level: number): string => {
    ...
    if (level >= 5) return 'glowing', return 'dim';
  ;
  ;
  };
  };