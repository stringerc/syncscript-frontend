...
interface UseAchievementsProps {
  tasks: Task[],
    energyLogs: EnergyLog[], projects: Project[],
...
interface UnlockedAchievement {
  ...
  useEffect(() => {
    ...
    if (tasksCompleted >= 10) { const { achievement, unlocked } = updateAchievementProgress('task_10', tasksCompleted)
  ...
  return {
    ...
    achievementId: a.id,
    currentValue: a.progress || 0, targetValue: a.target || 1, percentage: a.target ? ((a.progress || 0) / a.target) * 100 : (a.unlocked ? 100 : 0),
