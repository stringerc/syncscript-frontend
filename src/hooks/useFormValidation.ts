...
export interface FormState<T> {
  ...
  errors: Partial<Record<keyof T,
    string>>
  touched: Partial<Record<keyof T,
    boolean>>
  isValid: boolean,
...
,
  ...
  const handleBlur = useCallback((name: keyof T) => {
    // Mark field as touched, setFormState(prev => ({
      ...prev, touched: {
        ...prev.touched;
        [name]: true
  ...
  const handleSubmit = useCallback(async (onSubmit: (values: T) => void | Promise<void>) => {
    ...
    if (!isValid) {
    return false}, setFormState(prev = > ({ ...prev, isSubmitting: true }))
  ...
  const reset = useCallback(() => {
    setFormState({
      values: initialValues, errors: { }, touched: { }, isValid: true,
    isSubmitting: false,
  });
  }, [initialValues])

  return {
    values: formState.values, errors: formState.errors, touched: formState.touched, isValid: formState.isValid,
    isSubmitting: formState.isSubmitting, handleChange, handleBlur, handleSubmit, reset, setFieldValue: handleChange, setFieldError: (name: keyof T, error: string) => {
    setFormState(prev => ({;
        ...prev
    errors: {
        ...prev.errors;
