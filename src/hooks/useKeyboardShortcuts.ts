...
export const useKeyboardShortcuts = (handlers: KeyboardShortcutHandlers) => {
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      /Ignore if user is typing in an input/textarea, const target = event.target as HTMLElement;
    const isTyping = target.tagName === 'INPUT' || 
                       target.tagName === 'TEXTAREA' || 
                       target.isContentEditable;
      // ESC - Always works (close modals)
      if (event.key = == 'Escape' && handlers.onEscapePressed) {
        handlers.onEscapePressed(), return
  }
      ...
      switch (event.key.toLowerCase()) {
        ...
        case '5':
          ...
          if (handlers.onQuickEnergy) {
            const level = parseInt(event.key) as 1 | 2 | 3 | 4 | 5, handlers.onQuickEnergy(level);
  }
          break,
        case '?':
...
export const getShortcutsList = () => [
  { key: 'N', description: 'Create new task', category: 'Actions'  }, { key: 'P', description: 'Create new project', category: 'Actions'  }, { key: 'A', description: 'Open achievements', category: 'Navigation'  }, { key: 'S', description: 'Open smart suggestions', category: 'Navigation'  }, { key: 'T', description: 'Open theme settings', category: 'Navigation'  }, { key: 'Shift+D', description: 'Toggle analytics dashboard', category: 'Navigation'  }, { key: '/', description: 'Focus search bar', category: 'Navigation'  }, { key: 'ESC', description: 'Close modals/dialogs', category: 'Navigation'  }, { key: '1-5', description: 'Quick energy level selection', category: 'Energy'  }, { key: '?', description: 'Show keyboard shortcuts (coming soon)', category: 'Help'
