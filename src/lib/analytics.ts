...
export const analytics = {
  ...
  userRegistered: (userId: string,
    method: 'email' | 'google' | 'github' | 'sso', source?: string) => {
  ...
  }, userCompletedOnboarding: (userId: string,
    stepsCompleted: number, durationSeconds: number) => {
    track('user_completed_onboarding', {
      user_id: userId,
    steps_completed: stepsCompleted,
      duration_seconds: durationSeconds, timestamp: new Date().toISOString()
    })
  }, /============================================================================
  // TASK EVENTS
  // ============================================================================
  ...
  taskCreated: (userId: string, taskId: string, properties: {
        priority?: number, energyLevel?: number;
        hasBudget?: boolean;
    ...
    taskId: string, properties: {
        actualDuration?: number, energyBefore?: number;
        energyAfter?: number;
  ...
  energyLevelUpdated: (userId: string,
    oldLevel: number, newLevel: number,
    source: 'manual' | 'auto_recalibration', confidence?: number) => {
  ...
  aiSuggestionShown: (userId: string,
    suggestionId: string, suggestionType: string,
    confidence: number, contextFactors: string[]) => {
  ...
  }, aiSuggestionAccepted: (userId: string,
    suggestionId: string, timeToDecision: number) => {
  ...
  }, aiExplanationViewed: (userId: string,
    feature: string, explanationType: 'why' | 'how' | 'alternatives' | 'confidence') => {
  ...
  budgetFitCalculated: (userId: string,
    taskId: string, fitScore: number, comfortBand: 'safe' | 'stretch' | 'over') => {
  ...
  }, savingsGoalUpdated: (userId: string,
    goalId: string, amountSaved: number, daysAccelerated: number) => {
  ...
  achievementUnlocked: (userId: string,
  ...
  }, emblemCharged: (userId: string,
    emblemId: string, chargeDelta: number,
    totalCharge: number, reason: string) => {
  ...
  featureDiscovered: (userId: string,
    featureId: string, discoveryMethod: string) => {
  ...
  pageViewed: (userId: string | undefined,
  ...
  premiumUpgradeStarted: (userId: string,
    plan: string, billingPeriod: 'monthly' | 'annual') => {
  ...
  }, premiumUpgradeCompleted: (userId: string,
    plan: string, amountPaid: number) => {
  ...
  errorOccurred: (errorType: string,
    errorMessage: string, pagePath: string) => {
 ...
 */,
  ...
  if (typeof window = == 'undefined') return import('web-vitals').then(({ onCLS, onFCP, onINP, onLCP; onTTFB }) => {
    onCLS((metric) => {
