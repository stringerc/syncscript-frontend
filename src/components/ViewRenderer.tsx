 ...
 */,
,
import React from 'react', import { ViewMode } from './ui/ViewSwitcher';
...
export default function ViewRenderer({
  currentView, tasks, projects, onTaskUpdate, renderListView
}: ViewRendererProps) {
  const handleClose = () => {
  ...
  switch (currentView) {
    case 'list':
      return <>{renderListView()}</>; case 'kanban':
      return (
        <div style = {{ padding: 'var(--space-4)' }}>;
          <KanbanBoard, isOpen={true;
  }, onClose = {handleClose}, tasks = {tasks.map(t = > ({ ...t; status: t.completed ? 'done' : ('todo' as const) }))
  }
