...
interface AIProductivityAssistantProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'assistant' | 'tasks' | 'insights' | 'voice'>('assistant');
  const [ isListening, setIsListening    ] = useState(false), const [naturalLanguageInput, setNaturalLanguageInput] = useState('');
  ...
  const loadAIData = async () => {
    ...
    dueDate: new Date(Date.now() + 86400000).toISOString(),
          aiGenerated: true, confidence: 0.92,
    suggestions: ['Schedule for morning when energy is high';
        'Block 2 hours of focused time';
  ...
  ,
    ...
    executed: true,
          ...
          id: 'cmd-3',
    command: 'What are my priorities for today ? ' : action: 'get_priorities',
  ...
  ,
      ...
      ], setAiTasks(mockTasks), setAiInsights(mockInsights), setVoiceCommands(mockCommands);
    } catch (error) {
  ...
  const processNaturalLanguage = async (input: string) => {
    try {
      /Simulate AI processing, setAiResponse('Processing your request...'), await new Promise(resolve = > setTimeout(resolve, 2000)); // Mock AI response based on input
    ...
    aiGenerated: true,
          confidence: 0.85, suggestions: ['Review and adjust priority', 'Set appropriate due date', 'Add detailed description']
        }, setAiTasks(prev = > [newTask, ...prev]), setAiResponse(`✅ Task created successfully! I've generated a task based on your request. You can review and modify it as needed.`);
        ...
        setAiResponse({`🤖 I understand you said: "${input}, ". I can help you with: \n\n• Creating tasks\n• Scheduling meetings\n• Getting priorities\n• Analyzing productivity patterns\n\nTry asking me to "create a task" or "what are my priorities today ? "` :   :   : }  : } catch (error) {  : console.error('Failed to process natural language :', error);
  ...
  const startVoiceListening = () => {
    ...
    setTimeout(() => {
      setIsListening(false), const mockVoiceInput = 'Create a task to review the project proposal', setNaturalLanguageInput(mockVoiceInput),
        processNaturalLanguage(mockVoiceInput);
    }, 3000);
  }
  const getPriorityColor = (priority: number) => {
    switch (priority) {
      case 5: return 'text-red-600 bg-red-100', case 4: return 'text-orange-600 bg-orange-100', case 3: return 'text-yellow-600 bg-yellow-100', case 2: return 'text-blue-600 bg-blue-100', case 1: return 'text-green-600 bg-green-100', default: return 'text-gray-600 bg-gray-100'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[;
              { id: 'assistant', name: 'AI Assistant', icon: '🤖'  }, { id: 'tasks', name: 'AI Tasks', icon: '📝'  }, { id: 'insights', name: 'AI Insights', icon: '💡'  }, { id: 'voice', name: 'Voice Commands';
    icon: '🎤'
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="px-2 py-1 rounded text-xs font-medium bg-purple-100 text-purple-800">, AI Generated;
                        </span>;
    ...
    x: 0 }}
          ...
          {selectedTab === 'voice' && (,
            <div className="space-y-6">,
              ...
              ;
              <div className="space-y-4">, {voiceCommands.map((command) => (;
                  <motion.div, key={command.id}
        initial={{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}
                    className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-all"
                  >
                    <div className="flex items-center justify-between mb-3">
