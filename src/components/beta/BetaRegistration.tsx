...
const TIER_DESCRIPTIONS = {
  ...
  'power-user': {
    ...
    icon: '⚡',
    color: 'from-blue-500 to-cyan-500', benefits: ['Advanced feature testing', 'Performance optimization feedback', 'Workflow integration insights']
  }
  'casual-user': {
    title: 'Casual User', description: 'You use productivity tools occasionally and provide fresh perspective', icon: '🌟', color: 'from-green-500 to-emerald-500', benefits: ['User-friendly feedback', 'Onboarding experience insights', 'Accessibility testing']
  }
};
    const TESTING_FOCUS_OPTIONS = [
  { id: 'morning-brief',
    label: 'Morning Brief', description: 'Daily planning and energy management'  }, { id: 'evening-brief',
    label: 'Evening Brief', description: 'Daily reflection and task completion'  }, { id: 'ai-explainability', label: 'AI Explainability', description: 'Understanding AI recommendations'  }, { id: 'emblem-transparency', label: 'Emblem Transparency', description: 'Energy system and gamification'  }, { id: 'analytics-dashboard', label: 'Analytics Dashboard', description: 'Productivity insights and trends'  }, { id: 'budget-intelligence', label: 'Budget Intelligence', description: 'Financial productivity features'  }, { id: 'energy-recalibration', label: 'Energy Recalibration', description: 'Smart task recommendations'
  
  ,
  };
], const BetaRegistration: React.FC<BetaRegistrationProps> = ({ onClose, onSuccess }) => {
  const [ step, setStep    ] = useState(1), const [form, setForm] = useState<RegistrationForm>({
    email: '', name: '', tier: 'early-adopter', testingFocus: [], motivation: '',
  }) const [isSubmitting, setIsSubmitting] = useState(false), const [error, setError] = useState<string | null>(null),
        const handleInputChange = (field: keyof RegistrationForm, value: string | string[]) => { setForm(prev = > ({ ...prev, [field]: value })), setError(null);
  }
  const handleTestingFocusToggle = (focusId: string) => {
    setForm(prev => ({
      ...prev, testingFocus: prev.testingFocus.includes(focusId);
  ...
  const handleSubmit = async () => {
    ...
    setIsSubmitting(true), setError(null), try {
    ...
    x: 0 }},
              ...
              >;
                <div>;
                  ...
                  <p className="text-gray-600">, Select the tier that best describes your testing style and expertise level.;
                  </p>;
                </div>;
;
                <div className="space-y-4">;
                  {Object.entries(TIER_DESCRIPTIONS).map(([tierKey; tier]) => (<motion.div
                      key = {tierKey
  }
...
;
                <div>;
                  <label className="block text-sm font-medium text-gray-700 mb-2">, What motivates you to join our beta program ? </label> : <textarea: value={form.motivation}
