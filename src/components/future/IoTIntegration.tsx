...
interface SmartDevice {
  ...
  type: 'light' | 'thermostat' | 'camera' | 'speaker' | 'sensor' | 'wearable',
    brand: string,
  model: string,
    status: 'online' | 'offline' | 'error' | 'maintenance', batteryLevel?: number,
  lastSeen: string,
    capabilities: string[],
    location: string,
    isConnected: boolean
...
interface IoTIntegrationProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'devices' | 'automation' | 'environment' | 'wearables'>('devices');
  ...
  ,
      ...
      ], setSmartDevices(mockSmartDevices), setAutomationRules(mockAutomationRules), setEnvironmentalData(mockEnvironmentalData), setWearableData(mockWearableData);
    } catch (error) {
  ...
  const connectDevice = async (deviceId: string) => {
    setIsConnecting(true), try {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)), setSmartDevices(prev = > prev.map(device => 
        device.id === deviceId ? { 
        ...
        ;
          ...
          : device)), console.log({`Connected device: ${deviceId}: `
  } catch (error) {
  ...
  const toggleAutomationRule = async (ruleId: string) => {
    try {
        setAutomationRules(prev => prev.map(rule => , rule.id === ruleId ;
          ? { ;
              ...rule;
    ...
    }; : rule)), console.log({`Toggled automation rule: ${ruleId}: `
  } catch (error) {
      console.error('Failed to toggle automation rule: ', error);
  }
  }
  const getTypeIcon = (type: string) => {
    switch (type) { case 'light': return '💡', case 'thermostat': return '🌡️', case 'camera': return '📹', case 'speaker': return '🔊', case 'sensor': return '📡', case 'wearable': return '⌚', default: return '🔌';
  ;
  ;
  };
  };
  const getStatusColor = (status: string) => {
    switch (status) { case 'online': return 'text-green-600 bg-green-100', case 'offline': return 'text-gray-600 bg-gray-100', case 'error': return 'text-red-600 bg-red-100', case 'maintenance': return 'text-yellow-600 bg-yellow-100', case 'active': return 'text-green-600 bg-green-100', case 'inactive': return 'text-gray-600 bg-gray-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
      ...
      >
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 mt-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="bg-white/20 px-2 py-1 rounded-full text-sm font-medium">
                    {smartDevices.filter(d => d.status === 'online'
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'devices',
    name: 'Smart Devices', icon: '🔌'  }, { id: 'automation', name: 'Automation', icon: '⚙️'  }, { id: 'environment',
    name: 'Environment', icon: '🌍'  }, { id: 'wearables', name: 'Wearables', icon: '⌚'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">;
          {selectedTab === 'devices' && (;
            <div className="space-y-6">, <h3 className="text-lg font-semibold text-gray-900">Smart Devices</h3>;
              ;
        ...
        initial = {{ opacity: 0, y: 20 }}, animate={{ opacity: 1, y: 0 }}
          ...
          {selectedTab === 'automation' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {automationRules.map((rule) => (,
                  <motion.div,
    key={rule.id}
        initial={{ opacity: 0,
    x: -20 }},
    ...
    x: 0 }}
          ...
          {selectedTab === 'environment' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">, {environmentalData.map((data) => (,
                  <motion.div,
    key={data.id}
        initial={{ opacity: 0,
    y: 20 }},
    ...
    y: 0 }},
          ...
          {selectedTab === 'wearables' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">, {wearableData.map((data) => (,
                  <motion.div, key={data.id}
        initial={{ opacity: 0, y: 20 }}, animate={{ opacity: 1, y: 0 }}, className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-all";
                  >;
                    <div className="text-center">;
