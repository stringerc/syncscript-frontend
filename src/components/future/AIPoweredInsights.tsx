...
interface AIInsight {
    ...
    title: string,
  description: string,
    type: 'prediction' | 'recommendation' | 'pattern' | 'optimization' | 'alert', category: 'productivity' | 'health' | 'schedule' | 'team' | 'energy',
    confidence: number,
...
interface PredictiveModel {
    ...
    name: string,
  description: string,
    type: 'regression' | 'classification' | 'clustering' | 'time-series' | 'nlp', accuracy: number,
    precision: number,
  recall: number,
    f1Score: number,
...
interface IntelligentSuggestion {
  id: string,
    title: string,
  description: string,
    type: 'task' | 'schedule' | 'break' | 'focus' | 'collaboration' | 'learning', priority: 'low' | 'medium' | 'high' | 'urgent',
    confidence: number,
  reasoning: string,
  ...
  id: string,
    title: string,
  description: string,
    category: 'performance' | 'behavior' | 'trend' | 'anomaly' | 'opportunity', severity: 'info' | 'warning' | 'critical',
...
interface AIPoweredInsightsProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'insights' | 'models' | 'suggestions' | 'automated'>('insights');
  ...
  const loadAIData = async () => {
    ...
    you\'re likely to experience an energy dip around 3 PM. Consider scheduling a break or lighter tasks.',
          type: 'prediction',
    category: 'energy',
          confidence: 89.7,
    impact: 'medium',
  ...
  ,
      ...
      ], setAIInsights(mockAIInsights), setPredictiveModels(mockPredictiveModels), setIntelligentSuggestions(mockIntelligentSuggestions), setAutomatedInsights(mockAutomatedInsights);
    } catch (error) {
  ...
  const acceptSuggestion = async (suggestionId: string) => {
    try {
        setIntelligentSuggestions(prev => prev.map(suggestion => , suggestion.id === suggestionId ;
          ? { ...suggestion;
        isAccepted: true;
        ;
        ;
    ...
    }; : suggestion)), console.log({`Accepted suggestion: ${suggestionId}: `
  } catch (error) {
  ...
  const markInsightAsRead = async (insightId: string) => {
    try {
        setAutomatedInsights(prev => prev.map(insight => , insight.id === insightId ;
          ? { ...insight;
        isRead: true;
        ;
        ;
    ...
    }; : insight)), console.log({`Marked insight as read: ${insightId}: `
  } catch (error) {
      console.error('Failed to mark insight as read: ', error);
  }
  }
  const getTypeIcon = (type: string) => {
    switch (type) { case 'prediction': return '🔮', case 'recommendation': return '💡', case 'pattern': return '📊', case 'optimization': return '⚡', case 'alert': return '⚠️', case 'task': return '📋', case 'schedule': return '📅', case 'break': return '☕', case 'focus': return '🎯', case 'collaboration': return '👥', case 'learning': return '📚', case 'performance': return '📈', case 'behavior': return '🧠', case 'trend': return '📊', case 'anomaly': return '🔍', case 'opportunity': return '🚀', default: return '🤖';
  ;
  ;
  };
  };
  const getImpactColor = (impact: string) => {
    switch (impact) {
      case 'low': return 'text-gray-600 bg-gray-100', case 'medium': return 'text-yellow-600 bg-yellow-100', case 'high': return 'text-orange-600 bg-orange-100', case 'critical': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
    ...
    switch (severity) {
      case 'info': return 'text-blue-600 bg-blue-100', case 'warning': return 'text-yellow-600 bg-yellow-100', case 'critical': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[;
              { id: 'insights', name: 'AI Insights', icon: '🤖'  }, { id: 'models', name: 'Predictive Models', icon: '🧠'  }, { id: 'suggestions',
    name: 'Intelligent Suggestions', icon: '💡'  }, { id: 'automated', name: 'Automated Insights', icon: '⚡'
        ...
        initial={{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}
          ...
          {selectedTab === 'models' && (,
            <div className="space-y-6">,
              ...
              ;
              <div className="space-y-4">, {predictiveModels.map((model) => (;
    ...
    x: 0 }}
          ...
          {selectedTab === 'automated' && (,
            <div className="space-y-6">,
              ...
              ;
              <div className="space-y-4">, {automatedInsights.map((insight) => (;
                  <motion.div, key={insight.id}
        initial={{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}
                    className={`p-4 border rounded-lg hover:shadow-md transition-all ${
                      !insight.isRead 
