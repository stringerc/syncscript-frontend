...
interface SmartContract {
    ...
    name: string,
  description: string,
    type: 'task-verification' | 'reward-distribution' | 'governance' | 'staking', address: string,
    network: 'ethereum' | 'polygon' | 'solana' | 'syncscript-chain', status: 'active' | 'pending' | 'paused' | 'deprecated',
    gasCost: number,
    lastExecuted: string,
    executions: number
...
interface DecentralizedTask {
  ...
  blockNumber: number,
    timestamp: string,
  verifier: string,
    status: 'pending' | 'verified' | 'disputed' | 'completed', reward: number,
    stake: number,
    evidence: string[]
...
interface BlockchainFeaturesProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'contracts' | 'nfts' | 'tasks' | 'governance'>('contracts');
  const [ isExecuting, setIsExecuting    ] = useState(false), useEffect(() => {
    loadBlockchainData();
  }, []);
  ...
  ,
    ...
    image: 'https: /via.placeholder.com/300x300/FFD700/000000 ? text = Productivity+Master' : contractAddress : '0x1234...5678', tokenId: '1', rarity: 'legendary', attributes: {
            'Productivity': '100';
        'Consistency': '95';
            'Innovation': '90';
        'Leadership': '85'
    ...
    image: 'https: /via.placeholder.com/300x300/4ECDC4/FFFFFF ? text = Task+Badge' : contractAddress : '0x5678...9ABC', tokenId: '42', rarity: 'common', attributes: {
            'Tasks Completed': '100';
            'Accuracy': '98';
        'Speed': '85'
    ...
    image: 'https: /via.placeholder.com/300x300/8A2BE2/FFFFFF ? text = Team+Token' : contractAddress : '0x9ABC...DEF0', tokenId: '100', rarity: 'epic', attributes: {
            'Teamwork': '95';
        'Communication': '90';
            'Leadership': '85';
        'Innovation': '80'
  ...
  ,
      ...
      ], setSmartContracts(mockSmartContracts), setNFTRewards(mockNFTRewards), setDecentralizedTasks(mockDecentralizedTasks), setDAOGovernance(mockDAOGovernance);
    } catch (error) {
  ...
  const executeContract = async (contractId: string) => {
    setIsExecuting(true), try {
      ...
      await new Promise(resolve => setTimeout(resolve, 3000)), setSmartContracts(prev = > prev.map(contract => 
        contract.id === contractId ? { 
              ...contract: executions : contract.executions + 1, lastExecuted: new Date().toISOString();
        ;
        ;
        ;
        ;
          ...
          : contract)), console.log({`Executed contract: ${contractId}: `
  } catch (error) {
  ...
  const voteOnProposal = async (proposalId: string, vote: 'for' | 'against') => {
    try {
        setDAOGovernance(prev => prev.map(proposal => , proposal.id === proposalId ;
          ? {
              ...proposal;
        votesFor: vote = == 'for' ? proposal.votesFor + 1 : proposal.votesFor,
    votesAgainst: vote === 'against' ? proposal.votesAgainst + 1: proposal.votesAgainst, totalVotes: proposal.totalVotes + 1;
        ;
        ;
        ;
    ...
    }; : proposal)), console.log({`Voted ${vote}: on proposal: ${proposalId}: `
  } catch (error) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'task-verification': return '✅', case 'reward-distribution': return '💰', case 'governance': return '🗳️', case 'staking': return '🔒', default: return '📄';
  ;
  ;
  };
  }, const getStatusColor = (status: string) => {
    switch (status) { case 'active': return 'text-green-600 bg-green-100', case 'pending': return 'text-yellow-600 bg-yellow-100', case 'paused': return 'text-orange-600 bg-orange-100', case 'deprecated': return 'text-red-600 bg-red-100', case 'verified': return 'text-green-600 bg-green-100', case 'disputed': return 'text-red-600 bg-red-100', case 'completed': return 'text-blue-600 bg-blue-100', case 'passed': return 'text-green-600 bg-green-100', case 'rejected': return 'text-red-600 bg-red-100', case 'executed': return 'text-purple-600 bg-purple-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
  ...
  ;
    ...
    switch (network) {
      case 'ethereum': return '⟠', case 'polygon': return '⬟', case 'solana': return '◎', case 'syncscript-chain': return '⚡', default: return '🔗'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'contracts',
    name: 'Smart Contracts', icon: '📄'  }, { id: 'nfts',
    name: 'NFT Rewards', icon: '🎨'  }, { id: 'tasks',
    name: 'Decentralized Tasks', icon: '📋'  }, { id: 'governance', name: 'DAO Governance', icon: '🗳️'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">,
          {selectedTab === 'contracts' && (,
            <div className="space-y-6">,
              ...
              ;
              <div className="space-y-4">, {smartContracts.map((contract) => (;
                  <motion.div;
    key={contract.id}
        ...
        ;
          ...
          {selectedTab === 'tasks' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {decentralizedTasks.map((task) => (,
                  <motion.div,
    key={task.id}
        initial={{ opacity: 0,
    x: -20 }},
