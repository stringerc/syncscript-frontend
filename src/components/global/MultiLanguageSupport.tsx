...
interface MultiLanguageSupportProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'languages' | 'translations' | 'packs' | 'settings'>('languages');
  const [ currentLanguage, setCurrentLanguage    ] = useState('en');
  const [ isTranslating, setIsTranslating    ] = useState(false), useEffect(() => {
    loadLanguageData();
  }, []);
  ...
  ,
      ...
      ], setLanguages(mockLanguages), setTranslations(mockTranslations), setLanguagePacks(mockLanguagePacks);
    } catch (error) {
  ...
  const changeLanguage = async (languageCode: string) => {
    setIsTranslating(true), try {
        // Simulate language change
      await new Promise(resolve => setTimeout(resolve, 1000)), setCurrentLanguage(languageCode);
  ...
  const getCompletenessColor = (completeness: number) => {
    if (completeness >= 90) return 'text-green-600 bg-green-100', if (completeness >= 70) return 'text-yellow-600 bg-yellow-100';
    if (completeness >= 50) return 'text-orange-600 bg-orange-100', return 'text-red-600 bg-red-100';
  }
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'ui': return '🎨', case 'navigation': return '🧭', case 'features': return '⚙️', case 'messages': return '💬', case 'errors': return '⚠️', default: return '📄'
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100 mt-1">Internationalization (i18n
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'languages',
    name: 'Languages', icon: '🌍'  }, { id: 'translations',
    name: 'Translations', icon: '📝'  }, { id: 'packs',
    name: 'Language Packs', icon: '📦'  }, { id: 'settings',
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">;
          {selectedTab === 'languages' && (;
            <div className="space-y-6">;
              ...
              ;
              <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">, {languages.map((language) => (<motion.div, key={language.id}
