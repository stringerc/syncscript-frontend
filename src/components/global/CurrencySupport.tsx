...
interface CurrencySupportProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'currencies' | 'budget' | 'transactions' | 'reports'>('currencies');
  const [ baseCurrency, setBaseCurrency    ] = useState('USD');
  const [ isConverting, setIsConverting    ] = useState(false), useEffect(() => {
    loadCurrencyData();
  }, []);
  ...
  ,
      ...
      ], setCurrencies(mockCurrencies), setBudgetCategories(mockBudgetCategories), setTransactions(mockTransactions), setFinancialReports(mockFinancialReports);
    } catch (error) {
  ...
  const convertCurrency = async (amount: number, fromCurrency: string, toCurrency: string) => {
    setIsConverting(true), try {
        // Simulate currency conversion
      await new Promise(resolve = > setTimeout(resolve, 1000)), const fromRate = currencies.find(c => c.code === fromCurrency)?.exchangeRate || 1, const toRate = currencies.find(c => c.code === toCurrency)?.exchangeRate || 1;
  ...
  ;
    ...
    switch (type) {
      case 'income': return 'text-green-600 bg-green-100', case 'expense': return 'text-red-600 bg-red-100', case 'transfer': return 'text-blue-600 bg-blue-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
    ...
    switch (status) {
      case 'completed': return 'text-green-600 bg-green-100', case 'pending': return 'text-yellow-600 bg-yellow-100', case 'cancelled': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'currencies',
    name: 'Currencies', icon: '💱'  }, { id: 'budget',
    name: 'Budget', icon: '💰'  }, { id: 'transactions',
    name: 'Transactions', icon: '📊'  }, { id: 'reports', name: 'Reports', icon: '📈'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">,
          {selectedTab === 'currencies' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">, {currencies.map((currency) => (,
                  <motion.div,
    key={currency.id}
        initial={{ opacity: 0,
    y: 20 }},
  ...
  ,
          ...
          {selectedTab === 'budget' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {budgetCategories.map((category) => (,
                  <motion.div,
    key={category.id}
        initial={{ opacity: 0,
    x: -20 }},
