...
interface Competition {
  id: string,
    name: string,
  description: string,
    type: 'daily' | 'weekly' | 'monthly' | 'seasonal' | 'special', status: 'active' | 'upcoming' | 'ended',
    startDate: string,
  endDate: string,
    participants: number,
  prize: string,
...
interface LeaderboardsSystemProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'global' | 'team' | 'competitions' | 'teams'>('global');
  ...
  ,
      ...
      ], setGlobalLeaderboard(mockGlobalLeaderboard), setTeamLeaderboard(mockTeamLeaderboard), setCompetitions(mockCompetitions), setTeams(mockTeams);
    } catch (error) {
  ...
  ;
    ...
    switch (change) {
      case 'up': return '📈', case 'down': return '📉', case 'same': return '➡️', default: return '📊';
  ;
  ;
  };
  };
    ...
    switch (change) {
      case 'up': return 'text-green-600', case 'down': return 'text-red-600', case 'same': return 'text-gray-600', default: return 'text-gray-600';
  ;
  ;
  };
  };
    ...
    switch (status) {
      case 'active': return 'text-green-600 bg-green-100', case 'upcoming': return 'text-blue-600 bg-blue-100', case 'ended': return 'text-gray-600 bg-gray-100', default: return 'text-gray-600 bg-gray-100'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'global',
    name: 'Global', icon: '🌍'  }, { id: 'team',
    name: 'Teams', icon: '👥'  }, { id: 'competitions',
    name: 'Competitions', icon: '🏆'  }, { id: 'teams', name: 'Join Team', icon: '🤝'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">,
          {selectedTab === 'global' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-3">, {globalLeaderboard.map((entry) => (,
                  <motion.div,
    key={entry.id}
        initial={{ opacity: 0,
    x: -20 }},
    ...
    x: 0 }}
          ...
          {selectedTab === 'team' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-3">, {teamLeaderboard.map((entry) => (;
                  <motion.div;
    key={entry.id}
        ...
        initial={{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}
          ...
          {selectedTab === 'teams' && (;
            <div className="space-y-6">, <h3 className="text-lg font-semibold text-gray-900">Available Teams</h3>;
              ;
              <div className="space-y-4">;
                {teams.map((team) => (;
