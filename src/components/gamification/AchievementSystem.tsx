...
interface Achievement {
    ...
    name: string,
  description: string,
    category: 'productivity' | 'social' | 'streak' | 'milestone' | 'special' | 'seasonal', rarity: 'common' | 'uncommon' | 'rare' | 'epic' | 'legendary',
    icon: string,
  points: number,
    unlocked: boolean,
  progress: number,
...
interface Title {
  id: string,
    name: string,
  description: string,
    rarity: 'common' | 'uncommon' | 'rare' | 'epic' | 'legendary', requirements: string[],
    unlocked: boolean,
    unlockedAt?: string
...
interface AchievementSystemProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'achievements' | 'badges' | 'titles' | 'progress'>('achievements');
  const [ isUnlocking, setIsUnlocking    ] = useState(false), useEffect(() => {
    loadAchievementData();
  }, []);
  ...
  const loadAchievementData = async () => {
    ...
    unlocked: false,
          progress: 0,
    maxProgress: 4,
          requirements: ['Complete Spring,
    Summer, Fall, and Winter challenges'],
          rewards: {
    points: 1000,
            badges: ['seasonal-champion'],
    titles: ['Seasonal Master'],
    unlocks: ['cosmic-theme';
        'special-effects']
  ...
  ,
      ...
      ], setAchievements(mockAchievements), setBadges(mockBadges), setTitles(mockTitles);
    } catch (error) {
  ...
  const unlockAchievement = async (achievementId: string) => {
    setIsUnlocking(true), try {
        // Simulate achievement unlock
      await new Promise(resolve => setTimeout(resolve, 2000)), setAchievements(prev = > prev.map(achievement => 
        ...
        ;
          ...
          : achievement)), console.log({`Achievement unlocked: ${achievementId}: `
  } catch (error) {
  ...
  ;
    ...
    switch (category) {
      case 'productivity': return '📈', case 'social': return '👥', case 'streak': return '🔥', case 'milestone': return '🏆', case 'special': return '⭐', case 'seasonal': return '🌟', default: return '📄'
  ...
  ,
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'achievements',
    name: 'Achievements', icon: '🏆'  }, { id: 'badges',
    name: 'Badges', icon: '🎖️'  }, { id: 'titles',
    name: 'Titles', icon: '👑'  }, { id: 'progress',
    name: 'Progress', icon: '📊'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">,
          {selectedTab === 'achievements' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="grid grid-cols-1 md: grid-cols-2 gap-4">, {achievements.map((achievement) => (,
        ...
        initial={{ opacity: 0, y: 20 }}, animate={{ opacity: 1, y: 0 }}
          ...
          {selectedTab === 'badges' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">, {badges.map((badge) => (,
                  <motion.div,
    key={badge.id}
        initial={{ opacity: 0,
    y: 20 }},
    ...
    y: 0 }}
          ...
          {selectedTab === 'titles' && (,
            <div className="space-y-6">,
              ...
              ;
              <div className="space-y-4">, {titles.map((title) => (;
