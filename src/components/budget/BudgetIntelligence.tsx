...
interface BudgetData {
  ...
  insights: Array<{
    type: 'saving' | 'warning' | 'achievement' | 'suggestion', title: string,
    description: string,
    amount?: number
  }>
    trends: {
    monthlySpending: Array<{ month: string,
    amount: number ;
    ...
    }>, categoryTrends: Array<{ category: string,
  ...
  const [ data, setData    ] = useState<BudgetData | null>(null), const [loading, setLoading] = useState(true), const [activeTab, setActiveTab] = useState<'overview' | 'categories' | 'goals' | 'insights'>('overview');
  ...
  const loadBudgetData = async () => {
    ...
    percentage: 70, color: 'from-blue-400 to-cyan-500'  }, { name: 'Entertainment',
    budget: 400, spent: 280,
    percentage: 70, color: 'from-purple-400 to-indigo-500'  }, { name: 'Shopping',
    budget: 500, spent: 450,
    percentage: 90, color: 'from-green-400 to-emerald-500'  }, { name: 'Utilities',
    budget: 300, spent: 250,
    percentage: 83, color: 'from-yellow-400 to-orange-500'  }, { name: 'Healthcare',
    budget: 200, spent: 150,
    percentage: 75, color: 'from-teal-400 to-blue-500'
  ...
  ,
    ...
    trend: 'up', percentage: 5  }, { category: 'Transportation',
    trend: 'down', percentage: 10  }, { category: 'Entertainment',
    trend: 'stable',
    percentage: 0
  ...
  const getInsightIcon = (type: string) => {
    switch (type) {
      case 'saving': return '💰', case 'warning': return '⚠️', case 'achievement': return '🏆', case 'suggestion': return '💡', default: return '📊';
  ;
  ;
  };
  };
    ...
    switch (type) {
      case 'saving': return 'from-green-400 to-emerald-500', case 'warning': return 'from-red-400 to-pink-500', case 'achievement': return 'from-yellow-400 to-orange-500', case 'suggestion': return 'from-blue-400 to-cyan-500', default: return 'from-gray-400 to-gray-500';
  ;
  ;
  };
  };
    ...
    switch (priority) {
      case 'high': return 'text-red-600 bg-red-50', case 'medium': return 'text-yellow-600 bg-yellow-50', case 'low': return 'text-green-600 bg-green-50', default: return 'text-gray-600 bg-gray-50'
  ...
  }, const getProgressPercentage = (current: number, target: number) => { return Math.min((current / target) * 100, 100);
  }
  if (loading) {
    return (
      ...
      >
        ...
        <div className="bg-gray-50 px-6 py-3 border-b">,
          <div className="flex space-x-1">,
            {[,
              { id: 'overview',
    label: 'Overview', icon: '📊'  }, { id: 'categories',
    label: 'Categories', icon: '📂'  }, { id: 'goals',
    label: 'Savings Goals', icon: '🎯'  }, { id: 'insights', label: 'Insights', icon: '💡'
...
,
                <motion.div,
                  ...
                  transition={{ delay: 0.1 }}, className="bg-gradient-to-r from-red-500 to-pink-500 text-white p-6 rounded-xl";
                >;
                  <div className="flex items-center justify-between">;
                    <div>;
...
;
    ...
    className="bg-gradient-to-r from-green-400 to-emerald-500 h-3 rounded-full transition-all duration-500",
                          style={{ width: `${getProgressPercentage(goal.current,
    goal.target)}%` }}
                        ></div>
                      </div>
                      ...
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>Progress: {
        Math.round(getProgressPercentage(goal.current;
        goal.target))
