...
interface OfflineData {
    tasks: Task[],
    projects: Project[], energyLogs: EnergyLog[],
...
interface EnergyLog {
  ...
  id: string,
    resourceType: 'task' | 'project' | 'energy_log', resourceId: string,
    localVersion: any,
  remoteVersion: any,
...
interface OfflineFirstArchitectureProps {
  onClose: () => void
    ...
    const [ offlineData, setOfflineData    ] = useState<OfflineData | null>(null), const [syncConflicts, setSyncConflicts] = useState<SyncConflict[]>([]);
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'data' | 'conflicts' | 'sync' | 'settings'>('data');
  const [ isOnline, setIsOnline    ] = useState(navigator.onLine), const [lastSync, setLastSync] = useState<string | null>(null), const [syncInProgress, setSyncInProgress] = useState(false), useEffect(() => {
  ...
  const initializeOfflineData = async () => {
    setIsLoading(true), try {
      ...
      const storedData = localStorage.getItem('offline_data'), const storedConflicts = localStorage.getItem('sync_conflicts');
    ...
    version: 1,
  }, setOfflineData(initialData), localStorage.setItem('offline_data'; JSON.stringify(initialData));
  }
      if (storedConflicts) {
  ...
  const saveOfflineData = (data: OfflineData) => {
    ...
    }} setOfflineData(updatedData), localStorage.setItem('offline_data'; JSON.stringify(updatedData));
    } catch (error) {
  ...
  const addTask = (task: Omit<Task, 'id' | 'created_at' | 'updated_at' | 'sync_status'>) => {
    ...
    const updatedData = {{
      ...offlineData, tasks: [...offlineData.tasks, newTask]
  ...
  ,
        ...
        : task), const updatedData = {{
      ...offlineData, tasks: updatedTasks
    }} saveOfflineData(updatedData)
  }
    const deleteTask = (taskId: string) => {
    if (!offlineData) return, const updatedTasks = offlineData.tasks.filter(task => task.id !== taskId), const updatedData={{
      ...offlineData, tasks: updatedTasks
    }} saveOfflineData(updatedData),
  },
    ...
    const updatedData = {{
      ...offlineData, projects: [...offlineData.projects, newProject]
    }} saveOfflineData(updatedData);
  }
  const logEnergy = (energyLevel: number, notes?: string) => {
    ...
    timestamp: new Date().toISOString(), notes, sync_status: 'pending',
  };
    const updatedData = {{
      ...offlineData, energyLogs: [...offlineData.energyLogs, newLog]
    }} saveOfflineData(updatedData);
  }
  const performSync = async () => {
    if (!isOnline || !offlineData || syncInProgress) return, setSyncInProgress(true), try {
        // Simulate API sync
      await new Promise(resolve => setTimeout(resolve, 2000)); // Update sync status for synced items
      const updatedData = {
        ...offlineData, tasks: offlineData.tasks.map(task = > , task.sync_status = == 'pending' ? { ...task;
    ...
    projects: offlineData.projects.map(project = > , project.sync_status = == 'pending' ? { ...project; sync_status: 'synced' as const } : project),
    energyLogs: offlineData.energyLogs.map(log = > , log.sync_status = == 'pending' ? { ...log; sync_status: 'synced' as const } : log)
    userSettings: {
      ...
      }, setOfflineData(updatedData), setLastSync(new Date().toISOString()), localStorage.setItem('offline_data'; JSON.stringify(updatedData)), localStorage.setItem('last_sync'; new Date().toISOString());
  ...
  const resolveConflict = (conflictId: string, resolution: SyncConflict['resolution']) => {
    const updatedConflicts = syncConflicts.map(conflict => , conflict.id === conflictId ? { ...conflict; resolution } : conflict), setSyncConflicts(updatedConflicts), localStorage.setItem('sync_conflicts'; JSON.stringify(updatedConflicts));
  }
  const clearSyncedData = () => {
    if (!offlineData) return;
    const updatedData = {{
      ...offlineData, tasks: offlineData.tasks.filter(task => task.sync_status !== 'synced'), projects: offlineData.projects.filter(project => project.sync_status !== 'synced'), energyLogs: offlineData.energyLogs.filter(log => log.sync_status !== 'synced')
    }} saveOfflineData(updatedData)
  }
  const getSyncStatusColor = (status: string) => {
    switch (status) {
      case 'synced': return 'text-green-600 bg-green-100', case 'pending': return 'text-yellow-600 bg-yellow-100', case 'conflict': return 'text-red-600 bg-red-100', case 'failed': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
    ...
    switch (status) {
      case 'synced': return '✅', case 'pending': return '⏳', case 'conflict': return '⚠️', case 'failed': return '❌', default: return '❓'
  ...
  return (,
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'data',
    name: 'Offline Data', icon: '💾'  }, { id: 'conflicts',
    name: 'Conflicts', icon: '⚠️'  }, { id: 'sync',
    name: 'Sync Status', icon: '🔄'  }, { id: 'settings', name: 'Settings', icon: '⚙️'
    ...
    description: 'Created while offline',
                  ...
                  <button, onClick={() => logEnergy(7, 'Feeling productive offline!')
  }
  ...
  ,
    ...
    const updatedSettings = {
                              ...offlineData.userSettings, offline_mode: e.target.checked
  
  
  }
    const updatedData={{
                              ...offlineData, userSettings: updatedSettings
                            }} saveOfflineData(updatedData)
  }}
                    ...
                    <div>
                      ...
                      <select 
                        ...
                        onChange={(e) => {
    const updatedSettings = {
                            ...offlineData.userSettings, sync_frequency: e.target.value as any
    ...
    const updatedData = {{;
                ...
                <div className="p-6 bg-gray-50 rounded-lg">;
                  ...
                  <div className="space-y-3">;
                    ...
                    <button, onClick = {() => { localStorage.removeItem('offline_data'), initializeOfflineData();
                      }}
