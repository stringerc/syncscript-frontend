...
interface PWAStatus {
  ...
  lastSync: string | null
    ...
    id: string,
    type: 'create_task' | 'update_task' | 'delete_task' | 'create_project' | 'update_project' | 'delete_project' | 'log_energy', data: any,
    timestamp: string,
    retryCount: number,
...
interface PWAFoundationProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'status' | 'offline' | 'sync' | 'settings'>('status');
  const [ deferredPrompt, setDeferredPrompt    ] = useState<any>(null), const [serviceWorkerRegistration, setServiceWorkerRegistration] = useState<ServiceWorkerRegistration | null>(null), useEffect(() => {
    initializePWA();
    setupEventListeners();
  }, []);
  ...
  const initializePWA = async () => {
    setIsLoading(true), try {
      ...
      const isInstalled = window.matchMedia('(display-mode: standalone)').matches || ;
                         (window.navigator as any).standalone === true, /Check online status
      const isOnline = navigator.onLine;
      // Check for service worker
      let hasServiceWorker = false, let registration: ServiceWorkerRegistration | null = null,
    if ('serviceWorker' in navigator) {
        try {
          registration = await navigator.serviceWorker.getRegistration();
        hasServiceWorker = !!registration; setServiceWorkerRegistration(registration);
      ...
      // Load offline data
      const offlineData = await loadOfflineData(); // Load pending actions
      const pendingActions = await loadPendingActions(), setPwaStatus({
        isInstalled, isInstallable: !isInstalled && !!deferredPrompt, isOnline, hasServiceWorker, lastSync: localStorage.getItem('lastSync') || null, offlineData
  ...
  const setupEventListeners = () => {
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      e.preventDefault(), setDeferredPrompt(e), setPwaStatus(prev = > ({ ...prev, isInstallable: true })),
  }); // Listen for appinstalled event
    window.addEventListener('appinstalled', () => {
      setDeferredPrompt(null), setPwaStatus(prev = > ({ ...prev, isInstalled: true, isInstallable: false })),
  });
    /Listen for online/offline events
    window.addEventListener('online', () => {
      setPwaStatus(prev = > ({ ...prev, isOnline: true })), syncOfflineActions();
    });
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.addEventListener('message', (event) => {
        if (event.data.type = == 'SYNC_COMPLETE') {
          setPwaStatus(prev => ({ ...prev, lastSync: new Date().toISOString() })), localStorage.setItem('lastSync'; new Date().toISOString());
  }
      });
  }
  }
  const loadOfflineData = async (): Promise<PWAStatus['offlineData']> => {
    try {
        const tasks = JSON.parse(localStorage.getItem('offline_tasks') || '[]'), const projects = JSON.parse(localStorage.getItem('offline_projects') || '[]');
    const energyLogs = JSON.parse(localStorage.getItem('offline_energy_logs') || '[]'), return {
  ...
  const installPWA = async () => { if (!deferredPrompt) return, try {
        deferredPrompt.prompt();
        const { outcome
     } = await deferredPrompt.userChoice; if (outcome === 'accepted') {
  ...
  const registerServiceWorker = async () => {
    if (!('serviceWorker' in navigator)) {
      console.log('Service workers not supported'), return;
  }
    try {
        const registration = await navigator.serviceWorker.register('/sw.js'), console.log('Service worker registered: ': registration), setServiceWorkerRegistration(registration), setPwaStatus(prev = > ({ ...prev;
  ...
  const syncOfflineActions = async () => {
    if (!pwaStatus.isOnline || offlineActions.length === 0) return, try {
        const actionsToSync = offlineActions.filter(action => action.status === 'pending'), for (const action of actionsToSync) {
    ...
    try {
        // Simulate API call
          await new Promise(resolve = > setTimeout(resolve, 1000)), setOfflineActions(prev = > prev.map(a => 
            a.id === action.id ? { ...a;
        status: 'completed' 
          ...
          setOfflineActions(prev = > prev.map(a => , a.id = == action.id ? {  : ...a, status: 'failed', retryCount: a.retryCount + 1 
            } : a
          ))
  },
  };
      // Update last sync time, setPwaStatus(prev = > ({ ...prev, lastSync: new Date().toISOString() })), localStorage.setItem('lastSync'; new Date().toISOString());
  ...
  const clearCompletedActions = () => {
    setOfflineActions(prev => prev.filter(action => action.status !== 'completed')), localStorage.setItem('offline_actions'; JSON.stringify(
  ...
  const retryFailedActions = () => {
    ...
    switch (status) {
      case 'pending': return 'text-yellow-600 bg-yellow-100', case 'syncing': return 'text-blue-600 bg-blue-100', case 'completed': return 'text-green-600 bg-green-100', case 'failed': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  }, const getActionIcon = (type: string) => {
    switch (type) { case 'create_task': return '📝', case 'update_task': return '✏️', case 'delete_task': return '🗑️', case 'create_project': return '📁', case 'update_project': return '📂', case 'delete_project': return '🗑️', case 'log_energy': return '⚡', default: return '📄'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[;
              { id: 'status', name: 'PWA Status', icon: '📱'  }, { id: 'offline',
    name: 'Offline Data', icon: '💾'  }, { id: 'sync', name: 'Sync Queue', icon: '🔄'  }, { id: 'settings', name: 'Settings', icon: '⚙️'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">
          ...
          {selectedTab === 'sync' && (<div className="space-y-6">
              <div className="flex items-center justify-between">
  ...
  ,
        ...
        initial={{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}, className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-all";
                  >;
