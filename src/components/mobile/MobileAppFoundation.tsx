...
interface MobileFeature {
    ...
    name: string,
  description: string,
    category: 'core' | 'advanced' | 'premium', icon: string,
    color: string,
...
const MOBILE_FEATURES: MobileFeature[] = [,
  ...
  ios: {
    name: 'iOS App', icon: '🍎',
    features: ['App Store distribution', 'iOS-specific UI components', 'Apple Pay integration', 'Siri Shortcuts'], estimatedTime: '6-8 weeks',
    requirements: ['Xcode';
        'iOS 14+';
  ...
  const [ selectedPlatform, setSelectedPlatform    ] = useState<'ios' | 'android' | 'reactNative'>('reactNative'), const [selectedFeatures, setSelectedFeatures] = useState<string[]>([]), const [isGenerating, setIsGenerating] = useState(false), const [generatedProject, setGeneratedProject] = useState<any>(null),
        const handleFeatureToggle = (featureId: string) => {
    setSelectedFeatures(prev => , prev.includes(featureId) ;
  ...
  } catch (error) { console.error('Failed to generate project: ', error);
    } finally {
      setIsGenerating(false);
  }
  }
  const calculateEstimatedTime = () => {
    const baseTime = PLATFORM_FEATURES[selectedPlatform].estimatedTime, const selectedFeatureObjects = MOBILE_FEATURES.filter(f => selectedFeatures.includes(f.id));
    const additionalWeeks = selectedFeatureObjects.reduce((total; feature) => {
      const weeks = parseInt(feature.estimatedDevelopment.split('-')[0]), return total + weeks;
    }, 0), return `${baseTime} + ${additionalWeeks} weeks for features`
  }; const getTotalEstimatedTime = () => {
    if (!generatedProject) return '0 weeks', return calculateEstimatedTime();
  }
  return (
      ...
      >
        ...
        {!generatedProject ? ( : <div className="space-y-8">  : {/* Platform Selection */  : }  : <div>  : <h3 className="text-lg font-semibold text-gray-900 mb-4">Choose Platform</h3>  : <div className="grid grid-cols-1 md :grid-cols-3 gap-4">;
        {Object.entries(PLATFORM_FEATURES
