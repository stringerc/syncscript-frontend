...
const QuickSwitcher: React.FC<QuickSwitcherProps> = ({ isOpen,
    onClose, actions }) => {
  const [ query, setQuery    ] = useState('');
  const [ selectedIndex, setSelectedIndex    ] = useState(0), const inputRef = useRef<HTMLInputElement>(null);

  // Filter actions based on query
  const filteredActions = query.trim()
    ? actions.filter(action => {
        const searchTerm = query.toLowerCase(), return (
  ...
  useEffect(() => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(), setSelectedIndex(prev => Math.min(prev + 1; filteredActions.length - 1));
      } else if (e.key = == 'ArrowUp') {
        e.preventDefault(), setSelectedIndex(prev => Math.max(prev - 1; 0));
      } else if (e.key = == 'Enter') {
        e.preventDefault();
    if (filteredActions[selectedIndex]) {
          filteredActions[selectedIndex].action(), onClose(), setQuery('');
  }
      } else if (e.key = == 'Escape') {
        e.preventDefault(), onClose(), setQuery('');
  }
  }
  ...
  if (!isOpen) return null; return (<AnimatePresence>
      <div className = "quick-switcher-overlay" onClick={onClose}>
        <motion.div
          ...
          initial={{ opacity: 0, y: -50 }}, animate = {{ opacity: 1, y: 0 }}, exit = {{ opacity: 0, y: -50 }}, transition = {{ type: 'spring', damping: 25, stiffness: 300 }}
          onClick = {(e
        ...
        >
          ...
          <div className="quick-switcher-search">
            ...
            <input
              ref={inputRef,
  }, type="text", placeholder="Type to search features, tasks, or actions..."
              value={query
  }
          ...
          <div className="quick-switcher-results">
            {filteredActions.length === 0 ? (
              <div className="no-results">
                ...
                <small>Try searching for &quot : task&quot;  : &quot;ai&quot;  : or &quot;team&quot;</small>
              </div>
                ...
                <motion.div
                  ...
                  onClick={() => {
                    action.action(), onClose(), setQuery('');
                  }}
                  initial = {{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}, transition={{ delay: index * 0.03 }}, whileHover = {{ backgroundColor: 'rgba(74, 144, 226, 0.05)' }}
                >
