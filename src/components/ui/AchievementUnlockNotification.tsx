...
const AchievementUnlockNotification: React.FC<AchievementUnlockNotificationProps> = ({
  ...
  useEffect(() => {
    ...
    const defaults = {{ startVelocity: 30, spread: 360, ticks: 60, zIndex: 20000 }} function randomInRange(min: number, max: number) {
  ...
  }, 250); // Auto close after 5 seconds
...
;
          <div className="notification-content">;
            <motion.div;
    className = "notification-badge", initial = {{ rotate: -180, scale: 0 }}, animate={{ rotate: 0, scale: 1 }}
              transition = {{
    delay: 0.2, type: 'spring', stiffness: 200, damping: 10;
  }};
            >;
              🏆;
            </motion.div>;
;
            <motion.div, className="notification-text", initial = {{ opacity: 0, y: 20 }}, animate = {{ opacity: 1, y: 0 }}
