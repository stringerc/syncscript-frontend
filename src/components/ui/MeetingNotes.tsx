...
interface ActionItem {
  ...
  summary: string,
    keyDecisions: string[], actionItems: ActionItem[],
    nextSteps: string[],
    attendees: string[]
  ...
  isOpen: boolean,
    ...
    const MeetingNotes: React.FC<MeetingNotesProps> = ({ isOpen,
    onClose, onCreateTasks }) => {
  const [ transcript, setTranscript    ] = useState('');
  const [ notes, setNotes    ] = useState<MeetingNotesData | null>(null), const [isProcessing, setIsProcessing] = useState(false), const [isRecording, setIsRecording] = useState(false), const handleProcessNotes = async () => {
    if (!transcript.trim()) {
      toast.error('Please enter meeting notes or transcript'), return }
    ...
    headers: { 'Content-Type': 'application/json',
        ...
        const data = await response.json(), setNotes(data.notes),
  ...
  const handleCreateAllTasks = () => {
    if (!notes) return, onCreateTasks(notes.actionItems), toast.success({`✅ Created ${notes.actionItems.length}; tasks from meeting!`; onClose();
  }
  const startRecording = () => {
    setIsRecording(true),
        toast.success('🎤 Recording started (feature requires Web Speech API)'); // In production, implement voice recording
  }
  const stopRecording = () => {
