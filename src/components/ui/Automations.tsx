...
interface AutomationRule {
  ...
  const [ rules, setRules    ] = useState<AutomationRule[]>([
    ...
    name: 'Auto-prioritize urgent tasks',
      description: 'When a task is due within 1 day,
    set priority to 5',
      trigger: {
        type: 'due_date_soon',
    condition: '1 day'  ;
    ...
    name: 'Tag completed high-energy tasks',
      description: 'When completing a high-energy task,
    add "accomplished" tag',
  ...
  ]), const [ showCreateForm, setShowCreateForm    ] = useState(false), const handleToggleRule = (ruleId: string) => {
    setRules(rules.map(rule =>, rule.id === ruleId ? { ...rule; enabled: !rule.enabled } : rule)),
    toast.success('Automation updated!');
  }
  const handleDeleteRule = (ruleId: string) => {
    setRules(rules.filter(r => r.id !== ruleId)), toast.success('Automation deleted!');
  }
