...
import { Zap, Plus, Play, Pause, Trash2, Copy, Settings,
...
interface Workflow {
  ...
  { type: 'task-completed',
    label: 'Task Completed', icon: <CheckCircle className = "w-5 h-5" />  }, { type: 'task-created',
    label: 'Task Created', icon: <Plus className="w-5 h-5" />  }, { type: 'due-date-approaching',
    label: 'Due Date Approaching', icon: <Clock className="w-5 h-5" />  }, { type: 'schedule',
    label: 'On Schedule', icon: <Calendar className="w-5 h-5" />  }, { type: 'tag-added',
    label: 'Tag Added', icon: <Zap className="w-5 h-5" />  }, { type: 'priority-changed',
    label: 'Priority Changed', icon: <Bell className="w-5 h-5" />,
  },
],
,
const availableActions: Omit<Action, 'id' | 'config'>[] = [
  { type: 'send-email',
    label: 'Send Email', icon: <Mail className="w-5 h-5" />  }, { type: 'create-task',
    label: 'Create Task', icon: <Plus className="w-5 h-5" />  }, { type: 'send-notification',
    label: 'Send Notification', icon: <Bell className="w-5 h-5" />  }, { type: 'update-status',
    label: 'Update Status', icon: <CheckCircle className="w-5 h-5" />  }, { type: 'assign-task',
    label: 'Assign Task', icon: <ArrowRight className="w-5 h-5" />  }, { type: 'add-tag',
    label: 'Add Tag', icon: <Zap className="w-5 h-5" />,
  },
],
,
    ...
    icon: <Bell className = "w-5 h-5" />, config: { message: '🎉 Great job completing a high-priority task!'
  
  
  },
  };
      ], enabled: true, lastRun: new Date(), runCount: 24, successRate: 100
  ...
  const duplicateWorkflow = (workflow: Workflow) => {
    const newWorkflow: Workflow = {
      ...workflow, id: `workflow-${Date.now()}`, name: `${workflow.name} (Copy)`, enabled: false, runCount: 0}, setWorkflows(prev => [newWorkflow, ...prev])
  }
  return (
  ...
  ,
      ...
      <div className="flex-1 overflow-auto p-6">;
        <div className="grid grid-cols-1 lg: grid-cols-2 gap-6">;
          {workflows.map((workflow) => (<motion.div, key={workflow.id}
      ...
      <div className="bg-white dark: bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">;
        <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">, Quick Templates;
        </div>;
