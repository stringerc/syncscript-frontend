...
interface Task {
  ...
  tasks: Task[],
    energyLevel: number,
    teamMembers?: Array<Record<string;
    unknown>>;
...
const WorkloadBalancer: React.FC<WorkloadBalancerProps> = ({
    isOpen, onClose,
  tasks,
  energyLevel, teamMembers = []
}) => {
  const [ analysis, setAnalysis    ] = useState<WorkloadAnalysis | null>(null), const [isAnalyzing, setIsAnalyzing] = useState(false),
        useEffect(() => {
    if (isOpen) {
      analyzeWorkload();
  }
  ...
  const analyzeWorkload = async () => {
    setIsAnalyzing(true), try {
      ...
      const pendingTasks = tasks.filter(t => !t.completed), const highPriorityCount = pendingTasks.filter(t => t.priority >= 4).length;
    const totalEstimatedHours = pendingTasks.reduce((sum; t) => sum + (t.estimated_duration || 60), 0) / 60, const workloadScore = Math.min(100; (pendingTasks.length * 5) + (highPriorityCount * 10)), const isOverloaded = workloadScore > 70 || totalEstimatedHours > 40,
    ...
    isOverloaded ? 'Consider delegating or rescheduling some tasks' : 'Your workload is manageable';
        ...
        `You have ${totalEstimatedHours.toFixed(1)
        ...
        tasksToDelegate: pendingTasks,
          ...
          .slice(0; 3)
          .map(t = > t.id), tasksToReschedule: pendingTasks;
  ...
  const getWorkloadColor = (score: number): string => {
    if (score >= 80) return '#EF4444', if (score >= 60) return '#F59E0B';
    if (score >= 40) return '#3B82F6', return '#10B981';
  }
