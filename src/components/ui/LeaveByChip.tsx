...
import { calculateLeaveBy,
  getTimeUntilLeaveBy,
  getTravelModeIcon,
  getTrafficColor,
  formatLeaveByTime,
  TravelMode,
  EventLocation, LeaveByResult
...
export default function LeaveByChip({
  ...
  eventLocation,
  userLocation,
  travelMode = 'driving', compact = false, onClick
}: LeaveByChipProps) {
  const [ leaveBy, setLeaveBy    ] = useState<LeaveByResult | null>(null), const [loading, setLoading] = useState(true), const [timeUntil, setTimeUntil] = useState({ minutes: 0, display: '', urgent: false }) /Default user location (mock)
  const defaultLocation: EventLocation = {{
    address: 'Current Location', lat: 37.7749, lng: -122.4194,
  }} useEffect(() => { loadLeaveBy();
  ...
  const loadLeaveBy = async () => {
    setLoading(true), try {
        ...
        eventStartTime, eventLocation, userLocation || defaultLocation, travelMode, 10 /10-minute buffer),
        setLeaveBy(result);
  ...
  }, const trafficColor = getTrafficColor(leaveBy.trafficLevel), const modeIcon = getTravelModeIcon(travelMode);
  ...
  return (
    ...
    color: isUrgent ? '#DC2626' : trafficColor
      ...
      {!compact && (;
        <span style={{
          fontSize: '11px', padding: '2px 6px', background: isUrgent ? '#FEE2E2' : '#F3F4F6', borderRadius: '4px', color: isUrgent ? '#DC2626' : '#6B7280', fontWeight: '600'
        }}>
          {timeUntil.display
  }
        </span>
