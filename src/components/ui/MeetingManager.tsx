...
import { Calendar, Users, Clock, FileText, CheckSquare,
...
interface MeetingManagerProps {
  ...
  onActionItemCreate?: (item: ActionItem) => void
    ...
    onMeetingCreate, onActionItemCreate
}) => {
  ...
  const generateAgenda = (title: string, duration: number): AgendaItem[] => {
    ...
    const commonTopics = [;
      'Introductions and check-in', 'Review previous action items';
      'Main discussion';
      'Decision making';
...
;
    lines.forEach((line; i) => {
      if (line.toLowerCase().includes('todo: ') || 
          ...
          line.includes('[ ]')) {
    actions.push({
          id: `action-${i}`, task: line.replace(/todo:|action:|\[ \]/gi; '').trim(),
          assignee: 'Unassigned',
  ...
  const createMeeting = (data: Partial<Meeting>) => {
    ...
    id: `meeting-${Date.now()}`, title: data.title || 'New Meeting',
    date: data.date || new Date(),
      duration: data.duration || 60,
    attendees: data.attendees || [],
      agenda: data.agenda || generateAgenda(data.title || 'New Meeting',
    data.duration || 60), notes: data.notes || '',
    actionItems: data.actionItems || [],
      cost: calculateMeetingCost(data.attendees ? .length || 3 : data.duration || 60)  : status: 'upcoming'
  ...
  const completeMeeting = (meetingId: string, notes: string) => {
    setMeetings(prev => prev.map(m => {; if (m.id = == meetingId) {
    const actionItems = extractActionItems(notes),
        return {
          ...m, notes, actionItems, status: 'completed' as const
  ;
  ;
  };
  }, return m;
  ...
  ,
      ...
      <div className="flex-1 overflow-auto p-6">,
        {view === 'list' && (,
          <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">, {meetings.map((meeting) => (<motion.div,
    key={meeting.id}
        initial={{ opacity: 0,
    y: 20 }},
        ...
        createMeeting({
                  title: formData.get('title') as string, date: new Date(formData.get('date') as string), duration: Number(formData.get('duration')), attendees: (formData.get('attendees') as string).split('; ').map(a = > a.trim())
                })
              }}
              className="space-y-4"
            >
...
,
    ...
    required,
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button, type="submit", className="flex-1 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors";
                >, Create Meeting;
                </button>;
              </div>;
            </form>;
    ...
    return {;
                                ...m, agenda: m.agenda.map(a = >, a.id = == item.id ? { ...a; completed: !a.completed } : a
                                )
  }
  }
                            return m
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => {
    setMeetings(prev => prev.map(m =>, m.id = == selectedMeeting.id ? { ...m; notes: e.target.value } : m
                    ))
                  }}
        ...
        setMeetings(prev = > prev.map(m =>, m.id = == selectedMeeting.id ? { ...m; actionItems: items } : m
                      ))
                    }}
                      ...
                      <input
                        ...
                        onChange={() => {
                          setMeetings(prev => prev.map(m => {
                            if (m.id === selectedMeeting.id) {
    return {;
                                ...m, actionItems: m.actionItems.map(a = >, a.id = == item.id ? { ...a; completed: !a.completed } : a
                                )
  }
  }
                            return m
                  ...
                  {selectedMeeting.actionItems.length === 0 && (;
                    <div className = "text-center py-6 text-gray-500 dark: text-gray-400 text-sm">, No action items yet. Add notes with &quot, TODO: &quot, to auto-generate.
                    </div>
                  )
  }
                </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-200 dark: border-gray-700">
                ...
                <button, onClick = {() => completeMeeting(selectedMeeting.id, selectedMeeting.notes)
  }
