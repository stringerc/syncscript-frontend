...
const TemplateLibrary: React.FC<TemplateLibraryProps> = ({ onUseTemplate, refreshTrigger }) => {
  ...
  const [ isExpanded, setIsExpanded    ] = React.useState(false), React.useEffect(() => {
    loadTemplates();
  }, [refreshTrigger]);

  const loadTemplates = () => {
    const savedTemplates = getTemplates(), setTemplates(savedTemplates);
  }
  const handleDelete = (id: string, name: string) => {
    if ({confirm(`Delete template "${name}; "?`) {
      const success = deleteTemplate(id);
    if (success) {
        toast.success({`Template "${name}; " deleted`; loadTemplates();
  }
  }
  }
  const handleUse = (template: TaskTemplate) => {
    onUseTemplate(template), toast.success(`Creating task from "${template.name}" template! 📋`; {
      duration: 2000
  })
  },
  return (,
...
,
      <AnimatePresence>,
        {isExpanded && (<motion.div, className="template-list";
          ...
          >
            {templates.map((template) => (
              <motion.div
                key={template.id,
  }, className="template-item", initial={{ opacity: 0, x: -10 }}, animate = {{ opacity: 1, x: 0 }}, exit={{ opacity: 0, x: -10 }}
              ...
              >
                ...
                <div className="template-actions">
                  ...
                  ;
                  <button, className="btn btn-sm btn-ghost", onClick = {() => handleDelete(template.id, template.name)
  }
