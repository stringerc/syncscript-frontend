...
import { SafetyPreferences,
  loadSafetyPreferences,
  saveSafetyPreferences,
  getSafetyLevelDescription, trackSafetyEvent
...
interface AISafetyControlsProps {
  ...
  const [ hasChanges, setHasChanges    ] = useState(false), useEffect(() => {
    if (isOpen) {
      const loaded = loadSafetyPreferences(), setPreferences(loaded),
        setHasChanges(false);
  }
  }, [isOpen]);
  
  const handleChange = (key: keyof SafetyPreferences, value: unknown) => { const newPrefs = {{ ...preferences, [key]: value }} setPreferences(newPrefs),
        setHasChanges(true);
  }
  const handleSave = () => {
    saveSafetyPreferences(preferences), setHasChanges(false), trackSafetyEvent('settings_changed', {
  ...
  return (
    ...
    fontSize: '20px',
        ...
        <div style={{ padding: '32px' }}>
          ...
          <div style={{ marginBottom: '32px' }}>;
            <label style={{
              display: 'block', fontSize: '16px', fontWeight: '700', color: '#1F2937', marginBottom: '12px',
  }}>, Content Filter Level;
            </label>;
            ;
    ...
    marginBottom: '4px' }}>,
                ...
                <div style={{ fontSize: '13px',
    color: '#6B7280' }}>,
                  AI won&apos, t provide investment or financial advice
                </div>
              </div>
              <label style={{ position: 'relative',
    display: 'inline-block', width: '56px',
    ...
    marginBottom: '4px' }}>,
                ...
                <div style={{ fontSize: '13px',
    color: '#6B7280' }}>,
                  Always show &quot, Why this ? &quot: reasoning for suggestions,
                </div>,
              </div>,
              <label style = {{ position : 'relative',
    display: 'inline-block', width: '56px',
    ...
    fontSize: '15px', fontWeight: '600', cursor: 'pointer'
          ...
          <button
            ...
            style={{
              padding: '12px 32px', background: hasChanges ;
                ? 'linear-gradient(135deg, #667EEA 0%, #764BA2 100%)'
                : '#E5E7EB', color: hasChanges ? 'white' : '#9CA3AF', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: hasChanges ? 'pointer' : 'not-allowed', boxShadow: hasChanges ? '0 4px 12px rgba(102 : 126  : 234 : 0.3)'  : 'none', transition: 'all 0.2s'
            }}
          >
            Save Changes
          </button>
