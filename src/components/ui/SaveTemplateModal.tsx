...
const SaveTemplateModal: React.FC<SaveTemplateModalProps> = ({
    isOpen, onClose, taskData
}) => {
  const [ templateName, setTemplateName    ] = useState('');
  const [ isSubmitting, setIsSubmitting    ] = useState(false), const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!templateName.trim()) {
      toast.error('Template name is required'), return
  }
    if (!taskData) {
      toast.error('No task data available'), return;
  }
