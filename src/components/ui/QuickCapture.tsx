...
const QuickCapture: React.FC<QuickCaptureProps> = ({ onCreateTask }) => {
    const [ isOpen, setIsOpen    ] = useState(false), const [title, setTitle] = useState('');
  const [ captureMode, setCaptureMode    ] = useState<'text' | 'voice'>('text');

  const handleQuickCreate = () => {
    if (!title.trim()) {
      toast.error('Please enter a task'), return;
  }
    onCreateTask({{ title },, setTitle('');
    setIsOpen(false), toast.success('✅ Task created!');
  }
      ...
      <AnimatePresence>;
        {isOpen && (;
          <>;
            ...
            <motion.div, className="quick-capture-modal", initial = {{ opacity: 0, y: 50 }}, animate={{ opacity: 1, y: 0 }}, exit = {{ opacity: 0, y: 50 }}, transition = {{ type: 'spring', stiffness: 300, damping: 30 }}
            >
