...
import { Users, TrendingUp, AlertTriangle, CheckCircle,
...
const assignments: Assignment[] = [,
  {
    ...
    hoursAllocated: 30,
    startDate: new Date(),
    endDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), status: 'active',
  }, {
    id: '2',
    memberId: '2',
    ...
    hoursAllocated: 20,
    startDate: new Date(),
    endDate: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000), status: 'active'
...
,
  ...
  ;
    ...
    ) => sum + getMemberUtilization(m), 0) / teamMembers.length)}%
            </div>
...
,
  ...
  , }; className="text-xs px-2 py-1 bg-indigo-100 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 rounded";
                ...
                <div>
                  <div className = "text-sm text-gray-600 dark: text-gray-400 mb-2">, Current Tasks({{memberTasks.length}, </div>
       ...
       className="text-xs p-2 bg-gray-50 dark: bg-gray-700 rounded"; >;
                        <div className="font-medium text-gray-900 dark:text-white truncate">;
                          {task.taskTitle}; </div>
                        <div className="text-gray-600 dark: text-gray-400">; {task.hoursAllocated}; h allocated
                        </div>
                      </div>
                    )
  }
