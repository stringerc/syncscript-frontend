...
const DataExport: React.FC<DataExportProps> = ({
  ...
  const [ includeCompleted, setIncludeCompleted    ] = useState(true), const [dateRange, setDateRange] = useState<'all' | 'week' | 'month' | 'custom'>('all');

  const handleExport = () => {
    try {
        let dataToExport = tasks; // Filter by date range
      if (dateRange = == 'week') {
        const weekAgo = new Date(), weekAgo.setDate(weekAgo.getDate() - 7),
        dataToExport = filterTasksByDateRange(tasks;
        weekAgo);
        ...
        const monthAgo = new Date(), monthAgo.setMonth(monthAgo.getMonth() - 1), dataToExport = filterTasksByDateRange(tasks, monthAgo);
  }
      ...
      switch (exportType) {
        case 'tasks':
          ...
          break,
        case 'energy':
          exportEnergyLogsToCSV(energyLogs), break,
        case 'analytics':
          const analytics = {{
            totalTasks: tasks.length, completedTasks: tasks.filter(t = > t.completed).length, completionRate: tasks.length > 0 ? (tasks.filter(t = > t.completed).length / tasks.length * 100).toFixed(1) : 0, avgEnergy: energyLogs.length > 0 ? (energyLogs.reduce((sum: number, log: DataExportEnergyLog) => sum + log.energy_level, 0) / energyLogs.length).toFixed(1) : 0, productivityScore: 85, /Calculate this properly
            streak: 0, /Get from streak utils
            totalPoints: 0 /Get from gamification,
  }} exportAnalyticsToCSV(analytics), break, case 'complete':
          exportCompleteBackup({ tasks, projects, energyLogs, user }) break
  }
      toast.success({`✅ Exported ${exportType}; successfully!`; onClose();
    } catch (error) {
