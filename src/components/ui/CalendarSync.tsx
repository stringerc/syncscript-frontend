...
interface CalendarEvent {
  ...
  const [ providers, setProviders    ] = useState<CalendarProvider[]>([
    ...
    color: 'from-blue-500 to-blue-600', connected: false  }, { id: 'outlook',
    name: 'Outlook Calendar', icon: '📧',
    color: 'from-blue-600 to-indigo-600', connected: false  }, { id: 'apple',
    name: 'Apple Calendar', icon: '🍎',
    color: 'from-gray-700 to-gray-800', connected: false  }, { id: 'notion',
    name: 'Notion', icon: '📓',
    color: 'from-gray-800 to-black', connected: false
...
,
  ...
  const connectProvider = async (providerId: string) => {
    ...
    ,
    // Simulate OAuth connection, await new Promise(resolve = > setTimeout(resolve, 2000))
    
    setProviders(providers.map(p => 
      p.id === providerId ? { ...p: connected : true, lastSync: new Date(), eventsCount: Math.floor(Math.random() * 50) + 10
  ...
  const disconnectProvider = (providerId: string) => {
    ...
    p.id === providerId ,
        ? { ...p, connected: false,
    lastSync: undefined, eventsCount: 0
  ...
  const generateDemoEvents = (providerId: string) => {
    ...
    , for(let i = 0, i < 8, i++) {
      ...
      newEvents.push({
        id: `${providerId}-event-${i}`, title: getRandomEventTitle(), start, end, calendar: providerId, type: Math.random() > 0.5 ? 'event' : 'task', synced: true, location: Math.random() > 0.5 ? 'Virtual' : undefined, attendees: Math.random() > 0.5 ? ['user@example.com'] : undefined
      })
  ...
  const syncNow = async (providerId?: string) => {
    ...
    setProviders(providers.map(p = > ; p.connected ? { ...p; lastSync: new Date() } : p
      ))
  }
    setSyncInProgress(false)
  }
  ...
  return (
      ...
      <div className="max-w-7xl mx-auto">;
        {/* Header */}, <div className = "bg-white dark: bg-gray-800 rounded-2xl p-8 shadow-lg mb-6">;
          <div className="flex items-start justify-between mb-6">;
            <div>;
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">;
                📅 Calendar Integration;
              </h1>;
              <p className="text-gray-600 dark:text-gray-400">, Connect your calendars and sync tasks bidirectionally;
              </p>;
            </div>;
          </div>;
;
        ...
        <div className="grid grid-cols-1 lg: grid-cols-2 gap-6">;
          {/* Calendar Providers */}, <div className = "bg-white dark: bg-gray-800 rounded-2xl p-6 shadow-lg">;
...
;
            <div className="space-y-4">;
              {providers.map((provider => (;
                <motion.div, key = {provider.id}, initial = {{ opacity: 0,
    x: -20 }},
                      ...
                      <div>
                        ...
                        {provider.connected && (
                          <div className="text-sm text-gray-600 dark:text-gray-400">
                            ...
                            {provider.lastSync && (
                              <span className="ml-2">
                                • Synced {new Date(provider.lastSync
    ...
    scale: 1 }}
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                      <div>🕐 {event.start.toLocaleString(
