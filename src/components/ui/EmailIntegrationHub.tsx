...
import { Mail, Inbox, CheckCircle, Clock, AlertCircle,
  Zap, ArrowRight, Filter, Search, Send
    } from 'lucide-react'
import { Email, EmailTask, parseEmailToTask, sortInboxByPriority,
...
const EmailIntegrationHub: React.FC<EmailIntegrationHubProps> = ({
  ...
  const [ emails, setEmails    ] = useState<Email[]>([
    ...
    body: 'Hi, can you please review the Q4 financial report by end of day ? It\'s critical for tomorrow\'s board meeting.' : receivedAt: new Date(Date.now() - 2 * 60 * 60 * 1000),
    priority: 'high',
      hasAttachments: true,
    labels: ['work', 'urgent']
  }
  ...
  const convertToTask = (email: Email) => {
    ...
    setEmails(prev => prev.filter(e => e.id !== email.id)) :   :   : }  : const categories = ['all'  : 'Meetings'  : 'Tasks'  : 'Projects'  : 'Urgent'  : 'Updates'];
, return (;
        ;
  ...
  ,
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">;
                ...
                {email.receivedAt.toLocaleTimeString() }, {email.hasAttachments && (
                  <>
                    <span>•</span>
            ...
            <div className="flex-1 overflow-auto p-6">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl p-6 mb-6">;
                <h4 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">;
                  <Send className="w-5 h-5 text-blue-600 dark:text-blue-400" />, Smart Reply Suggestions;
                </h4>;
