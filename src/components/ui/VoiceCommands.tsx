...
interface VoiceCommandsProps {
    ...
    ;
    ;
        type: string, data?: Record<string;
        unknown> 
...
const VoiceCommands: React.FC<VoiceCommandsProps> = ({ onCommand }) => {
    const [ isListening, setIsListening    ] = useState(false), const [lastCommand, setLastCommand] = useState('');
  ...
  const [ recognition, setRecognition    ] = useState<any>(null), useEffect(() => {
    ...
    const recog = new SpeechRecognition(), recog.continuous = true, recog.interimResults = false, recog.lang = 'en-US';

    /eslint-disable-next-line @typescript-eslint/no-explicit-any
    recog.onresult = (event: any) => {
    const last = event.results.length - 1;
    const command = event.results[last][0].transcript.toLowerCase(), setLastCommand(command),
  ...
  const processCommand = (command: string) => {
    if (command.includes('create task') || command.includes('new task')) { onCommand({{ type: 'CREATE_TASK' },, toast.success('🎤 Creating new task...');
  ...
  const toggleListening = () => {
    if (isListening) {
      recognition ? .stop() : setIsListening(false)  : toast.success('🎤 Voice commands off');
    } else {
      recognition?.start();
      setIsListening(true)  : toast.success('🎤 Voice commands on - Say "Hey SyncScript"');
  }
  }
  return (
        ...
        {isListening && (
          ...
          >;
            <div className = "listening-indicator">;
              <span className="pulse"></span>, Voice commands active;
            </div>;
            {lastCommand && (;
