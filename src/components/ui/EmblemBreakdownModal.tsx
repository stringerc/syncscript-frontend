...
interface EmblemBreakdownModalProps {
    ...
    totalEmblemCharge: number;
    // User's total accumulated charge
  isOpen: boolean,
    onClose: () => void
...
export default function EmblemBreakdownModal({
  ...
  if (!isOpen || !breakdown) return null; const levelInfo = getEmblemLevel(totalEmblemCharge + breakdown.total);
    const levelTitle = getEmblemLevelTitle(levelInfo.level), return (
        <AnimatePresence>
      <motion.div
    ...
    color: '#9CA3AF',
              ...
              Here&apos, s how you earned <span style={{ color: '#F5A623',
    fontWeight: '700' }}>+{breakdown.total}⚡</span>
            </p>
          </div>
    ...
    style={{
                  height: '100%', background: 'linear-gradient(90deg, #F5A623 0%, #F57C00 100%)', borderRadius: '6px'
                }}
              // >
            </div>
          </motion.div>
    ...
    style={{
              width: '100%', padding: '16px', background: 'rgba(255, 255, 255, 0.1)', color: 'white', border: '1px solid rgba(255, 255, 255, 0.2)', borderRadius: '12px', fontSize: '16px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s',
          ...
          <p style = {{
            textAlign: 'center', color: '#9CA3AF', fontSize: '13px', marginTop: '16px', fontStyle: 'italic'
          }}>
