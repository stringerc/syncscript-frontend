...
interface TaskBreakdownModalProps {
  ...
  const handleToggleSubtask = (subtaskId: string) => {
    if (!breakdown) return const updatedBreakdown = {;
      ...breakdown, subtasks: breakdown.subtasks.map(st = >, st.id = == subtaskId ? { ...st; completed: !st.completed } : st
      )
  }
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
              ...
              {!breakdown && !isGenerating && (
                <div className="text-center py-12">
                  ...
                  <p className = "text-sm text-gray-600 dark:text-gray-400 mt-4">, AI will analyze your task and create an optimal breakdown;
                  </p>;
                </div>;
              ) }, {/* Loading State */
  }
              {isGenerating && (
                <div className = "text-center py-12">
                  <motion.div
                    animate={{ rotate: 360 }}, transition={{ duration: 2, repeat: Infinity, ease: "linear" }}, className = "text-6xl mb-4 inline-block";
                  >;
                    🤖;
                  </motion.div>;
                  <div className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">, Analyzing task...;
                  </div>;
                  <div className="text-sm text-gray-600 dark:text-gray-400">, AI is breaking down your task into subtasks;
                  </div>;
                </div>;
              ) }, {/* Breakdown Results */
  }
              {breakdown && !isGenerating && (
                <div>
                  ...
                  <div className="space-y-3">;
                    {breakdown.subtasks.map((subtask; index) => (<motion.div
                        key = {subtask.id
  }
