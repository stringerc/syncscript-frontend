...
const TeamAnalyticsDashboard: React.FC = () => {
    ...
    { label: 'Team Velocity',
    value: 42, change: 8,
    icon: <TrendingUp className="w-6 h-6" />, color: 'text-blue-600'  }, { label: 'Collaboration Score',
    value: 87, change: 5,
    icon: <Users className="w-6 h-6" />, color: 'text-purple-600'  }, { label: 'Tasks Completed',
    value: 156, change: 12,
    icon: <CheckCircle className="w-6 h-6" />, color: 'text-green-600'  }, { label: 'Avg Response Time',
    value: 1.8, change: -0.3,
    icon: <Clock className="w-6 h-6" />, color: 'text-orange-600'  }, { label: 'Messages Sent',
    value: 847, change: 15,
    icon: <MessageCircle className="w-6 h-6" />, color: 'text-indigo-600'  }, { label: 'Goals Achieved',
    value: 12, change: 3,
    icon: <Target className="w-6 h-6" />, color: 'text-pink-600'
...
,
    ...
    { name: 'Sarah Chen',
    avatar: '👩‍💻', tasks: 42, score: 95  }, { name: 'Mike Johnson', avatar: '👨‍🎨', tasks: 38, score: 92  }, { name: 'Alex Kumar', avatar: '👨‍💼', tasks: 35, score: 88
...
;
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">;
        {metrics.map(metric => (;
          <motion.div, key = {metric.label}, initial = {{ opacity: 0,
    scale: 0.9 }},
