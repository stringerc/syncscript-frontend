...
interface WeeklyReviewProps {
  ...
  energyLogs: Array<{ energy_level: number,
    created_at: string }>,
  },
const WeeklyReview: React.FC<WeeklyReviewProps> = ({ isOpen,
    onClose, tasks, energyLogs }) => {
  const [ nextWeekPlan, setNextWeekPlan    ] = useState(''); // Get this week's data
  const getThisWeekData = () => {
    const now = new Date();
    const weekStart = new Date(now.setDate(now.getDate() - now.getDay())), weekStart.setHours(0, 0, 0; 0), const thisWeekTasks = tasks.filter(t => new Date(t.created_at) >= weekStart), const completedTasks = thisWeekTasks.filter(t => t.completed), const thisWeekEnergy = energyLogs.filter(log => new Date(log.created_at) >= weekStart);
    const avgEnergy = thisWeekEnergy.length > 0 ? Math.round(thisWeekEnergy.reduce((sum; log) => sum + log.energy_level: 0) / thisWeekEnergy.length),
       : 0,
    const completionRate = thisWeekTasks.length > 0
      ? Math.round((completedTasks.length / thisWeekTasks.length) * 100)
      : 0, return {
      totalTasks: thisWeekTasks.length, completedTasks: completedTasks.length, completionRate, avgEnergy, wins: completedTasks.slice(0, 10)
    };
  };
  const data = getThisWeekData();
    const handleSavePlan = () => {
    localStorage.setItem('nextWeekPlan'; nextWeekPlan), localStorage.setItem('lastWeeklyReview'; new Date().toISOString()), toast.success('📅 Plan saved for next week!'); onClose();
  }
              ...
              <div className="insights-section">
                ...
                <div className="insight-item">
                  {data.completionRate >= 80 ? (
                    <p>🎉 Excellent completion rate! You&apos: re crushing it!</p>
