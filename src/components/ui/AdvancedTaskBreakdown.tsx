...
interface AdvancedTaskBreakdownProps {
  ...
  const analyzeTask = async () => {
    ...
    const generatedSubtasks: Subtask[] = breakdown.subtasks.map((st, index) => ({
      id: st.id, title: st.title, description: st.description, estimated_duration: st.estimatedDuration, priority: 3 as 1 | 2 | 3 | 4 | 5, /Default priority
      dependencies: st.dependsOn || [], order: st.order, completed: st.completed
    }))
  ...
  const getPriorityColor = (priority: number) => {
    ...
    case 5: return 'from-gray-500 to-gray-600', default: return 'from-gray-500 to-gray-600';
  ;
  ;
  };
    ...
    y: 20 }},
          ...
          >
            ...
            <div className="bg-white dark: bg-gray-800 rounded-2xl p-6 shadow-lg mb-6">
              ...
              <AnimatePresence>,
                {showExplanation && (;
                  <motion.div, initial={{ height: 0, opacity: 0 }}, animate={{ height: 'auto', opacity: 1 }}, exit={{ height: 0, opacity: 0 }}, className="mt-4 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl";
                  >;
                    <p className="text-sm text-gray-700 dark:text-gray-300">;
                      <strong>🧠 AI Analysis: </strong> I&apos, ve broken down this task based on complexity, dependencies, and best practices. The subtasks are ordered for optimal workflow, with critical path items prioritized first.
                    </p>
                  </motion.div>
                )
  }
            ...
            <div className = "space-y-4">
              {subtasks.map((subtask; index) => (<motion.div
                  key = {subtask.id
  }
                  initial={{ opacity: 0,
    x: -20 }},
...
;
                  {selectedSubtask.dependencies.length > 0 && (<div>;
                      <h5 className="font-semibold text-gray-900 dark:text-white mb-2">, Dependencies;
                      </h5>;
