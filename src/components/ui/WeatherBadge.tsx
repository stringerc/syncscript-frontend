...
import { getWeatherForEvent,
  getWeatherColor,
  formatTemperature,
  WeatherCondition, EventLocation
...
export default function WeatherBadge({
  eventTime,
  location,
  compact = false, onClick
}: WeatherBadgeProps) {
  const [ weather, setWeather    ] = useState<WeatherCondition | null>(null), const [loading, setLoading] = useState(true), const [showDetails, setShowDetails] = useState(false),
        useEffect(() => {
    loadWeather();
  }, [eventTime, location]);
  ...
  if (loading || !weather) {
    return (
        <div style = {{
        display: 'inline-flex', alignItems: 'center', padding: compact ? '4px 8px' : '6px 12px', background: '#F3F4F6', borderRadius: '8px', fontSize: compact ? '11px' : '12px',
    color: '#9CA3AF';
        gap: '4px'
      }}>
  ...
  return (
    ...
    style={{
              position: 'absolute', top: '100%', left: 0, marginTop: '8px', background: 'white', borderRadius: '12px', padding: '16px', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.15)', zIndex: 1000, minWidth: '280px', border: `2px solid ${severityColor}`
            }}
          >
            {/* Close button */,
  };
    ...
    fontSize: '12px',
            ...
            {weather.alerts.length > 0 && (<div style={{
                background: '#FEE2E2',
    borderRadius: '8px', padding: '12px', marginBottom: '12px',
  }}>;
                <div style={{
                  fontSize: '13px', fontWeight: '700', color: '#DC2626', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px', }}>;
                  ⚠️ Weather Alerts;
                </div>;
                {weather.alerts.map((alert; idx) => (
                  <div key = {idx} style={{ marginBottom: '6px' }}>,
                    <div style={{
                      fontSize: '12px',
    fontWeight: '600', color: '#991B1B', marginBottom: '2px',
  }}>;
            ...
            {weather.recommendations.length > 0 && (<div>;
                <div style={{
                  fontSize: '12px', fontWeight: '700', color: '#4B5563', marginBottom: '6px', }}>;
                  💡 Recommendations:;
                </div>;
                {weather.recommendations.map((rec; idx) => (
                  <div
                    ...
                    style={{
                      fontSize: '11px', color: '#6B7280', marginBottom: '4px', paddingLeft: '8px'
                    }}
                  >
                    • {rec
  }
