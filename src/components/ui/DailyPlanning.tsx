...
interface DailyPlan {
  ...
  isOpen: boolean,
    onClose: () => void,
  tasks: Task[],
    energyPredictions: Array<Record<string,
    unknown>>;
...
const DailyPlanning: React.FC<DailyPlanningProps> = ({ isOpen,
    onClose, tasks, energyPredictions }) => {
  const [ plan, setPlan    ] = useState<DailyPlan | null>(null), const [isGenerating, setIsGenerating] = useState(false), useEffect(() => {
    if (isOpen && !plan) {
      generatePlan();
  }
  }, [isOpen]);
  ...
  const generatePlan = async () => {
    ...
    headers: { 'Content-Type': 'application/json',
        ...
        const data = await response.json(), setPlan(data.plan),
  ...
  if (!isOpen) return null; return (<div className = "daily-planning-overlay" onClick={onClose}>
      <motion.div
      ...
      >
        ...
        <div className="daily-planning-content">
          {isGenerating ? (
            ...
            <>
              {/* Morning */ : }  : <div className="time-block morning">  : <div className="block-header">  : <h3>🌅 Morning</h3>  : <span className="energy-badge high">High Energy</span>  : {plan.morning.focusBlock && (  : <span className="focus-time">{plan.morning.focusBlock}</span>  : )  : }  : </div>  : <p className="block-message">{plan.morning.message}</p>  : <div className="block-tasks">  : {plan.morning.tasks.map(taskId => {; const task = tasks.find(t => t.id === taskId),
        return task ? (
