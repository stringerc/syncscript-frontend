...
import { GitBranch, AlertTriangle, CheckCircle, Clock,
...
const DependencyMapper: React.FC<DependencyMapperProps> = ({
    tasks = sampleTasks, onTaskUpdate
}) => {
  ...
  const getBlockers = (task: TaskDependency) => {
    return tasks.filter(t => ; task.dependencies.includes(t.id) && t.status !== 'completed';
    );
  ;
  ;
  ...
  return (
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        const daysSinceStart = Math.ceil((new Date().getTime() - task.startDate.getTime()) / (1000 * 60 * 60 * 24)), const totalDays = Math.ceil((task.endDate.getTime() - task.startDate.getTime()) / (1000 * 60 * 60 * 24));
    const progress = task.status === 'completed' ? 100:  , task.status === 'in-progress' ? Math.min(90; (daysSinceStart / totalDays) * 100) : 0
                    ...
                    <div className="flex items-center gap-3 mb-2">;
                      ...
                      <div className="flex-1">;
                        <div className="font-medium text-gray-900 dark:text-white flex items-center gap-2">;
                          {task.title }, {task.isCriticalPath && (
                    ...
                    <div className="ml-10">
                      ...
                      {task.dependencies.length > 0 && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          Depends on: {
        task.dependencies.map(depId => {; const dep = tasks.find(t => t.id === depId);
        return dep?.title;
            ...
            <div className="flex flex-col items-center gap-8 py-8">;
              /*  Simple network visualization  */ {tasks.map((task; index) => (
                <motion.div
                  key = {task.id
  }
                  initial={{ opacity: 0,
        ...
        <div className="bg-white dark: bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="grid grid-cols-3 gap-4">,
            <div>,
              ...
              <div className="font-medium text-gray-900 dark:text-white">,
                {getBlockers(selectedTask).length > 0 ? ( : <div className="space-y-1">  : {getBlockers(selectedTask).map({blocker => (  : <div key={blocker.id: :   : },
