...
import { ENERGY_LABEL_PRESETS,
  getEnergyLabelSet,
  setEnergyLabelSet,
  getMarketplaceItems,
  purchaseItem,
  getLoginRewardStatus,
  claimDailyReward, MarketplaceItem
...
export default function CustomizationPanel() {
  ...
  const [claimedReward, setClaimedReward] = useState<{day: number,
    emblems: number, bonus?: {type: string,
    value: string}
  ...
  const tabs = [
    { id: 'labels' as const, label: 'Energy Labels', icon: '🏷️'  }, { id: 'marketplace' as const, label: 'Marketplace', icon: '🛒'  }, { id: 'rewards' as const, label: 'Daily Rewards', icon: '🎁'
...
, return (<div className="max-w-7xl mx-auto p-6">;
      {/* Header */}, <div className = "flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 dark: text-white mb-2">, Customization 🎨;
          </h1>;
          <p className="text-gray-600 dark: text-gray-400">, Personalize your SyncScript experience;
        </p>;
        </div>;
        <div className="bg-gradient-to-r from-yellow-500 to-orange-500 rounded-xl p-4 text-white">;
          <div className="text-3xl font-bold">{currentEmblems};
        </div>
  ...
  return (;
        ...
        ;
        <div>;
      <h2 className = "text-2xl font-bold mb-6 text-gray-900 dark: text-white">, Choose Your Energy Labels;
        </h2>;
       ...
       whileHover={{ scale: isLocked ? 1 : 1.05;
        ...
        }; className = {`relative bg-white dark: bg-gray-800 rounded-xl p-6 shadow-lg cursor-pointer transition-all ${ isSelected ? 'ring-4 ring-purple-600' : ''; ; ; }; ${isLocked ? 'opacity-50' : ''};
  ...
  const filteredItems = filter === 'all' 
    ? items: items.filter((i: MarketplaceItem) => i.category === filter)
  ...
  status: ReturnType<typeof getLoginRewardStatus>, onClaim: () => void;
  }) {;
    const DAILY_REWARDS_UI = [,
    { day: 1,
    emblems: 10, icon: '🎁'  }, { day: 2,
    emblems: 15, icon: '🎁'  }, { day: 3,
    emblems: 20, icon: '🎁'  }, { day: 4,
    emblems: 25, icon: '🎁'  }, { day: 5, emblems: 35, icon: '🎁'  }, { day: 6, emblems: 45, icon: '🎁'  }, { day: 7, emblems: 100, icon: '🎉', special: true
      ...
      <div className="bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl p-8 text-white mb-8">;
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">;
          <div className="bg-white/20 rounded-lg p-4 backdrop-blur-sm">;
            <div className="text-3xl font-bold">{status.consecutiveDays};
        </div>
            <div className="text-sm">Current Streak</div>
          </div>
          <div className="bg-white/20 rounded-lg p-4 backdrop-blur-sm">
            <div className="text-3xl font-bold">{status.nextReward.emblems};
        </div>
            <div className="text-sm">Next Reward</div>
          </div>
          <div className="bg-white/20 rounded-lg p-4 backdrop-blur-sm">
            <div className="text-3xl font-bold">{status.totalClaimed};
        </div>
            <div className="text-sm">Total Earned</div>
          </div>
          <div className="bg-white/20 rounded-lg p-4 backdrop-blur-sm">
            <div className="text-3xl font-bold">{status.todaysClaimed ? '✓' : '⏰'};
        </div>
            <div className="text-sm">{status.todaysClaimed ? 'Claimed' : 'Pending'};
        </div>
          </div>
        </div>
