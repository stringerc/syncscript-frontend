...
interface Task {
  ...
  const taskChain = getDependencyChain(task.id, dependencies, allTasks), const blockingReasons = getBlockingReasons(task.id, dependencies, allTasks), const suggestions = generateDependencySuggestions(allTasks, dependencies), const availableTasks = allTasks.filter(t => 
    t.id !== task.id && 
    !t.completed &&
    ...
    const newDependency = createDependency(task.id, selectedTaskId, dependencyType), onAddDependency(newDependency),
        setSelectedTaskId('');
  }
  const getDependencyTypeColor = (type: TaskDependency['type']) => {
    switch (type) {
      case 'blocks': return '#EF4444', case 'requires': return '#F59E0B', case 'suggests': return '#10B981', default: return '#6B7280';
  ;
  ;
  };
  };
    ...
    switch (type) {
      case 'blocks': return '🚫', case 'requires': return '⚡', case 'suggests': return '💡', default: return '🔗';
  ;
  ;
  };
  };
    ...
    switch (type) {
      case 'blocks': return 'Blocks', case 'requires': return 'Requires', case 'suggests': return 'Suggests', default: return 'Unknown'
    ...
    exit={{ opacity: 0, scale: 0.9 }}, transition = {{ type: 'spring', stiffness: 300, damping: 30 }}
          ...
          >;
            ...
            <div className="dependency-modal-content">
              {activeTab === 'current' && (<div className="current-dependencies">
                  {/* Blocking Status */
  }
                  {taskChain.isBlocked && (
       ...
       className="dependency-item">
                ...
                <div className="add-dependency">
                  ...
                  <div className="form-group">
                    ...
                    >;
                      ...
                      {availableTasks.map(task => (; <option key={task.id;
  }; value={task.id; }; >;
                          {task.title}; </option>
                      )
  }
              ...
              {activeTab === 'suggestions' && (
                <div className="dependency-suggestions">
                  ...
                  <p className="suggestions-description">, AI-powered suggestions based on task context and patterns;
                  </p>;
                  ;
                  {suggestions.filter(s => s.taskId === task.id).length === 0 ? ( : <div className="no-suggestions">  : <span className="no-suggestions-icon">💡</span>  : <p>No dependency suggestions available</p>  : </div>  : )  : (;
       ...
       className="suggestion-item">
        ...
        % match
                                ...
                                <p className = "suggestion-reason">{suggestion.reason};
        </p>
                              </div>
                              <button
    ...
    ), setDependencyType('requires'), setActiveTab('add');
                                }}
                              >
                                Add
                              </button>
