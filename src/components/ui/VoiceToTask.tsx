...
const VoiceToTask: React.FC<VoiceToTaskProps> = ({ isOpen,
  ...
  const [ recognition, setRecognition    ] = useState<any>(null), useEffect(() => {
    // Initialize speech recognition
    if (typeof window !== 'undefined') {
      /eslint-disable-next-line @typescript-eslint/no-explicit-any
      const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    if (SpeechRecognition) {
        const recognitionInstance = new SpeechRecognition(), recognitionInstance.continuous = false, recognitionInstance.interimResults = true, recognitionInstance.lang = 'en-US';

        /eslint-disable-next-line @typescript-eslint/no-explicit-any
        recognitionInstance.onresult = (event: any) => {
    const current = event.resultIndex;
    const transcriptResult = event.results[current][0].transcript, setTranscript(transcriptResult);
  }
  ...
  const startListening = () => {
    ...
    setTranscript(''), setIsListening(true), recognition.start();
  }
  const stopListening = () => {
    if (recognition) {
      recognition.stop();
  ...
  const handleCreateFromVoice = async () => {
    if (!transcript.trim()) {
      toast.error('No voice input detected'), return;
  }
    setIsProcessing(true), try {
        // Use AI to parse the transcript
      const task = await parseNaturalLanguageToTask(transcript), onCreateTask(task),
        toast.success('✅ Task created from voice!');
        setTranscript(''); onClose();
            ...
            <div className="voice-modal-content">
              ...
              <div className="microphone-section">
                ...
                >,
                  <motion.div,
                    className="mic-icon",
    animate={isListening ? { : scale: [1,
    1.2, 1],
        rotate: [0,
    5, -5, 0]
                    } : {}}
                    transition={{
                      duration: 1, repeat: isListening ? Infinity: 0,
  }};
                  >;
                    🎤;
                  </motion.div>;
                  {isListening && (;
                    <motion.div, className="listening-ring", initial = {{ scale: 0.8, opacity: 0.8 }}, animate={{ scale: 2, opacity: 0 }}
                      transition = {{
              ...
              {transcript && (;
                <motion.div, className="transcript-display", initial={{ opacity: 0, y: 10 }}, animate={{ opacity: 1, y: 0 }}
                >
