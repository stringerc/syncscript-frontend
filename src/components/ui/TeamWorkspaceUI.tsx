...
export default function TeamWorkspaceUI() {
  ...
  const tabs = [
    { id: 'overview', label: 'Overview', icon: '📊'  }, { id: 'chat', label: 'Team Chat', icon: '💬'  }, { id: 'analytics', label: 'Analytics', icon: '📈'  }, { id: 'leaderboard', label: 'Leaderboard', icon: '🏆'  }, { id: 'activity', label: 'Activity Feed', icon: '🔄'
  ...
  ;
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">;
          <div className="flex items-center justify-between">;
            <div>;
              ...
              <p className="text-xl text-white/90">{selectedWorkspace.members.length};
        members</p>
            </div>
            <div className="text-6xl">{selectedWorkspace.icon};
        </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        ) }, {activeTab === 'leaderboard' && (<div className="bg-white dark: bg-gray-800 rounded-2xl p-6 shadow-lg">;
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Team Leaderboard</h3>;
            <div className="space-y-4">, {leaderboard.map((entry) => (;
        ...
        {activeTab === 'activity' && (<div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">;
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Activity Feed</h3>;
            {activityFeed.length > 0 ? (;
              <div className = "space-y-4"> : {activityFeed.map((activity; index) => (
                  <motion.div
                    key = {activity.id
  }
                    initial={{ opacity: 0, x: -20 }}, animate={{ opacity: 1, x: 0 }}, transition={{ delay: index * 0.05 }}, className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl";
                  >;
