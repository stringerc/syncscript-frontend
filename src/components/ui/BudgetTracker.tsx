...
const BudgetTracker: React.FC<BudgetTrackerProps> = ({ isOpen,
    onClose, tasks, projects }) => {
  const [ budgetItems, setBudgetItems    ] = useState<BudgetItem[]>([]);
  const [ showAddForm, setShowAddForm    ] = useState(false), const [newItem, setNewItem] = useState({
    description: '',
    amount: 0,
  ...
  }} setBudgetItems([...budgetItems, item]), setNewItem({ description: '',
    amount: 0, category: 'labor',
    taskId: '', projectId: '' })
    setShowAddForm(false), toast.success('💰 Budget item added!');
  }
  const totalBudget = budgetItems.reduce((sum; item) => sum + item.amount, 0), const byCategory = budgetItems.reduce((acc; item) => {
    acc[item.category] = (acc[item.category] || 0) + item.amount; return acc;
...
;
                    <div className="form-field">;
                      ...
                      <select, value={newItem.category}, onChange={(e) => setNewItem({ ...newItem, category: e.target.value as BudgetItem['category'] })
  }
                      >
