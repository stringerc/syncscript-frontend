...
const AVAILABLE_SSO_PROVIDERS: SSOProvider[] = [,
  {
    ...
    configuration: {
        ...
        'bind_dn': 'CN = service-account,OU=Service Accounts,DC=your-domain;
        DC = com'
  ...
  const [ ssoProviders, setSsoProviders    ] = useState<SSOProvider[]>(AVAILABLE_SSO_PROVIDERS), const [selectedProvider, setSelectedProvider] = useState<SSOProvider | null>(null), const [isConfiguring, setIsConfiguring] = useState(false), const [isTesting, setIsTesting] = useState(false), const [testResults, setTestResults] = useState<any>(null), const [configurationStep, setConfigurationStep] = useState<'basic' | 'mapping' | 'security' | 'test'>('basic');
  ...
  const handleConfigureProvider = async (providerId: string, config: Partial<SSOProvider['configuration']>) => {
    ...
    }; isConfigured: true; ; ; }; : provider)); console.log('✅ SSO provider configured successfully');
    } catch (error) {
  ...
  const handleActivateProvider = async (providerId: string) => {
    try {
      // Simulate provider activation, await new Promise(resolve = > setTimeout(resolve, 1000)), setSsoProviders(prev = > prev.map(provider => 
        provider.id === providerId 
          ? { ...provider;
        isActive: true;
        ;
  ...
  const getProviderStatusColor = (provider: SSOProvider) => {
    ...
    if (provider.isConfigured) return 'text-blue-600 bg-blue-100', return 'text-gray-600 bg-gray-100';
  }
  const getProviderStatusText = (provider: SSOProvider) => {
    if (provider.isActive) return 'Active';
    if (provider.isConfigured) return 'Configured', return 'Available';
  }
  return (
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 mt-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="bg-white/20 px-2 py-1 rounded-full text-sm font-medium">
                    {ssoProviders.filter(p => p.isActive
    ...
    type="text",
                        ...
                        <div>;
                          <label className="block text-sm font-medium text-gray-700 mb-1">, Bind DN;
                          </label>;
                          <input, type="text", placeholder="CN=service-account,OU=Service Accounts,DC=your-domain,DC=com"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus: ring-2 focus:ring-blue-500 focus:border-transparent"
                          // >
                        </div>
                        <div>
                      ...
                      <div className={`p-4 rounded-lg ${
                        ...
                        {testResults.userInfo && (;
                          <div className="text-sm text-gray-700">;
                            ...
                            <div>Groups: {
        testResults.userInfo.groups.join(';
        ')
    }</div>
                          </div>
                        )
                ...
                <div className="pt-4 border-t border-gray-200">;
                  <button, onClick = {() => handleConfigureProvider(selectedProvider.id, selectedProvider.configuration)
  }
