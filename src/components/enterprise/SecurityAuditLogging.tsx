...
interface AuditEvent {
  ...
  action: string,
    resource: string,
  resourceId: string,
    category: 'authentication' | 'authorization' | 'data_access' | 'configuration' | 'security' | 'system', severity: 'low' | 'medium' | 'high' | 'critical',
    ipAddress: string,
  userAgent: string,
    location: string,
...
const COMPLIANCE_FRAMEWORKS: ComplianceFramework[] = [,
  ...
  ,
    ...
    name: 'Consumer Rights Management',
        description: 'Enable consumers to access,
  ...
  const [ securityMetrics, setSecurityMetrics    ] = useState<SecurityMetrics>({
    ...
    dataBreaches: 0,
    complianceScore: 0,
  }) const [complianceFrameworks, setComplianceFrameworks] = useState<ComplianceFramework[]>(COMPLIANCE_FRAMEWORKS), const [selectedEvent, setSelectedEvent] = useState<AuditEvent | null>(null), const [selectedFramework, setSelectedFramework] = useState<ComplianceFramework | null>(null), const [filterCategory, setFilterCategory] = useState<string>('all');
  ...
  const [ isLoading, setIsLoading    ] = useState(true), useEffect(() => {
    ...
    const loadAuditData = async () => {
      setIsLoading(true), try {
        await new Promise(resolve => setTimeout(resolve, 2000)); // Generate mock audit events
    ...
    size: '2.5MB';
        ...
        setAuditEvents(mockEvents), setSecurityMetrics({
          totalEvents: mockEvents.length,
    criticalEvents: mockEvents.filter(e = > e.severity === 'critical').length,
          failedLogins: mockEvents.filter(e => e.action === 'LOGIN_FAILED').length, suspiciousActivity: mockEvents.filter(e => e.severity === 'high').length, dataBreaches: 0, complianceScore: 95
        })
  ...
  const filteredEvents = auditEvents.filter(event => {
    const categoryMatch = filterCategory === 'all' || event.category === filterCategory, const severityMatch = filterSeverity === 'all' || event.severity === filterSeverity;
    const statusMatch = filterStatus === 'all' || event.status === filterStatus; return categoryMatch && severityMatch && statusMatch;
  });

  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'critical': return 'text-red-800 bg-red-100', case 'high': return 'text-orange-800 bg-orange-100', case 'medium': return 'text-yellow-800 bg-yellow-100', case 'low': return 'text-green-800 bg-green-100', default: return 'text-gray-800 bg-gray-100';
  ;
  ;
  };
  };
    ...
    switch (status) {
      case 'success': return 'text-green-600', case 'failure': return 'text-red-600', case 'warning': return 'text-yellow-600', default: return 'text-gray-600';
  ;
  ;
  };
  };
    ...
    switch (status) {
      case 'compliant': return 'text-green-600 bg-green-100', case 'partial': return 'text-yellow-600 bg-yellow-100', case 'non_compliant': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100'
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <div className="text-sm font-medium text-gray-700">IP Address</div> : <div className="text-gray-900">{selectedEvent.ipAddress}</div>  : </div>  : <div>  : <div className="text-sm font-medium text-gray-700">Location</div>  : <div className="text-gray-900">{selectedEvent.location}</div>  : </div>  : <div>  : <div className="text-sm font-medium text-gray-700">User Agent</div>  : <div className="text-gray-900 text-xs break-all">{selectedEvent.userAgent}</div>  : </div>  : <div>  : <div className="text-sm font-medium text-gray-700">Details</div>;
