...
interface Webhook {
  ...
  url: string,
    description: string,
  events: string[],
    status: 'active' | 'inactive' | 'error', secret: string
    retryPolicy: {
    maxRetries: number,
  ...
  eventType: string,
    payload: any,
  timestamp: string,
    status: 'pending' | 'delivered' | 'failed' | 'retrying', attempts: number,
    response?: {
    statusCode: number,
...
interface WebhookSystemProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'webhooks' | 'events' | 'logs' | 'settings'>('webhooks');
  const [ selectedWebhook, setSelectedWebhook    ] = useState<Webhook | null>(null), const [isCreating, setIsCreating] = useState(false), useEffect(() => {
    loadWebhookData();
  }, []);
  ...
  ,
      ...
      ], setWebhooks(mockWebhooks), setWebhookEvents(mockEvents);
    } catch (error) {
  ...
  const updateWebhook = async (webhookId: string, updates: Partial<Webhook>) => {
    try {
      setWebhooks(prev => prev.map(webhook => , webhook.id === webhookId ? { ...webhook;
        ...updates 
  ...
  const testWebhook = async (webhookId: string) => {
    ...
    if (!webhook) return, const testEvent: WebhookEvent = {
    id: `test_${Date.now();
    ...
    setWebhookEvents(prev => [testEvent, ...prev]); // Simulate webhook delivery
      setTimeout(() => {
        setWebhookEvents(prev = > prev.map(event => 
          event.id === testEvent.id ? { ...event: status : 'delivered', attempts: 1
  ...
  const getStatusColor = (status: string) => {
    switch (status) { case 'active': return 'text-green-600 bg-green-100', case 'inactive': return 'text-gray-600 bg-gray-100', case 'error': return 'text-red-600 bg-red-100', case 'delivered': return 'text-green-600 bg-green-100', case 'failed': return 'text-red-600 bg-red-100', case 'pending': return 'text-yellow-600 bg-yellow-100', case 'retrying': return 'text-blue-600 bg-blue-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
  const getEventIcon = (eventType: string) => {
    switch (eventType) { case 'task_created': return '📝', case 'task_updated': return '✏️', case 'task_completed': return '✅', case 'task_deleted': return '🗑️', case 'project_created': return '📁', case 'project_archived': return '📦', case 'energy_logged': return '⚡', case 'user_activity': return '👤', case 'test_event': return '🧪', default: return '📄'
      ...
      >
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 mt-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="bg-white/20 px-2 py-1 rounded-full text-sm font-medium">
                    {webhooks.filter(w => w.status === 'active'
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'webhooks',
    name: 'Webhooks', icon: '🔗'  }, { id: 'events',
    name: 'Events', icon: '📊'  }, { id: 'logs',
    name: 'Logs', icon: '📋'  }, { id: 'settings',
  ...
  ,
          ...
          {selectedTab === 'events' && (,
            <div className="space-y-6">,
              ...
              ;
              <div className="space-y-3">, {webhookEvents.map((event) => (;
