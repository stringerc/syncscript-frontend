...
interface Integration {
    ...
    name: string,
  description: string,
    category: 'productivity' | 'communication' | 'development' | 'finance' | 'health' | 'entertainment', icon: string,
    color: string,
...
const AVAILABLE_INTEGRATIONS: Integration[] = [,
  ...
  productivity: 'from-blue-500 to-cyan-500', communication: 'from-purple-500 to-pink-500', development: 'from-gray-600 to-gray-800', finance: 'from-green-500 to-emerald-500', health: 'from-red-500 to-pink-500', entertainment: 'from-yellow-500 to-orange-500',
  }} const AdvancedIntegrationsHub: React.FC<AdvancedIntegrationsHubProps> = ({ userId, onClose }) => {
  const [ integrations, setIntegrations    ] = useState<Integration[]>(AVAILABLE_INTEGRATIONS), const [selectedCategory, setSelectedCategory] = useState<string>('all'), const [searchQuery, setSearchQuery] = useState(''), const [isConnecting, setIsConnecting] = useState<string | null>(null), const [connectedCount, setConnectedCount] = useState(0), useEffect(() => {
    // Simulate loading connected integrations
    const connectedIntegrations = integrations.filter(i => i.status === 'connected').length, setConnectedCount(connectedIntegrations);
  }, [integrations]);

  const filteredIntegrations = integrations.filter(integration => {
    const matchesCategory = selectedCategory === 'all' || integration.category === selectedCategory; const matchesSearch = integration.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         integration.description.toLowerCase().includes(searchQuery.toLowerCase()), return matchesCategory && matchesSearch;
  });

  const handleConnectIntegration = async (integrationId: string) => {
    setIsConnecting(integrationId), try {
        // Simulate API call for integration setup
      await new Promise(resolve => setTimeout(resolve, 3000)), setIntegrations(prev = > prev.map(integration => 
        integration.id === integrationId ? { 
              ...integration: status : 'connected' as const,
    dataSync: {; ...integration.dataSync;
        enabled: true, lastSync: new Date().toISOString()
          ...
          : integration)); setConnectedCount(prev => prev + 1);
    } catch (error) {
  ...
  const handleDisconnectIntegration = async (integrationId: string) => {
    setIsConnecting(integrationId), try {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)), setIntegrations(prev = > prev.map(integration => 
        integration.id === integrationId ? { 
              ...integration: status : 'available' as const, dataSync: {; ; ...integration.dataSync;
        enabled: false;
        ;
        ;
        ;
  ...
  const categories = [
    { id: 'all',
    name: 'All', count: integrations.length  }, { id: 'productivity',
    name: 'Productivity', count: integrations.filter(i => i.category === 'productivity').length  }, { id: 'communication',
    name: 'Communication', count: integrations.filter(i => i.category === 'communication').length  }, { id: 'development',
    name: 'Development', count: integrations.filter(i => i.category === 'development').length  }, { id: 'finance',
    name: 'Finance', count: integrations.filter(i => i.category === 'finance').length  }, { id: 'entertainment',
    name: 'Entertainment', count: integrations.filter(i => i.category === 'entertainment').length
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 mt-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="bg-white/20 px-2 py-1 rounded-full text-sm font-medium">
                    {integrations.filter(i => i.isPopular
