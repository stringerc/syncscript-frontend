...
interface AnalyticsData {
    ...
    avgEnergy: number, totalTasks: number }>,
...
interface AnalyticsDashboardProps {
  ...
  const [ data, setData    ] = useState<AnalyticsData | null>(null), const [loading, setLoading] = useState(true), const [activeTab, setActiveTab] = useState<'overview' | 'energy' | 'productivity' | 'insights'>('overview');
  ...
  const loadAnalyticsData = async () => {
    ...
    setData(mockData); // Track analytics dashboard view
  ...
  const generateDailyData = () => {
    const data = [], for(let i = 6, i >= 0, i--) {
      const date = new Date(), date.setDate(date.getDate() - i), data.push({
        date: date.toISOString().split('T')[0],
        energy: Math.floor(Math.random() * 10) + 1, tasks: Math.floor(Math.random() * 8) + 2
      })
  }, return data;
  };
    const generateWeeklyData = () => {
    const data = [], for(let i = 3, i >= 0, i--) {
      const date = new Date(), date.setDate(date.getDate() - (i * 7)), data.push({
        week: `Week ${4 - i}`, avgEnergy: Math.floor(Math.random() * 5) + 5, totalTasks: Math.floor(Math.random() * 20) + 10
      })
  }, return data;
  }, const generateMonthlyData = () => {
    const data = [], for(let i = 2, i >= 0, i--) {
      const date = new Date(), date.setMonth(date.getMonth() - i), data.push({
    ...
    switch (type) {
    case 'achievement': return '🏆', case 'suggestion': return '💡', case 'warning': return '⚠️', case 'celebration': return '🎉', default: return '📊';
  ;
  ;
  };
  };
    ...
    switch (type) {
      case 'achievement': return 'from-yellow-400 to-orange-500', case 'suggestion': return 'from-blue-400 to-cyan-500', case 'warning': return 'from-red-400 to-pink-500', case 'celebration': return 'from-green-400 to-emerald-500', default: return 'from-gray-400 to-gray-500'
      ...
      >
        ...
        <div className="bg-gray-50 px-6 py-3 border-b">,
          <div className="flex space-x-1">,
            {[,
              { id: 'overview',
    label: 'Overview', icon: '📊'  }, { id: 'energy',
    label: 'Energy Patterns', icon: '⚡'  }, { id: 'productivity',
    label: 'Productivity', icon: '🎯'  }, { id: 'insights',
    label: 'Insights', icon: '💡'
...
,
                <motion.div,
                  ...
                  transition={{ delay: 0.1 }}, className="bg-gradient-to-r from-green-500 to-emerald-500 text-white p-6 rounded-xl";
                >;
                  <div className="flex items-center justify-between">;
                    <div>;
