...
interface CustomMetric {
    ...
    name: string,
  description: string,
    type: 'number' | 'percentage' | 'currency' | 'ratio' | 'score', formula: string,
    dataSource: string,
...
interface CustomReport {
  id: string,
    name: string,
  description: string,
    category: 'executive' | 'operational' | 'financial' | 'custom', widgets: ReportWidget[],
    layout: 'grid' | 'freeform' | 'dashboard', theme: 'light' | 'dark' | 'corporate' | 'modern',
    isPublic: boolean,
  isScheduled: boolean
    schedule: {
...
interface CustomReportCreationProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'metrics' | 'widgets' | 'reports' | 'sources'>('metrics');
  const [ isCreating, setIsCreating    ] = useState(false), useEffect(() => {
  ...
  ,
    ...
    height: 200
        ...
        'show_legend': true
          ...
          data: [,
            { date: '2024-01-01',
    completion_rate: 75.2  }, { date: '2024-01-02',
    completion_rate: 78.9  }, { date: '2024-01-03',
    ...
    height: 250
        ...
        'pagination': true
          ...
          data: [,
            { team: 'Development',
    score: 92.5, tasks: 45,
    efficiency: 94.2  }, { team: 'Design',
    score: 88.3, tasks: 32,
    efficiency: 89.7  }, { team: 'Marketing',
    score: 85.7, tasks: 28,
    efficiency: 87.3
  ...
  ,
    ...
    category: 'executive',
          widgets: mockReportWidgets.slice(0, 2), layout: 'dashboard',
    theme: 'corporate',
          isPublic: false,
    isScheduled: true,
          schedule: {
    ...
    category: 'operational',
          widgets: mockReportWidgets.slice(2, 4), layout: 'grid',
    theme: 'modern',
          isPublic: true,
    isScheduled: false,
          schedule: {
    ...
    costs, and profitability analysis',
          category: 'financial',
    widgets: mockReportWidgets.slice(0, 3), layout: 'freeform',
    theme: 'light',
          isPublic: false,
    isScheduled: true,
          schedule: {
  ...
  ,
      ...
      ], setCustomMetrics(mockCustomMetrics), setReportWidgets(mockReportWidgets), setCustomReports(mockCustomReports), setDataSources(mockDataSources);
    } catch (error) {
  ...
  const createCustomReport = async () => {
    ...
    lastModified: new Date().toISOString(), lastGenerated: new Date().toISOString(),
  }, setCustomReports(prev = > [...prev, newReport]), console.log('Created custom report: ': newReport.id);
    } catch (error) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) { case 'number': return '🔢', case 'percentage': return '📊', case 'currency': return '💰', case 'ratio': return '⚖️', case 'score': return '⭐', case 'metric': return '📈', case 'chart': return '📊', case 'table': return '📋', case 'text': return '📝', case 'image': return '🖼️', case 'database': return '🗄️', case 'api': return '🔌', case 'file': return '📁', case 'manual': return '✋', default: return '📄';
  ;
  ;
  };
  };
  const getCategoryColor = (category: string) => {
    switch (category) { case 'productivity': return 'text-blue-600 bg-blue-100', case 'team': return 'text-green-600 bg-green-100', case 'financial': return 'text-yellow-600 bg-yellow-100', case 'operational': return 'text-purple-600 bg-purple-100', case 'executive': return 'text-red-600 bg-red-100', case 'custom': return 'text-gray-600 bg-gray-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
  const getThemeColor = (theme: string) => {
    switch (theme) {
      case 'light': return 'text-gray-600 bg-gray-100', case 'dark': return 'text-gray-800 bg-gray-200', case 'corporate': return 'text-blue-600 bg-blue-100', case 'modern': return 'text-purple-600 bg-purple-100', default: return 'text-gray-600 bg-gray-100'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'metrics',
    name: 'Custom Metrics', icon: '📊'  }, { id: 'widgets',
    name: 'Report Widgets', icon: '🧩'  }, { id: 'reports',
    name: 'Custom Reports', icon: '📋'  }, { id: 'sources', name: 'Data Sources', icon: '🗄️'
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh]">,
          {selectedTab === 'metrics' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {customMetrics.map((metric) => (,
                  <motion.div,
    key={metric.id}
        initial={{ opacity: 0,
    x: -20 }},
        ...
        initial={{ opacity: 0, y: 20 }}, animate={{ opacity: 1, y: 0 }}
          ...
          {selectedTab === 'reports' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {customReports.map((report) => (,
                  <motion.div,
    key={report.id}
        initial={{ opacity: 0,
    x: -20 }},
