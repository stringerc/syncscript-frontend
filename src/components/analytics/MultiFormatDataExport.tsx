...
interface ExportTemplate {
    ...
    name: string,
  description: string,
    category: 'productivity' | 'team' | 'financial' | 'custom', format: string,
    dataSource: string,
  filters: Record<string, any>;
  isPublic: boolean,
    isScheduled: boolean,
...
interface MultiFormatDataExportProps {
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'formats' | 'jobs' | 'templates' | 'sources'>('formats');
  const [ isExporting, setIsExporting    ] = useState(false), useEffect(() => {
    loadExportData();
  }, []);
  ...
  const loadExportData = async () => {
    ...
    icon: '📊',
          isSupported: true, maxRecords: 1000000,
    features: ['Large datasets';
  ...
  ,
      ...
      ], setExportFormats(mockExportFormats), setExportJobs(mockExportJobs), setExportTemplates(mockExportTemplates), setDataSources(mockDataSources);
    } catch (error) {
  ...
  const startExport = async (templateId: string) => {
    ...
    fileSize: 0, createdAt: new Date().toISOString(),
  }, setExportJobs(prev = > [newJob, ...prev]), console.log('Started export job: ': newJob.id);
  }
    } catch (error) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'pending': return 'text-yellow-600 bg-yellow-100', case 'processing': return 'text-blue-600 bg-blue-100', case 'completed': return 'text-green-600 bg-green-100', case 'failed': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
    ...
    switch (category) {
      case 'productivity': return 'text-blue-600 bg-blue-100', case 'team': return 'text-green-600 bg-green-100', case 'financial': return 'text-yellow-600 bg-yellow-100', case 'custom': return 'text-gray-600 bg-gray-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
    ...
    switch (type) {
      case 'database': return '🗄️', case 'api': return '🔌', case 'file': return '📁', case 'manual': return '✋', default: return '📄'
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[;
              { id: 'formats', name: 'Export Formats', icon: '📊'  }, { id: 'jobs', name: 'Export Jobs', icon: '⚙️'  }, { id: 'templates', name: 'Templates', icon: '📋'  }, { id: 'sources', name: 'Data Sources';
    icon: '🗄️'
          ...
          {selectedTab === 'formats' && (;
            <div className="space-y-6">;
              ...
              <div className="grid grid-cols-1 md: grid-cols-2 lg:grid-cols-3 gap-4">;
                {exportFormats.map((format) => (<motion.div, key={format.id}
        initial={{ opacity: 0, y: 20 }}, animate={{ opacity: 1, y: 0 }}
          ...
          {selectedTab === 'jobs' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {exportJobs.map((job) => (,
                  <motion.div,
    key={job.id}
        initial={{ opacity: 0,
    x: -20 }},
    ...
    x: 0 }}
          ...
          {selectedTab === 'templates' && (,
            <div className="space-y-6">,
              ...
              ,
              <div className="space-y-4">, {exportTemplates.map((template) => (,
