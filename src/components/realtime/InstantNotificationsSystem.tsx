...
interface Notification {
    id: string,
    type: 'task_assigned' | 'task_due' | 'project_update' | 'team_mention' | 'deadline_approaching' | 'milestone_reached' | 'system_alert' | 'collaboration_invite', title: string,
    message: string,
  userId: string,
    teamId: string,
  timestamp: string,
  ...
  priority: 'low' | 'medium' | 'high' | 'urgent',
    category: 'task' | 'project' | 'team' | 'system' | 'collaboration', actionUrl?: string;
    metadata: {;
    ;
    ;
        resourceId?: string;
...
interface InstantNotificationsSystemProps {
    ...
    const InstantNotificationsSystem: React.FC<InstantNotificationsSystemProps> = ({ teamId,
    userId, onClose }) => {
  const [ notifications, setNotifications    ] = useState<Notification[]>([]);
  const [ preferences, setPreferences    ] = useState<NotificationPreferences | null>(null), const [stats, setStats] = useState<NotificationStats | null>(null), const [isLoading, setIsLoading] = useState(true), const [selectedTab, setSelectedTab] = useState<'notifications' | 'preferences' | 'stats' | 'settings'>('notifications');
  ...
  const loadNotificationData = async () => {
    setIsLoading(true), try {
        // Simulate API calls
      await new Promise(resolve => setTimeout(resolve, 1500)); // Mock notifications
    ...
    total: mockNotifications.length, unread: mockNotifications.filter(n => !n.read).length
    byType: {
          'task_assigned': 1;
  ...
  }, setNotifications(mockNotifications), setPreferences(mockPreferences), setStats(mockStats);
    } catch (error) {
  ...
  const deleteNotification = (notificationId: string) => {
    ...
    if (preferences) { setPreferences({ ...preferences, ...newPreferences });
  }
  }
  const filteredNotifications = notifications.filter(notif => {
    const typeMatch = filterType === 'all' || notif.type === filterType;
    const priorityMatch = filterPriority === 'all' || notif.priority === filterPriority; const readMatch = filterRead === 'all' || (filterRead === 'read' ? notif.read: !notif.read),
        return typeMatch && priorityMatch && readMatch;
  });

  const getNotificationIcon = (type: string) => {
    switch (type) { case 'task_assigned': return '📝', case 'task_due': return '⏰', case 'project_update': return '📂', case 'team_mention': return '👥', case 'deadline_approaching': return '⚠️', case 'milestone_reached': return '🎯', case 'system_alert': return '🚨', case 'collaboration_invite': return '🤝', default: return '📄';
  ;
  ;
  };
  };
  ...
  ;
    ...
    switch (category) {
      case 'task': return 'text-blue-600 bg-blue-100', case 'project': return 'text-purple-600 bg-purple-100', case 'team': return 'text-green-600 bg-green-100', case 'system': return 'text-red-600 bg-red-100', case 'collaboration': return 'text-indigo-600 bg-indigo-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[;
              { id: 'notifications', name: 'Notifications', icon: '🔔'  }, { id: 'preferences', name: 'Preferences', icon: '⚙️'  }, { id: 'stats', name: 'Statistics', icon: '📊'  }, { id: 'settings', name: 'Settings';
    icon: '🔧'
                        ...
                        {['task_assigned', 'team_mention', 'collaboration_invite', 'deadline_approaching'].map((type) => (
                          <label key={type} className="flex items-center">
                            <input 
                              ...
                              onChange={(e) => {
                                const newTypes = e.target.checked ? [...preferences.push.types: type],
                                   : preferences.push.types.filter(t => t !== type), updatePreferences({
                                  push: {
        ...preferences.push;
        types: newTypes
