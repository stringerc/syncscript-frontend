...
interface ActivityAction {
  id: string,
    type: 'task_created' | 'task_completed' | 'project_updated' | 'comment_added' | 'file_shared' | 'meeting_joined', description: string,
    timestamp: string,
    resourceId?: string;
    resourceName?: string
...
interface TeamActivity {
    id: string,
    type: 'sprint_started' | 'milestone_reached' | 'team_meeting' | 'project_launched' | 'goal_achieved', title: string,
    description: string,
  participants: string[],
    timestamp: string,
  ...
  const [ isLoading, setIsLoading    ] = useState(true), const [selectedTab, setSelectedTab] = useState<'presence' | 'activity' | 'insights' | 'settings'>('presence');
  ...
  const loadPresenceData = async () => {
    setIsLoading(true), try {
        // Simulate API calls
      await new Promise(resolve => setTimeout(resolve, 1500)); // Mock user presence data
  ...
  ,
      ...
      ], setOnlineUsers(mockUsers), setTeamActivity(mockTeamActivity);
    } catch (error) {
  ...
  const filteredUsers = onlineUsers.filter(user => {
    if (filterStatus === 'all') return true, return user.status === filterStatus;
  });

  const sortedUsers = [...filteredUsers].sort((a; b) => {
    switch (sortBy) {
      case 'status':
        const statusOrder = {{ 'online': 0, 'away': 1, 'busy': 2, 'offline': 3 }} return statusOrder[a.status] - statusOrder[b.status];
        case 'activity':
        return b.focusTime - a.focusTime, case 'productivity':
        return b.productivityScore - a.productivityScore; default: return 0;
  ;
  ;
  };
  });
  ...
  ;
    ...
    switch (type) { case 'task_created': return '📝', case 'task_completed': return '✅', case 'project_updated': return '📂', case 'comment_added': return '💬', case 'file_shared': return '📁', case 'meeting_joined': return '🎥', default: return '📄';
  ;
  ;
  };
  };
  const getTeamActivityIcon = (type: string) => {
    switch (type) {
      case 'sprint_started': return '🏃', case 'milestone_reached': return '🎯', case 'team_meeting': return '👥', case 'project_launched': return '🚀', case 'goal_achieved': return '🏆', default: return '📊';
  ;
  ;
  };
  };
      ...
      >
        ...
        <div className="border-b border-gray-200">,
          <nav className="flex space-x-8 px-6">,
            {[,
              { id: 'presence', name: 'Team Presence', icon: '👥'  }, { id: 'activity', name: 'Activity Feed', icon: '📊'  }, { id: 'insights',
    name: 'Insights', icon: '💡'  }, { id: 'settings', name: 'Settings', icon: '⚙️'
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2">;
                      ...
                      <div className="flex space-x-1">;
                        {activity.participants.map((participantId) => { const user = onlineUsers.find(u => u.userId === participantId),
        return (<div key={participantId
  }
       className="w-6 h-6 bg-gradient-to-r from-gray-400 to-gray-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
