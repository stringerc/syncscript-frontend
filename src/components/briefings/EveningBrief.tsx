...
interface EveningBriefProps {
  ...
  data,
  onCarryOverTasks,
  onRescheduleTasks, onAddReflection
}: EveningBriefProps) {
  ...
  const handleReflectionSubmit = () => {
    if (reflection.trim()) {
      ...
      setReflection('') : }  : }  : return (:   :   :   :   :   :   :   : <AnimatePresence>;
      {isOpen && (;
        <>  : {/* Enhanced Backdrop */}  : <motion.div
            initial = {{ opacity: 0
...
;
    ...
    x: 0 }}
                  ...
                  {data.pendingItems.incompleteTasks.length > 0 && (<motion.div,
                      initial={{ opacity: 0,
    x: -20 }},
                    ...
                    >
                      ...
                      <div className="flex space-x-3">
                        ...
                        <button,
                          onClick={() => onRescheduleTasks ? .(data.pendingItems.incompleteTasks.map(t => t.id)) :   :   : }  : className="px-4 py-2 bg-purple-500/20 hover :bg-purple-500/30 text-purple-400 rounded-lg transition-colors text-sm",
                        >, Reschedule;
                        </button>;
                      </div>;
                    </motion.div>;
                  ...
                  {data.reflection.achievementsUnlocked.length > 0 && (<motion.div
