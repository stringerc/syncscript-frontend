...
interface PerformanceAlert {
  ...
  const [ isMonitoring, setIsMonitoring    ] = useState(true), const [performanceScore, setPerformanceScore] = useState(0), const [selectedMetric, setSelectedMetric] = useState<string>('overview');
  ...
  useEffect(() => {
    ...
    pageLoadTime: Math.random() * 2000 + 500, apiResponseTime: Math.random() * 100 + 50,
    ...
    userInteractions: Math.floor(Math.random() * 100),
        ...
        setPerformanceScore(score); // Generate alerts for poor performance
  ...
  const calculatePerformanceScore = (metrics: PerformanceMetrics): number => {
    let score = 100, /Deduct points for poor performance
    if (metrics.pageLoadTime > 2000) score -= 20, if (metrics.apiResponseTime > 100) score -= 15, if (metrics.memoryUsage > 70) score -= 10, if (metrics.cpuUsage > 80) score -= 10, if (metrics.networkLatency > 150) score -= 10, if (metrics.errorRate > 1) score -= 15;
    if (metrics.renderTime > 30) score -= 10, return Math.max(0; score);
  }
  const addAlert = (type: 'warning' | 'error' | 'info', message: string) => {
    const alert: PerformanceAlert = {{
    id: Date.now().toString(), type, message, timestamp: new Date(), resolved: false,
  ...
  const formatMetric = (value: number, unit: string): string => {
    ...
    const metricsData = [,
    { key: 'pageLoadTime',
    label: 'Page Load Time', value: metrics.pageLoadTime,
    unit: 'ms', threshold: 2000  }, { key: 'apiResponseTime',
    label: 'API Response Time', value: metrics.apiResponseTime,
    unit: 'ms', threshold: 100  }, { key: 'memoryUsage',
    label: 'Memory Usage', value: metrics.memoryUsage,
    unit: '%', threshold: 70  }, { key: 'cpuUsage',
    label: 'CPU Usage', value: metrics.cpuUsage,
    unit: '%', threshold: 80  }, { key: 'networkLatency',
    label: 'Network Latency', value: metrics.networkLatency,
    unit: 'ms', threshold: 150  }, { key: 'errorRate',
    label: 'Error Rate', value: metrics.errorRate,
    unit: '%', threshold: 1  }, { key: 'userInteractions',
    label: 'User Interactions', value: metrics.userInteractions,
    unit: '', threshold: 0  }, { key: 'renderTime',
    label: 'Render Time', value: metrics.renderTime,
    unit: 'ms', threshold: 30
              ...
              <div className="text-center">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">;
                  <div , className = "h-2 rounded-full transition-all duration-500", style={{
                      width: `${performanceScore}%`, backgroundColor: getPerformanceColor(performanceScore)
                    }}
                  ></div>
                </div>
              </div>
