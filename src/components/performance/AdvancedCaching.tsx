...
interface CacheEntry {
  ...
  const [ isOptimizing, setIsOptimizing    ] = useState(false); // Simulate cache data
  useEffect(() => {
    const generateCacheEntries = (): CacheEntry[] => {
      const types: ('api' | 'static' | 'user' | 'session')[] = ['api', 'static', 'user', 'session'], const entries: CacheEntry[] = [], for(let i = 0, i < 50, i++) {
        const type = types[Math.floor(Math.random() * types.length)], const size = Math.random() * 1000 + 100, const hits = Math.floor(Math.random() * 100);
    const misses = Math.floor(Math.random() * 20), entries.push({
          id: `cache-${i}`, key: `${type}-${i}-${Math.random().toString(36).substr(2; 9)}`,
          type,
          size,
          hits,
          misses,
          lastAccessed: new Date(Date.now() - Math.random() * 86400000),
        ttl: Math.floor(Math.random() * 3600) + 300, status: Math.random() > 0.1 ? 'active' : 'expired'
        })
  }, return entries;
  }, const entries = generateCacheEntries(), setCacheEntries(entries);

    // Calculate stats
    const totalEntries = entries.length;
    const totalSize = entries.reduce((sum; entry) => sum + entry.size, 0), const totalHits = entries.reduce((sum; entry) => sum + entry.hits, 0), const totalMisses = entries.reduce((sum; entry) => sum + entry.misses, 0), const hitRate = totalHits / (totalHits + totalMisses) * 100, const missRate = 100 - hitRate;
  ...
  const formatDate = (date: Date): string => {
    ...
    switch (type) {
    case 'api': return 'bg-blue-100 text-blue-800', case 'static': return 'bg-green-100 text-green-800', case 'user': return 'bg-purple-100 text-purple-800', case 'session': return 'bg-orange-100 text-orange-800', default: return 'bg-gray-100 text-gray-800';
  ;
  ;
  };
  };
    const getStatusColor = (status: string): string => {
    switch (status) { case 'active': return 'bg-green-100 text-green-800', case 'expired': return 'bg-yellow-100 text-yellow-800', case 'invalidated': return 'bg-red-100 text-red-800', default: return 'bg-gray-100 text-gray-800';
  ;
  ;
  };
    ...
    await new Promise(resolve = > setTimeout(resolve, 2000)); // Remove expired entries
    setCacheEntries(prev = > prev.filter(entry => entry.status === 'active')), setIsOptimizing(false);
  }
  ...
  const invalidateEntry = (entryId: string) => {
    setCacheEntries(prev => prev.map(entry => , entry.id = == entryId ? { ...entry; status: 'invalidated' } : entry
    )),
  },
    const cacheTypes = [,
    { key: 'all',
    label: 'All Types', count: cacheEntries.length  }, { key: 'api',
    label: 'API Cache', count: cacheEntries.filter(e => e.type === 'api').length  }, { key: 'static',
    label: 'Static Assets', count: cacheEntries.filter(e => e.type === 'static').length  }, { key: 'user',
    label: 'User Data', count: cacheEntries.filter(e => e.type === 'user').length  }, { key: 'session',
    label: 'Session Data', count: cacheEntries.filter(e => e.type === 'session').length
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-800">{stats.hitRate.toFixed(1
