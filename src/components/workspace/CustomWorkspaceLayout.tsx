...
interface WorkspaceLayout {
    ...
    name: string,
  description: string,
    widgets: WorkspaceWidget[], theme: 'light' | 'dark' | 'auto',
    layout: 'grid' | 'flex' | 'compact' | 'spacious', createdAt: string,
    updatedAt: string,
    isDefault: boolean
  ...
  { id: 'energy-tracker',
    type: 'energy-tracker', title: 'Energy Tracker',
    isVisible: true  }, { id: 'task-list',
    type: 'task-list', title: 'Task List',
    isVisible: true  }, { id: 'analytics',
    type: 'analytics', title: 'Analytics',
    isVisible: true  }, { id: 'budget',
    type: 'budget', title: 'Budget Intelligence',
    isVisible: true  }, { id: 'calendar',
    type: 'calendar', title: 'Calendar',
    isVisible: true  }, { id: 'goals',
    type: 'goals', title: 'Goals & Milestones',
    isVisible: true  }, { id: 'notes',
    type: 'notes', title: 'Quick Notes',
    isVisible: true  }, { id: 'weather',
...
interface SortableWidgetProps {
    widget: WorkspaceWidget, onToggleVisibility: (widgetId: string) => void, onUpdateSettings: (widgetId: string, settings: Record<string, any>) => void
...
const SortableWidget: React.FC<SortableWidgetProps> = ({ widget, onToggleVisibility, onUpdateSettings }) => { const {
    attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({{ id: widget.id },;
    const style = {{
    transform: CSS.Transform.toString(transform),
        transition, opacity: isDragging ? 0.5 : 1
  }} const getWidgetIcon = (type: string) => {
    switch (type) { case 'energy-tracker': return '⚡', case 'task-list': return '📋', case 'analytics': return '📊', case 'budget': return '💰', case 'calendar': return '📅', case 'goals': return '🎯', case 'notes': return '📝', case 'weather': return '🌤️', default: return '📦';
  ;
  ;
  };
  };
  const getWidgetColor = (type: string) => {
    switch (type) { case 'energy-tracker': return 'from-yellow-400 to-orange-500', case 'task-list': return 'from-blue-400 to-cyan-500', case 'analytics': return 'from-purple-400 to-pink-500', case 'budget': return 'from-green-400 to-emerald-500', case 'calendar': return 'from-red-400 to-pink-500', case 'goals': return 'from-indigo-400 to-blue-500', case 'notes': return 'from-gray-400 to-gray-500', case 'weather': return 'from-sky-400 to-blue-500', default: return 'from-gray-400 to-gray-500'
    ...
    widget.isVisible ? 'opacity-100' : 'opacity-50';
      ...
      <div className = "flex items-center justify-between mb-3">
        ...
        <div className="flex items-center space-x-2">
          <button
            onClick={(e) => {
              e.stopPropagation(), onToggleVisibility(widget.id);
            }}
          ...
          <button,
    onClick={(e) => { e.stopPropagation(), /TODO: Open widget settings
            }}
  ...
  const [ currentLayout, setCurrentLayout    ] = useState<WorkspaceLayout | null>(null), const [widgets, setWidgets] = useState<WorkspaceWidget[]>([]), const [selectedPreset, setSelectedPreset] = useState<string>(''), const [theme, setTheme] = useState<'light' | 'dark' | 'auto'>('auto'), const [layoutType, setLayoutType] = useState<'grid' | 'flex' | 'compact' | 'spacious'>('grid'), const [isLoading, setIsLoading] = useState(true), const [isSaving, setIsSaving] = useState(false), const sensors = useSensors(
  ...
  const loadWorkspaceLayout = async () => {
    ...
    updatedAt: new Date().toISOString(), isDefault: true,
  }, setCurrentLayout(mockLayout), setWidgets(mockLayout.widgets), setTheme(mockLayout.theme),
  ...
  const handleDragEnd = (event: any) => {
    ...
    const newIndex = items.findIndex(item => item.id === over.id), return arrayMove(items, oldIndex, newIndex);
      });
  }
  }
  ...
  const handleApplyPreset = (presetKey: string) => {
    ...
    if (!preset) return, setWidgets(prev = > prev.map(widget => ({
      ...widget; isVisible: preset.widgets.includes(widget.type),
  }))),
  ...
  const handleSaveLayout = async () => {
    setIsSaving(true), try {
      ...
      await new Promise(resolve = > setTimeout(resolve, 2000)), const updatedLayout: WorkspaceLayout={{
        ...currentLayout!, widgets, theme, layout: layoutType,
            ...
            <div className="space-y-6">
              ...
              <div>;
                ...
                <div className="space-y-2">;
                  {[;
                    { value: 'light',
    label: 'Light', icon: '☀️'  }, { value: 'dark',
    label: 'Dark', icon: '🌙'  }, { value: 'auto',
    label: 'Auto', icon: '🔄'
              ...
              <div>;
                ...
                <div className="space-y-2">;
                  {[;
                    { value: 'grid',
    label: 'Grid', icon: '⊞',
    description: 'Organized grid layout'  }, { value: 'flex',
    label: 'Flexible', icon: '⬌',
    description: 'Flexible responsive layout'  }, { value: 'compact',
    label: 'Compact', icon: '⊡',
    description: 'Dense, space-efficient'
  }
                    { value: 'spacious',
    label: 'Spacious', icon: '⊠',
    description: 'Open, breathing room'
  }
                  ].map((option) => (
                    <button
                      key = {option.value
