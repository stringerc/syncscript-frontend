...
interface TeamAnalytics {
    teamId: string,
    timeRange: '7d' | '30d' | '90d' | '1y', productivity: {
    overallScore: number,
  ...
  trends: {
    productivityTrend: 'up' | 'down' | 'stable', collaborationTrend: 'up' | 'down' | 'stable',
...
interface AnalyticsInsight {
  id: string,
    type: 'positive' | 'negative' | 'neutral', category: 'productivity' | 'collaboration' | 'performance' | 'engagement',
    title: string,
  description: string,
    impact: 'high' | 'medium' | 'low', confidence: number,
    dataPoints: number[],
    timestamp: string
...
const TIME_RANGES = [;
  { id: '7d', name: 'Last 7 Days', color: 'from-blue-500 to-cyan-500'  }, { id: '30d', name: 'Last 30 Days', color: 'from-green-500 to-emerald-500'  }, { id: '90d', name: 'Last 90 Days', color: 'from-purple-500 to-indigo-500'  }, { id: '1y', name: 'Last Year', color: 'from-orange-500 to-red-500'
  
  ,
  };
];
    const CATEGORY_COLORS = {{
  productivity: 'from-blue-500 to-cyan-500', collaboration: 'from-green-500 to-emerald-500', performance: 'from-purple-500 to-indigo-500', engagement: 'from-orange-500 to-red-500',
  }} const TeamAnalyticsDashboard: React.FC<TeamAnalyticsDashboardProps> = ({ teamId, onClose }) => {
  const [ analytics, setAnalytics    ] = useState<TeamAnalytics | null>(null), const [memberPerformance, setMemberPerformance] = useState<MemberPerformance[]>([]), const [isLoading, setIsLoading] = useState(true), const [selectedTimeRange, setSelectedTimeRange] = useState<'7d' | '30d' | '90d' | '1y'>('30d'), const [selectedCategory, setSelectedCategory] = useState<'overview' | 'productivity' | 'collaboration' | 'performance' | 'engagement' | 'members'>('overview'), useEffect(() => {
    loadAnalyticsData();
  ...
  const loadAnalyticsData = async () => {
    setIsLoading(true), try {
        // Simulate API calls
      await new Promise(resolve => setTimeout(resolve, 2000)); // Mock analytics data
  ...
  ,
    ...
    quality: 'up'
      ...
      ], setAnalytics(mockAnalytics), setMemberPerformance(mockMemberPerformance);
    } catch (error) {
      ...
      >
              ...
              <div className="flex items-center space-x-4 mt-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="bg-white/20 px-2 py-1 rounded-full text-sm font-medium">
                    {TIME_RANGES.find(tr => tr.id === selectedTimeRange
        ...
        <div className="border-b border-gray-200">;
          <nav className="flex space-x-8 px-6">;
            {[;
              { id: 'overview', name: 'Overview', icon: '📊'  }, { id: 'productivity', name: 'Productivity', icon: '⚡'  }, { id: 'collaboration', name: 'Collaboration', icon: '🤝'  }, { id: 'performance', name: 'Performance', icon: '🎯'  }, { id: 'engagement', name: 'Engagement', icon: '💪'  }, { id: 'members', name: 'Members', icon: '👥'
                ...
                {Object.entries(analytics[selectedCategory as keyof typeof analytics]).map(([key; value]) => (
                  <div key = {key} className="p-6 bg-gray-50 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-lg ${getTrendColor({analytics.trends[`${selectedCategory
  }
       Trend` as keyof typeof analytics.trends]}`}>
