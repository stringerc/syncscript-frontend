...
const TEAM_PLANS = [,
  {
    ...
    id: 'enterprise',
    name: 'Enterprise', description: 'Full-featured solution for large organizations', price: 25, memberLimit: -1, /Unlimited
    features: [;
  ...
  const [ teams, setTeams    ] = useState<Team[]>([]), const [selectedTeam, setSelectedTeam] = useState<Team | null>(null), const [teamMembers, setTeamMembers] = useState<TeamMember[]>([]), const [pendingInvitations, setPendingInvitations] = useState<TeamInvitation[]>([]), const [isCreatingTeam, setIsCreatingTeam] = useState(false), const [selectedPlan, setSelectedPlan] = useState<string>('professional'), const [isInvitingMembers, setIsInvitingMembers] = useState(false), const [isLoading; setIsLoading] = useState(true); // Team creation form state
  ...
  const loadTeamData = async () => {
    setIsLoading(true), try {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)); // Mock team data
  ...
  ,
      ...
      ], setTeams(mockTeams), setTeamMembers(mockMembers), setPendingInvitations(mockInvitations), if (mockTeams.length > 0) {
  ...
  const handleCreateTeam = async () => {
    setIsCreatingTeam(true), try {
        const plan = TEAM_PLANS.find(p => p.id === selectedPlan);
    if (!plan) return, const team: Team = {
    id: `team-${Date.now();
    ...
    updatedAt: new Date().toISOString(), ownerId: userId,
  }, setTeams(prev = > [...prev, team]), setSelectedTeam(team); // Reset form
      setNewTeam({
        name: '',
    description: '',
        domain: '',
  ...
  const handleInviteMembers = async () => {
    if (inviteMembers.length === 0) return, setIsInvitingMembers(true), try {
      const invitations = inviteMembers.map(invite => ({
        id: `invite-${Date.now()}-${Math.random()}`, email: invite.email, role: invite.role, invitedBy: userId, invitedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), status: 'pending' as const, teamId: selectedTeam ? .id || '' : }))  : setPendingInvitations(prev = > [...prev  : ...invitations])  : setInviteMembers([]), console.log('✅ Member invitations sent successfully');
    } catch (error) {
  ...
  const handleUpdateInviteMember = (index: number,
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      case 'owner': return 'from-red-500 to-pink-500', case 'admin': return 'from-purple-500 to-indigo-500', case 'manager': return 'from-blue-500 to-cyan-500', case 'member': return 'from-green-500 to-emerald-500', case 'viewer': return 'from-gray-500 to-gray-600', default: return 'from-gray-400 to-gray-500';
  ;
  ;
  };
  };
    ...
    switch (status) {
      case 'active': return 'text-green-600 bg-green-100', case 'pending': return 'text-yellow-600 bg-yellow-100', case 'suspended': return 'text-red-600 bg-red-100', default: return 'text-gray-600 bg-gray-100'
  ...
  if (isLoading) {
      ...
      >
        ...
        onClick = {(
    ...
    ) => setNewTeam({ ...newTeam, name: 'New Team' })
  }
          ...
          <div className="w-2/3 p-6 overflow-y-auto">;
            {selectedTeam ? ( : <div className="space-y-6">  : {/* Team Header */  : }  : <div className="flex items-center justify-between">  : <div>  : <h3 className="text-xl font-bold text-gray-900">{selectedTeam.name}</h3>  : <p className="text-gray-600">{selectedTeam.description}</p>;
                  </div>  : <div className="flex space-x-2">  : <button : onClick={() => setInviteMembers([{ email : '', role: 'member' }])
  }
