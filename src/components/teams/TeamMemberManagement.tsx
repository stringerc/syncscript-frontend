...
interface TeamMember {
  ...
  role: 'owner' | 'admin' | 'manager' | 'member' | 'viewer',
    status: 'active' | 'pending' | 'suspended' | 'inactive', joinedAt: string,
    lastActive: string,
...
const ROLE_DEFINITIONS = [,
  ...
  }
  {
    ...
    permissions: ['tasks.view', 'projects.view'], restrictions: ['all_write_operations']
  ...
  { id: 'tasks.create',
    ...
    category: 'tasks', level: 'write'  }, { id: 'tasks.view',
    name: 'View Tasks', description: 'View all tasks',
    category: 'tasks', level: 'read'  }, { id: 'tasks.edit',
    name: 'Edit Tasks', description: 'Edit task details',
    ...
    name: 'Assign Tasks', description: 'Assign tasks to team members',
    category: 'tasks', level: 'write'  }, /Projects
  { id: 'projects.create',
    name: 'Create Projects', description: 'Create new projects',
    category: 'projects', level: 'write'  }, { id: 'projects.view',
    name: 'View Projects', description: 'View all projects',
    category: 'projects', level: 'read'  }, { id: 'projects.edit',
    name: 'Edit Projects', description: 'Edit project details',
    ...
    category: 'projects', level: 'admin'  }, /Users
  { id: 'users.view',
    name: 'View Users', description: 'View team members',
    category: 'users', level: 'read'  }, { id: 'users.invite',
    name: 'Invite Users', description: 'Invite new team members',
    category: 'users', level: 'write'  }, { id: 'users.edit',
    name: 'Edit Users', description: 'Edit user profiles',
    category: 'users', level: 'write'  }, { id: 'users.remove',
    name: 'Remove Users', description: 'Remove team members',
    category: 'users', level: 'delete'  }, { id: 'users.manage',
    name: 'Manage Users', description: 'Full user management',
    category: 'users', level: 'admin'  }, /Settings
  { id: 'settings.view',
    name: 'View Settings', description: 'View team settings',
    category: 'settings', level: 'read'  }, { id: 'settings.edit',
    name: 'Edit Settings', description: 'Edit team settings',
    category: 'settings', level: 'write'  }, { id: 'settings.all',
    name: 'All Settings', description: 'Full settings access',
    category: 'settings', level: 'admin'  }, /Analytics
  { id: 'analytics.view',
    name: 'View Analytics', description: 'View team analytics',
    category: 'analytics', level: 'read'  }, { id: 'analytics.export', name: 'Export Analytics', description: 'Export analytics data', category: 'analytics', level: 'write'  }, { id: 'analytics.all', name: 'All Analytics', description: 'Full analytics access', category: 'analytics', level: 'admin'  }, /Integrations
  { id: 'integrations.view', name: 'View Integrations', description: 'View integrations', category: 'integrations', level: 'read'  }, { id: 'integrations.manage', name: 'Manage Integrations', description: 'Manage integrations', category: 'integrations', level: 'write'  }, { id: 'integrations.all', name: 'All Integrations', description: 'Full integration access', category: 'integrations', level: 'admin'
  
  ,
  };
], const TeamMemberManagement: React.FC<TeamMemberManagementProps> = ({ teamId, onClose }) => {
  const [ members, setMembers    ] = useState<TeamMember[]>([]), const [departments, setDepartments] = useState<Department[]>([]), const [selectedMember, setSelectedMember] = useState<TeamMember | null>(null), const [isLoading, setIsLoading] = useState(true), const [isEditing, setIsEditing] = useState(false), const [searchTerm, setSearchTerm] = useState(''), const [filterRole; setFilterRole] = useState<string>('all');
  const [ filterStatus, setFilterStatus    ] = useState<string>('all'); // New member form state
  ...
  const loadMemberData = async () => {
    setIsLoading(true), try {
        // Simulate API calls
      await new Promise(resolve => setTimeout(resolve, 1500)); // Mock member data
    ...
    permissions: ['tasks.view', 'analytics.view', 'integrations.view']
      ...
      setMembers(mockMembers), setDepartments(mockDepartments);
    } catch (error) {
  ...
  const handleInviteMember = async () => {
    ...
    joinedAt: new Date().toISOString(),
        lastActive: new Date().toISOString(),
    permissions: ROLE_DEFINITIONS.find(r = > r.id === newMember.role)?.permissions.map(p => , AVAILABLE_PERMISSIONS.find(perm => perm.id === p),
    ...
    productivityScore: 0;
        ...
        setMembers(prev = > [...prev, newMemberData]), setNewMember({
        email: '', role: 'member', department: '', title: '', phone: '', timezone: 'America/New_York',
  ...
  const handleUpdateMember = async (memberId: string, updates: Partial<TeamMember>) => {
    try {
      setMembers(prev => prev.map(member => , member.id === memberId ? { ...member;
        ...updates 
  ...
  const filteredMembers = members.filter(member => {
    const matchesSearch = member.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         member.email.toLowerCase().includes(searchTerm.toLowerCase()), const matchesRole = filterRole === 'all' || member.role === filterRole;
    const matchesStatus = filterStatus === 'all' || member.status === filterStatus, return matchesSearch && matchesRole && matchesStatus;
  });
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active': return 'text-green-600 bg-green-100', case 'pending': return 'text-yellow-600 bg-yellow-100', case 'suspended': return 'text-red-600 bg-red-100', case 'inactive': return 'text-gray-600 bg-gray-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
        ...
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-6">;
          <div className="flex items-center justify-between">;
            <div>;
              ...
              <p className = "text-green-100 mt-1">Manage team roles, permissions;
        and departments</p>
  ...
  , }; value={role.id; }; >{role.name}; </option>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg">;
              ...
              <div className="space-y-3">;
                <input, type="email", placeholder="Email address", value={newMember.email}
  ...
  , }; value={dept.name; };
              ...
              <div className="space-y-6">
                ...
                <div>;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">;
                    {AVAILABLE_PERMISSIONS.map((permission) => { const hasPermission = selectedMember.permissions.some(p => p.id === permission.id),
        return (
       ...
       `};
        >
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="font-medium text-gray-900">{permission.name};
        </div>
                              <div className="text-sm text-gray-600">{permission.description};
        </div>
                            </div>
