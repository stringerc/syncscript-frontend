...
interface Project {
    ...
    name: string,
  description: string,
    status: 'planning' | 'active' | 'on_hold' | 'completed' | 'cancelled', priority: 'low' | 'medium' | 'high' | 'urgent',
    progress: number,
  startDate: string,
    dueDate?: string,
  ...
  members: ProjectMember[],
    tags: string[], attachments: ProjectAttachment[],
    milestones: Milestone[], tasks: ProjectTask[],
...
interface ProjectTask {
    ...
    title: string,
  description: string,
    status: 'todo' | 'in_progress' | 'review' | 'completed', priority: 'low' | 'medium' | 'high' | 'urgent',
    assigneeId?: string,
  assigneeName?: string,
  dueDate?: string,
  ...
  actualHours?: number,
  progress: number,
    subtasks: ProjectTask[], dependencies: string[],
    tags: string[], comments: TaskComment[],
    createdAt: string,
    updatedAt: string
...
const PROJECT_STATUSES = [,
  { id: 'planning', name: 'Planning', color: 'from-blue-500 to-cyan-500', icon: '📋'  }, { id: 'active', name: 'Active', color: 'from-green-500 to-emerald-500', icon: '🚀'  }, { id: 'on_hold', name: 'On Hold', color: 'from-yellow-500 to-orange-500', icon: '⏸️'  }, { id: 'completed', name: 'Completed', color: 'from-purple-500 to-indigo-500', icon: '✅'  }, { id: 'cancelled', name: 'Cancelled', color: 'from-red-500 to-pink-500', icon: '❌'
  
  ,
  };
];
    const PRIORITY_LEVELS = [
  { id: 'low', name: 'Low', color: 'from-gray-400 to-gray-500', icon: '🟢'  }, { id: 'medium', name: 'Medium', color: 'from-blue-400 to-blue-500', icon: '🟡'  }, { id: 'high', name: 'High', color: 'from-orange-400 to-orange-500', icon: '🟠'  }, { id: 'urgent', name: 'Urgent', color: 'from-red-400 to-red-500', icon: '🔴'
  
  ,
  };
], const CollaborativeProjectManagement: React.FC<CollaborativeProjectManagementProps> = ({ teamId, onClose }) => {
  const [ projects, setProjects    ] = useState<Project[]>([]), const [selectedProject, setSelectedProject] = useState<Project | null>(null), const [isLoading, setIsLoading] = useState(true), const [viewMode, setViewMode] = useState<'grid' | 'list' | 'kanban' | 'timeline'>('grid'), const [filterStatus, setFilterStatus] = useState<string>('all'), const [filterPriority, setFilterPriority] = useState<string>('all');
  const [ searchTerm, setSearchTerm    ] = useState('');
  const [ isCreatingProject, setIsCreatingProject    ] = useState(false), const [isAddingTask, setIsAddingTask] = useState(false); // New project form
  ...
  const loadProjects = async () => {
    setIsLoading(true), try {
        // Simulate API calls
      await new Promise(resolve => setTimeout(resolve, 1500)); // Mock project data
  ...
  const handleCreateProject = async () => {
    ...
    attachments: [],
        milestones: [],
    tasks: [], comments: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
  }, setProjects(prev = > [...prev, project]), setSelectedProject(project); // Reset form
      setNewProject({
        name: '',
    description: '',
        priority: 'medium',
  ...
  const handleAddTask = async () => {
    ...
    tags: newTask.tags,
        ...
        project.id === selectedProject.id ? { ...project: tasks : [...project.tasks, task]
  }
          : project
  ...
  const handleUpdateTaskStatus = (projectId: string,
    taskId: string, status: ProjectTask['status']) => {
    setProjects(prev => prev.map(project => ,
      project.id === projectId;
        ? {
            ...project, tasks: project.tasks.map(task = >, task.id = == taskId ? { ...task: status, updatedAt: new Date().toISOString() } : task
            )
  }
        : project
    )),
    ...
    const matchesSearch = project.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         project.description.toLowerCase().includes(searchTerm.toLowerCase()), const matchesStatus = filterStatus === 'all' || project.status === filterStatus;
    const matchesPriority = filterPriority === 'all' || project.priority === filterPriority, return matchesSearch && matchesStatus && matchesPriority;
  });
  ...
  const getStatusConfig = (status: string) => {
    ...
    switch (status) {
      case 'todo': return 'bg-gray-100 text-gray-800', case 'in_progress': return 'bg-blue-100 text-blue-800', case 'review': return 'bg-yellow-100 text-yellow-800', case 'completed': return 'bg-green-100 text-green-800', default: return 'bg-gray-100 text-gray-800'
