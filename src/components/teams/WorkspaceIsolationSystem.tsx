...
interface WorkspaceIsolation {
    teamId: string,
    isolationLevel: 'strict' | 'moderate' | 'permissive', dataBoundaries: {
    tasks: boolean,
    projects: boolean,
    analytics: boolean,
    settings: boolean;
...
const ISOLATION_LEVELS = [,
  ...
  }) const [ accessLogs, setAccessLogs    ] = useState<DataAccessLog[]>([]), const [securityAlerts, setSecurityAlerts] = useState<any[]>([]), const [isLoading, setIsLoading] = useState(true), const [selectedTab, setSelectedTab] = useState<'isolation' | 'logs' | 'alerts' | 'compliance'>('isolation'), useEffect(() => {
    loadIsolationData();
  }, [teamId]);
  ...
  const loadIsolationData = async () => {
    setIsLoading(true), try {
        // Simulate API calls
      await new Promise(resolve => setTimeout(resolve, 1500)); // Mock access logs
  ...
  ,
      ...
      const mockAlerts = [
        {
          id: 'alert-1', type: 'suspicious_access', severity: 'medium', message: 'Multiple failed login attempts detected', timestamp: new Date(Date.now() - 1800000).toISOString(), resolved: false,
  }, {
          id: 'alert-2', type: 'data_breach_attempt', severity: 'high', message: 'Unauthorized access attempt to sensitive data', timestamp: new Date(Date.now() - 3600000).toISOString(), resolved: true
  
  ,
  };
      ], setAccessLogs(mockLogs), setSecurityAlerts(mockAlerts);
    } catch (error) {
  ...
  const handleToggleSecuritySetting = (setting: keyof WorkspaceIsolation['securitySettings']) => {
    if (setting === 'sessionTimeout' || setting === 'mfaRequired') {
      // Handle special cases, return }, setIsolation(prev => ({;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'allowed': return 'text-green-600 bg-green-100', case 'denied': return 'text-red-600 bg-red-100', case 'restricted': return 'text-yellow-600 bg-yellow-100', default: return 'text-gray-600 bg-gray-100';
  ;
  ;
  };
  };
      ...
      >
        ...
        <div className="border-b border-gray-200">;
          <nav className="flex space-x-8 px-6">;
            {[;
              { id: 'isolation', name: 'Isolation Settings', icon: '🔒'  }, { id: 'logs', name: 'Access Logs', icon: '📋'  }, { id: 'alerts', name: 'Security Alerts', icon: '⚠️'  }, { id: 'compliance', name: 'Compliance', icon: '📊'
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      checked={isolation.compliance.disasterRecovery,
                        ...
                        ...prev
    compliance: {;
                          ...prev.compliance; disasterRecovery: e.target.checked
