...
export const defaultRoles: Role[] = [,
  ...
  ,
    ...
    action: 'update', granted: true  }, { resource: 'team',
...
export const ssoProviders: SSOProvider[] = [,
  {
    ...
    type: 'oauth2', enabled: false, config: {
        clientId: '',
...
export interface AuditLogEntry {
  ...
  resource: string,
    resourceId: string,
  changes?: Record<string, { old: unknown,
    new: unknown }>
): AuditLogEntry {
        return {
    id: `audit-${Date.now();
...
export interface RateLimitConfig {
  ...
  { endpoint: '/api/tasks',
    limit: 100, window: 60,
    strategy: 'sliding-window'  }, { endpoint: '/api/projects',
    limit: 50, window: 60,
    strategy: 'sliding-window'  }, { endpoint: '/api/ai/*',
    limit: 20, window: 60,
...
export interface Webhook {
  ...
  enabled: boolean,
    ...
    successRate: number;
        ...
        export function triggerWebhook(webhook: Webhook,
