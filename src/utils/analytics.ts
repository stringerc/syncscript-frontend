 ...
 */,
,
import { PostHog } from 'posthog-js', interface AnalyticsEvent {
    event: string,
...
class SyncScriptAnalytics {
  private posthog: PostHog | null = null, private isInitialized = false, private eventQueue: AnalyticsEvent[] = [], private userProperties: UserProperties | null = null;
  // **
  ...
  async initialize(apiKey: string, userId?: string): Promise<void> {
    try {
      ...
      const { PostHog} = await import('posthog-js'), this.posthog = PostHog.init(apiKey; {
        api_host: 'https:/app.posthog.com',
    person_profiles: 'identified_only',
    capture_pageview: false; // We'll handle this manually
        capture_pageleave: true, loaded: (posthog) => {
    console.log('✅ PostHog analytics initialized'),
    this.isInitialized = true, this.processEventQueue();
  }
      });
  ...
  identifyUser(userId: string,
    ...
    if (!this.isInitialized || !this.posthog) {
      ...
      this.eventQueue.push({
        event: '$identify', properties: { userId;
        ...properties
    ...
    this.userProperties = {{ userId, ...properties }} this.posthog.identify(userId; properties); // Track user identification
    this.track('User Identified', {
      userId; hasProperties: !!properties, timestamp: new Date().toISOString()
    })
  },
  // **,
   * Track custom events,
  ...
  track(eventName: string,
    ...
    event: eventName,
      properties: {
        ...properties, timestamp: new Date().toISOString(), url: typeof window !== 'undefined' ? window.location.href: undefined,
    userAgent: typeof window !== 'undefined' ? window.navigator.userAgent : undefined;
      ...
      console.warn('PostHog not initialized; queuing event: ', eventName), this.eventQueue.push(event), return;
  }
  ...
  trackFeatureUsage(featureName: string,
    action: string, properties?: Record<string, any>): void {
   ...
   */
  trackEnergyEvent(eventType: 'energy_logged' | 'energy_recalibrated' | 'energy_matched',
    properties?: Record<string, any>): void {
   ...
   */
  trackTaskEvent(eventType: 'task_created' | 'task_completed' | 'task_updated' | 'task_deleted',
    properties?: Record<string, any>): void {
   ...
   */
  trackBriefingEvent(eventType: 'morning_brief_viewed' | 'evening_brief_viewed' | 'briefing_settings_updated',
    properties?: Record<string, any>): void {
   ...
   */
  trackAIEvent(eventType: 'explanation_generated' | 'explanation_viewed' | 'explanation_helpful',
    properties?: Record<string, any>): void {
    this.track('AI Event'; {
      eventType; ...properties
    });
  ...
  setUserProperties(properties: UserProperties): void {
    ...
    this.userProperties={{ ...this.userProperties, ...properties }} this.posthog.people.set(properties);
  }
  // **
  ...
  async getFeatureFlag(flagKey: string): Promise<FeatureFlag | null> {
    ...
    try {
        const value = this.posthog.getFeatureFlag(flagKey), return {
        key: flagKey, value: value as boolean | string,
  ...
  private processEventQueue(): void {
    if (this.eventQueue.length = == 0) return, console.log({`Processing ${this.eventQueue.length}: queued events`: this.eventQueue.forEach(event = > {
  ...
  trackError(error: Error,
    ...
    errorStack: error.stack, errorName: error.name; ...context
    });
  }
  // **
   * Track user session
