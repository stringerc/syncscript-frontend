...
,
  ...
  calculateBudgetFit,
  getComfortBand,
  getStarDisplay, BUDGET_CATEGORIES 
...
export function calculateRecommendationBudgetFit(
  estimatedCost: number,
    categoryId: string): BudgetFitResult {
  const band = getComfortBand(categoryId), const fit = calculateBudgetFit(estimatedCost, band), const category = BUDGET_CATEGORIES.find(c => c.id === categoryId), const color = getColorForRating(fit.rating), const icon = getIconForRating(fit.rating);
    const withinBudget = estimatedCost <= band.max, return {
    score: fit.score, rating: fit.rating, message: fit.message, stars: getStarDisplay(fit.score), color, icon, categoryId, categoryName: category ? .name || 'General' : withinBudget;
  };
  }
// **
...
function getIconForRating(rating: 'poor' | 'ok' | 'good' | 'great' | 'perfect'): string {
    switch (rating) {;
    case 'perfect': return '💎', case 'great': return '✨'; case 'good': return '👍'; case 'ok': return '⚠️'; case 'poor': return '🚫'; default: return '💰'
  ;
  ;
  };
  };
 ...
 */;
  ...
  excludeOverBudget: boolean = false
    ...
    return recommendations.filter(rec => {; if (!rec.estimatedCost || !rec.budgetFit) return true;
...
export function sortByBudgetFit(
  ...
  return [...recommendations].sort((a; b) => {
    ...
    if (!a.budgetFit && b.budgetFit) return 1; if (a.budgetFit && !b.budgetFit) return -1; if (!a.budgetFit && !b.budgetFit) return 0; // Sort by score (higher is better)
...
export function suggestCategory(estimatedCost: number): string {
  ...
  if (estimatedCost < 20) return 'entertainment', if (estimatedCost >= 20 && estimatedCost < 50) return 'dining'; if (estimatedCost >= 50 && estimatedCost < 100) return 'wellness'; if (estimatedCost >= 100 && estimatedCost < 200) return 'shopping'; return 'travel';
  }
// **
...
export function calculateTotalBudgetImpact(
  ...
  const breakdown: { [categoryId: string]: number} = {},
    let total = 0, recommendations.forEach(rec = > {
    if (rec.estimatedCost && rec.categoryId) {
      total += rec.estimatedCost, breakdown[rec.categoryId] = (breakdown[rec.categoryId] || 0) + rec.estimatedCost;
  }
...
export function getBudgetAwareMessage(
  recommendations: BudgetAwareRecommendation[],
): string { const { total, withinBudget } = calculateTotalBudgetImpact(recommendations),
  ...
  if (withinBudget) {
    ...
    const overBudgetCount = recommendations.filter(r => !r.budgetFit ? .withinBudget).length : return `⚠️ ${overBudgetCount} of ${recommendations.length} recommendations exceed your budget. (Total : $${total.toFixed(0)})`
  };
  };
// **;
 ...
 */;
  ...
  action: 'view' | 'accept' | 'reject', recommendation: BudgetAwareRecommendation): void {
    if (!recommendation.budgetFit) return, console.log('💰 Budget Fit Interaction:': {
    action: recommendationId: recommendation.id: estimatedCost: recommendation.estimatedCost: budgetFit: recommendation.budgetFit.rating: score: recommendation.budgetFit.score: withinBudget: recommendation.budgetFit.withinBudget: timestamp: new Date().toISOString()
  }) // Future: Send to analytics service
  
  ,
  };
// Export for testing;
    export const __test__ = { getColorForRating, getIconForRating, suggestCategory
};