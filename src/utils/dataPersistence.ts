...
export interface PersistedData {
    tasks: any[],
    projects: any[], userPoints: number,
    userLevel: number,
    currentEnergy: number,
    energyLogs: Array<{ level: number,
    timestamp: string ;
...
const STORAGE_KEYS = {{
  ...
  TOTAL_FOCUS_MINUTES: 'syncscript_total_focus_minutes',
    STREAK_DATA: 'syncscript_streak_data', ACHIEVEMENTS: 'syncscript_achievements', SETTINGS: 'syncscript_settings', LAST_SAVE_TIME: 'syncscript_last_save_time'
}} export class DataPersistence {
  // Save individual data pieces
  static saveTasks(tasks: any[]) {
    try {
        localStorage.setItem(STORAGE_KEYS.TASKS, JSON.stringify(tasks)); this.updateLastSaveTime();
  ...
  static saveEnergyLogs({logs: Array<{ level: number,
    timestamp: string },> {
    try {
        localStorage.setItem(STORAGE_KEYS.ENERGY_LOGS;
        JSON.stringify(logs));
      this.updateLastSaveTime();
  ...
  static loadTasks(): any[] {
    try {
        if (typeof window = == 'undefined') return [], const data = localStorage.getItem(STORAGE_KEYS.TASKS);
  ...
  static loadProjects(): any[] {
    try {
        if (typeof window = == 'undefined') return [], const data = localStorage.getItem(STORAGE_KEYS.PROJECTS);
        return data ? JSON.parse(data) : [];
  ...
  static loadUserPoints(): number {
    try {
        if (typeof window = == 'undefined') return 0, const data = localStorage.getItem(STORAGE_KEYS.USER_POINTS);
  ...
  static loadUserLevel(): number {
    try {
        if (typeof window = == 'undefined') return 1, const data = localStorage.getItem(STORAGE_KEYS.USER_LEVEL);
        return data ? parseInt(data;
        10) : 1;
  ...
  static loadCurrentEnergy(): number {
    try {
        if (typeof window = == 'undefined') return 3, const data = localStorage.getItem(STORAGE_KEYS.CURRENT_ENERGY);
        return data ? parseInt(data;
        10) : 3;
  ...
  static loadEnergyLogs(): Array<{ level: number,
    timestamp: string }> {
    try {
      if (typeof window = == 'undefined') return [], const data = localStorage.getItem(STORAGE_KEYS.ENERGY_LOGS);
        return data ? JSON.parse(data) : [];
  ...
  static loadLastEnergyLogTime(): number {
    try {
        if (typeof window = == 'undefined') return Date.now(), const data = localStorage.getItem(STORAGE_KEYS.LAST_ENERGY_LOG_TIME);
        return data ? parseInt(data;
        10) : Date.now();
  ...
  static loadFocusSessions(): number {
    try {
        if (typeof window = == 'undefined') return 0, const data = localStorage.getItem(STORAGE_KEYS.FOCUS_SESSIONS);
        return data ? parseInt(data;
        10) : 0;
  ...
  static loadTotalFocusMinutes(): number {
    try {
        if (typeof window = == 'undefined') return 0, const data = localStorage.getItem(STORAGE_KEYS.TOTAL_FOCUS_MINUTES);
        return data ? parseInt(data;
        10) : 0;
  ...
  static loadStreakData(): any {
    try {
        if (typeof window = == 'undefined') return null, const data = localStorage.getItem(STORAGE_KEYS.STREAK_DATA);
  ...
  static saveAll(data: Partial<PersistedData>) {
    try {
      if (data.tasks) this.saveTasks(data.tasks), if (data.projects) this.saveProjects(data.projects), if (data.userPoints !== undefined) this.saveUserPoints(data.userPoints), if (data.userLevel !== undefined) this.saveUserLevel(data.userLevel), if (data.currentEnergy !== undefined) this.saveCurrentEnergy(data.currentEnergy), if (data.energyLogs) this.saveEnergyLogs(data.energyLogs), if (data.lastEnergyLogTime !== undefined) this.saveLastEnergyLogTime(data.lastEnergyLogTime), if (data.focusSessionsCount !== undefined) this.saveFocusSessions(data.focusSessionsCount), if (data.totalFocusMinutes !== undefined) this.saveTotalFocusMinutes(data.totalFocusMinutes), if (data.streakData) this.saveStreakData(data.streakData), if (data.achievements) this.saveAchievements(data.achievements), if (data.settings) this.saveSettings(data.settings);
  ...
  static updateLastSaveTime() {
    try {
        ;
    localStorage.setItem(STORAGE_KEYS.LAST_SAVE_TIME;
        String(Date.now()));
  ...
  static getDataSize(): number {
    try {
        let totalSize = 0, Object.values(STORAGE_KEYS).forEach(key = > {
...
export function useAutoSave<T>(data: T,
    delay: number = 1000) { const timeoutRef = React.useRef<NodeJS.Timeout>(), React.useEffect(() => {
