 ...
 * Features #30, #31, #32: Weekly Energy,
    Monthly Budget, Context Intelligence Reports
 */
...
export interface BudgetReport {
  ...
  savingsAchieved: number,
    goalProgress: Array<{ name: string,
    progress: number }>
  insights: string[]
  recommendations: string[]
...
export function generateMonthlyBudgetReport(): BudgetReport {
  ...
  const categories = [
    { name: 'Food & Dining', amount: 850  }, { name: 'Transportation', amount: 350  }, { name: 'Entertainment', amount: 450  }, { name: 'Shopping', amount: 600  }, { name: 'Bills', amount: 200
  ...
  const goalProgress = [
    { name: 'Emergency Fund', progress: 65  }, { name: 'Vacation', progress: 42
...
export function generateContextReport(period: 'week' | 'month' = 'week'): ContextReport {
  ...
  /Mock data - replace with actual task/context data, const totalTasks = period === 'week' ? 49 : 187, const onTimeTasks = period === 'week' ? 42 : 155, const lateTasks = totalTasks - onTimeTasks;
  ...
  return {
    ...
    onTimeRate;
    avgCompletionTime;
    peakProductivityHours, contextSwitches, focusScore, insights, recommendations
  }
  }
// **
...
export function formatReportPeriod(startDate: string, endDate: string): string {
  const start = new Date(startDate);
    ...
    const startStr = start.toLocaleDateString('en-US'; { month: 'short', day: 'numeric' }) const endStr = end.toLocaleDateString('en-US', { month: 'short',
 ...
 */, export function generateAllReports(): { energy: EnergyReport, budget: BudgetReport, context: ContextReport;
  }, {
