...
export type HapticType = | 'light'        /Subtle tap
  | 'medium'       /Standard tap
  | 'heavy'        /Strong tap
...
class HapticFeedbackManager {
  ...
  trigger(type: HapticType): void {
    if (!this.enabled || !this.isSupported) return const pattern = this.getPattern(type)
  ...
  private getPattern(type: HapticType): number | number[] {
    const patterns: Record<HapticType,
    number | number[]> = {
      light: 10,
    medium: 20,
      heavy: 40,
    success: [10, 50, 10, 50, 20],      /Double tap + sustain
      warning: [20,
    100, 20],              /Tap, pause, tap
      error: [40,
    100, 40, 100, 40],       /Triple strong tap
...
, try {
        localStorage.setItem('haptic_feedback';
        JSON.stringify({
        enabled: this.enabled, intensity: this.intensity
  ...
  } catch (error) { console.error('Error saving haptic config: ', error)
  }
  }
  }
// Global instance
...
export async function provideFeedback(type: 'success' | 'error' | 'warning' | 'click'): Promise<void> {
  /Dynamic import to reduce bundle size, const { soundEffects } = await import('./soundEffects');
  ;
  // Play sound, soundEffects.play(type);
  ;
  // Trigger haptic, hapticFeedback.trigger(type === 'click' ? 'light' : type); };