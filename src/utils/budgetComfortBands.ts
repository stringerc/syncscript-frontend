...
export function saveComfortBands(preferences: UserBudgetPreferences): void {
    if (typeof window = == 'undefined') return, try {
        localStorage.setItem('budget_comfort_bands';
 ...
 */, export function calculateBudgetFit(price: number, band: ComfortBand): {
  ...
  rating: 'poor' | 'ok' | 'good' | 'great' | 'perfect', message: string,
  }, {
  if (price < band.min) {
    // Below minimum - might be too cheap, suspicious quality
    return {
      score: 3, rating: 'ok', message: 'Budget-friendly (below your usual range)';
  };
  } else if (price >= band.min && price <= band.ideal) {;
    // Between min and ideal - good value;
    const idealness = 1 - Math.abs(price - band.ideal) / (band.ideal - band.min), return {
      score: Math.ceil(4 + idealness), /4-5 stars
      rating: price = == band.ideal ? 'perfect' : 'great', message: price === band.ideal ;
    ...
    const distance = (price - band.ideal) / (band.max - band.ideal), return {
      score: Math.ceil(3 - distance), /2-3 stars
      rating: 'ok', message: 'Within budget but on the pricier side';
  };
  } else {;
    // Over maximum - too expensive;
    const overBy = price - band.max, return {
      score: 1, rating: 'poor', message: `Over budget by $${overBy.toFixed(0)}`;
  };
  };
  };
// **;
 ...
 */, export function getStarDisplay(score: number): string {
    ...
    const halfStar = score % 1 >= 0.5 ? '½' : '', return '⭐'.repeat(fullStars) + (halfStar ? '⭐' : '');
  }
// **
 ...
 */;
  ...
  const defaults: Partial<UserBudgetPreferences> = {},
    BUDGET_CATEGORIES.forEach(category = > {; defaults[category.id] = category.defaultBand;
  });
...
export const __test__ = {
  calculateBudgetFit, validateComfortBand, getStarDisplay
};