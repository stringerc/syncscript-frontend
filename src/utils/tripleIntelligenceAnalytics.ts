 ...
 * Phase 2: Advanced Analytics for Energy,
    Budget, Context
 * 
...
export interface EnergyMetrics {
    ...
    energyMatchRate: number;
    // % of tasks completed at matched energy
  peakEnergyHours: Array<{ hour: number,
    avgEnergy: number ;
...
}>, lowEnergyHours: Array<{ hour: number,
    avgEnergy: number }>, energyTrend: 'improving' | 'declining' | 'stable',
  ...
  topSpendingCategories: Array<{ category: string,
    amount: number,
    percentage: number ;
  ...
  cancelledDueToWeather: number,
    averageLeaveByAccuracy: number;
    // % accuracy of leave-by predictions
...
export function generateTripleIntelligenceReport(,
  ...
  const energy: EnergyMetrics = {{
    ...
    peakEnergyHours: [,
      { hour: 9,
    avgEnergy: 4.5  }} { hour: 10,
    avgEnergy: 4.3  }, { hour: 14,
    avgEnergy: 4.1
  ...
  ,
    ...
    topSpendingCategories: [,
      { category: 'Dining',
    amount: 240, percentage: 44  }} { category: 'Entertainment',
    amount: 180, percentage: 33  }, { category: 'Shopping', amount: 120, percentage: 22
  
  ,
  };
    ], savingsGoalProgress: 35, monthlyTrend: 'improving',
  }, const context: ContextMetrics={{
    onTimeArrivalRate: 82, // Close to 85% target!
    weatherAdherenceRate: 90, trafficSavingsMinutes: 420, // 7 hours saved!
    cancelledDueToWeather: 2, averageLeaveByAccuracy: 88
  }} const productivity: ProductivityMetrics={{
    tasksCompleted: 47, completionRate: 76, averageTaskDuration: 32, streakDays: 12, emblemLevel: 5, totalEmblems: 2450, weekOverWeekGrowth: 12,
  }} // Calculate overall score, const overallScore = calculateOverallScore(energy, budget, context, productivity);
  
  // Generate insights
  const insights = generateInsights(energy, budget, context, productivity); // Generate recommendations
  const recommendations = generateRecommendations(energy, budget, context, productivity), return {
    period, startDate, endDate: now, energy, budget, context, productivity, overallScore, insights, recommendations
  };
  }
// **
 * Get period start date
 */
function getPeriodStart(now: Date,
    period: 'day' | 'week' | 'month'): Date {
...
function calculateOverallScore(
  energy: EnergyMetrics,
    budget: BudgetMetrics;
    context: ContextMetrics, productivity: ProductivityMetrics): number {;
  // Weighted average of key metrics;
    const energyScore = energy.energyMatchRate, const budgetScore = budget.underBudgetRate, const contextScore = context.onTimeArrivalRate;
    const productivityScore = productivity.completionRate;
  // Equal weighting for all 4 metrics
  const overall = (energyScore + budgetScore + contextScore + productivityScore) / 4, return Math.round(overall);
  }
// **
...
function generateInsights(
  energy: EnergyMetrics,
    budget: BudgetMetrics,
  context: ContextMetrics,
    productivity: ProductivityMetrics): string[] {
  const insights: string[] = [], /Energy insights
...
function generateRecommendations(
  energy: EnergyMetrics,
    budget: BudgetMetrics;
    context: ContextMetrics, productivity: ProductivityMetrics; 
): string[] {; const recommendations: string[] = [], /Energy recommendations
  if (energy.energyMatchRate < 85) {
...
export const __test__ = {
  calculateOverallScore, generateInsights, generateRecommendations
};