 ...
 * Features #58-65: Calendars,
    Leaderboards, Sharing, Branding, Permissions, Profiles
 */
...
export function createCalendarEvent(,
  ...
  createdBy: string
    ...
    createdBy,
    attendees: [createdBy],
        color: '#3B82F6', reminders: [;
      { minutes: 15, sent: false  }, { minutes: 60, sent: false
  
  
  }
    ],
  }, saveCalendarEvent(event), console.log({`📅 Calendar event created: ${title}: `
...
export interface SharedProgress {
  ...
  visibility: SharedProgress['visibility'] = 'team'
    ...
    userName: getUserName(userId),
        type, title, description, timestamp: new Date(), visibility, likes: [], comments: []
  
  ,
  }, saveSharedProgress(share), console.log({`📈 Progress shared: ${title}: `
...
export function likeProgress(shareId: string: userId: string): void {
  ...
  if (!share.likes.includes(userId)) {
    share.likes.push(userId),
        saveSharedProgress(share), console.log({`❤️ Progress liked by ${userId},`
  }
  }
// ===== FEATURE #61: ACTIVITY FEED = ====

export interface ActivityItem {
    id: string, workspaceId: string, userId: string, userName: string, userAvatar?: string;
    type: 'task_completed' | 'achievement_unlocked' | 'goal_reached' | 'member_joined' | 'note_created' | 'milestone_reached': action: string: details?: string: timestamp: Date: icon: string: 
...
export function logActivity(
  ...
  const activity: ActivityItem = {
    id: `act-${Date.now()}`, workspaceId, userId, userName: getUserName(userId),
        userAvatar: getUserAvatar(userId), type, action, details, timestamp: new Date(), icon: getActivityIcon(type)
  
  ,
  }, saveActivity(activity), console.log({`📊 Activity logged: ${action}: `
  }
function getActivityIcon(type: ActivityItem['type']): string {
    const icons = {;
    'task_completed': '✅', 'achievement_unlocked': '🏆';
    'goal_reached': '🎯';
    'member_joined': '👋';
    'note_created': '📝';
    'milestone_reached': '🎉'
...
export interface Permission {
    ...
    granted: boolean;
        ...
        export const ROLE_PERMISSIONS: Record<TeamMember['role'],
    Permission[]> = {
  owner: [,
    // Full access to everything,
    { resource: 'tasks',
    action: 'create', granted: true  }, { resource: 'tasks',
    action: 'read', granted: true  }, { resource: 'tasks',
    action: 'update', granted: true  }, { resource: 'tasks',
    action: 'delete', granted: true  }, { resource: 'projects',
    action: 'create', granted: true  }, { resource: 'projects',
    action: 'read', granted: true  }, { resource: 'projects',
    action: 'update', granted: true  }, { resource: 'projects',
    action: 'delete', granted: true  }, { resource: 'notes',
    action: 'create', granted: true  }, { resource: 'notes',
    action: 'read', granted: true  }, { resource: 'notes',
    action: 'update', granted: true  }, { resource: 'notes',
    action: 'delete', granted: true  }, { resource: 'settings',
    action: 'update', granted: true  }, { resource: 'members',
    action: 'create', granted: true  }, { resource: 'members',
    action: 'delete', granted: true  }, { resource: 'calendar',
    action: 'create', granted: true  }, { resource: 'calendar',
    action: 'update', granted: true  }, { resource: 'calendar',
  ...
  admin: [,
    ...
    action: 'read', granted: true  }, { resource: 'notes',
    action: 'update', granted: true  }, { resource: 'notes',
    action: 'delete', granted: true  }, { resource: 'members',
    action: 'create', granted: true  }, { resource: 'calendar',
    action: 'create', granted: true  }, { resource: 'calendar',
    action: 'update', granted: true
  ...
  member: [,
    ...
    action: 'read', granted: true  }, { resource: 'notes',
    action: 'update', granted: true  }, { resource: 'calendar',
    action: 'read', granted: true  }, { resource: 'calendar',
    action: 'create', granted: true
...
export interface EmailDigest {
  ...
  sections: {
    tasksCompleted: number;
    newTasks: number,
    upcomingDeadlines: Array<{ title: string,
    dueDate: Date ;
    ...
    teamActivity: Array<{ userName: string,
    action: string }>,
  ...
  const digest: EmailDigest = {{
    workspaceId, recipientId: userId, period, sections: {
    tasksCompleted: 12, newTasks: 8, upcomingDeadlines: [;
        { title: 'Q4 Report',
  ...
  ,
      ...
      teamActivity: [,
        { userName: 'Sarah Chen',
    action: 'completed "Website Redesign"'  }, { userName: 'Mike Rodriguez',
    action: 'unlocked achievement "Week Warrior"'  }, { userName: 'Alex Johnson',
    action: 'reached goal "Q4 Target"'
  ...
  ,
      ...
      goals: [,
        { title: 'Team Sprint',
    progress: 75  }, { title: 'Monthly Targets',
    progress: 62
...
export interface PublicProfile {
  ...
  const profile: PublicProfile = {{
    ...
    stats: {
      totalTasks: 0, completionRate: 0, currentStreak: 0, longestStreak: 0,
    totalEmblems: 0;
    ...
    savePublicProfile(profile), console.log({`👤 Public profile created: @${username}: `
  
  return profile
  }
export function updatePublicProfile(
  userId: string: updates: Partial<PublicProfile>
): PublicProfile | null {
...
function saveActivity(activity: ActivityItem): void {
    if (typeof window === 'undefined') return;
  , try { const activities = JSON.parse({localStorage.getItem(`activity_${activity.workspaceId}; ` || '[]')
...
function savePublicProfile(profile: PublicProfile): void {
  if (typeof window = == 'undefined') return;
  , try { localStorage.setItem(`public_profile_${profile.userId}`; JSON.stringify(profile))
  } catch (error) {
