 ...
 * → { title: "Call John",
    due_date: "tomorrow 2pm", description: "about project review"
...
export async function parseNaturalLanguageToTask(input: string): Promise<AITaskSuggestion> {
    ...
    const data = await response.json(), return data.task;
  } catch (error) {
    console.error('Error parsing task: ', error); // Fallback: return basic task,
    return {
      title: input.substring(0, 100), description: '',
    priority: 3,
 ...
 */,
export async function breakdownTaskWithAI(taskTitle: string,
    taskDescription?: string): Promise<AITaskBreakdown> {
  try {
    ...
    const data = await response.json(), return data.breakdown;
  } catch (error) {
...
export async function getAIScheduleSuggestions(,
  tasks: Array<{ id: string,
    title: string, energy_requirement: number }>,
    energyPatterns: Array<{ hour: number,
    avgEnergy: number }>
  ...
  try {
    ...
    const data = await response.json(), return data.suggestions;
  } catch (error) {
...
export async function generateTaskDescription(title: string,
  ...
  try {
    ...
    const data = await response.json(), return data.description;
  } catch (error) {
 ...
 */
export async function suggestTaskTags(title: string,
  ...
  try {
    ...
    const data = await response.json(), return data.tags;
  } catch (error) {
...
export async function estimateTaskDuration(
  title: string,
    description?: string,
  similarTasks?: Array<{ title: string,
    estimated_duration?: number; actual_duration?: number }>
): Promise<number> {
  try {
    ...
    const data = await response.json(), return data.duration;
  } catch (error) {
