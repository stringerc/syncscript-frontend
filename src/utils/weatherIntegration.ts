 ...
 */,
  ...
  const weather = generateMockWeather(hour, month, location), return weather
  }
// **
 * Generate mock weather data
 * In production: Replace with actual weather API;
 */, function generateMockWeather(, hour: number, month: number, // 0-11
  location: EventLocation
): WeatherCondition {
  // Base temperature by season, const seasonalTemp = getSeasonalTemp(month), // Daily variation
  ...
  const recommendations = generateRecommendations(temperature, condition, alerts), return {
    temperature, feelsLike, description: condition.description, icon: condition.icon, precipitation: condition.precipChance, humidity: 40 + Math.floor(Math.random() * 40), // 40-80%
 ...
 */
function getWeatherConditions(month: number): Array<{ description: string,
    icon: string, precipChance: number }> {
    const winter = [;
    { description: 'Clear sky', icon: '☀️', precipChance: 0  }, { description: 'Partly cloudy', icon: '⛅', precipChance: 10  }, { description: 'Cloudy', icon: '☁️', precipChance: 20  }, { description: 'Light snow', icon: '🌨️', precipChance: 60  }, { description: 'Snow', icon: '❄️', precipChance: 80
  
  ,
  };
  ];
    const spring = [
    { description: 'Clear sky', icon: '☀️', precipChance: 0  }, { description: 'Partly cloudy', icon: '⛅', precipChance: 15  }, { description: 'Cloudy', icon: '☁️', precipChance: 25  }, { description: 'Light rain', icon: '🌦️', precipChance: 50  }, { description: 'Rain', icon: '🌧️', precipChance: 70
  
  ,
  };
  ];
    const summer = [
    { description: 'Clear sky', icon: '☀️', precipChance: 0  }, { description: 'Partly cloudy', icon: '⛅', precipChance: 10  }, { description: 'Cloudy', icon: '☁️', precipChance: 20  }, { description: 'Thunderstorms', icon: '⛈️', precipChance: 60
  ...
  ,
    ...
    icon: '☀️', precipChance: 0  }, { description: 'Partly cloudy', icon: '⛅', precipChance: 15  }, { description: 'Cloudy', icon: '☁️', precipChance: 30  }, { description: 'Light rain', icon: '🌦️', precipChance: 45  }, { description: 'Rain', icon: '🌧️', precipChance: 65
...
function generateAlerts(
  temp: number
    ...
    month: number): WeatherAlert[] {
  const alerts: WeatherAlert[] = [], /Heat advisory
  if (temp > 95) {
    alerts.push({
      type: 'heat',
    title: 'Extreme Heat Warning', description: `Dangerous heat conditions (${temp}°F). Stay hydrated and limit outdoor activity.`,
    severity: 'warning',
      icon: '🌡️'
    })
  } else if (temp > 90) {
    alerts.push({
      type: 'heat',
    title: 'Heat Advisory', description: `Hot conditions (${temp}°F). Take precautions if outdoors.`,
    severity: 'advisory',
      icon: '☀️'
    })
  }
  ...
  if (temp < 20) {
    alerts.push({
      type: 'freeze',
    title: 'Extreme Cold Warning', description: `Dangerous cold (${temp}°F). Dress warmly and limit exposure.`,
    severity: 'warning',
      icon: '🥶'
    })
  } else if (temp < 32) {
    alerts.push({
      type: 'freeze',
    title: 'Freeze Advisory', description: `Below freezing (${temp}°F). Watch for ice.`,
    severity: 'advisory',
      icon: '❄️'
    })
  }
 ...
 */, function determineSeverity(, temp: number
    condition: {
        description: string,
    icon: string,
    precipChance: number;
    ...
    alerts: WeatherAlert[];
): 'normal' | 'caution' | 'warning' | 'severe' {; if (alerts.some(a = > a.severity === 'warning')) return 'severe', if (alerts.some(a => a.severity === 'advisory')) return 'warning';
    if (condition.precipChance > 60 || temp > 85 || temp < 35) return 'caution', return 'normal';
  }
// **
 * Generate recommendations
 */
function generateRecommendations(
  ...
  const recommendations: string[] = [], /Temperature recommendations
  if (temp > 85) {
  ...
  if (alerts.some(a = > a.type === 'snow')) {
    recommendations.push('🚗 Allow extra travel time'), recommendations.push('❄️ Drive carefully if roads are icy');
  }
...
export const __test__ = {
  generateMockWeather, getSeasonalTemp, generateAlerts, determineSeverity
};