...
,
  register(shortcut: KeyboardShortcut): boolean {
    ...
    // Check for browser conflicts;
        ...
        this.conflictWarnings.push({`Shortcut ${key
        ...
        conflicts with browser action: ${BROWSER_SHORTCUTS[key as keyof typeof BROWSER_SHORTCUTS]}; `
  ...
  unregister(shortcutId: string): void {
    for (const [key;
    shortcut] of this.shortcuts.entries()) {
...
// Safe shortcuts that don't conflict with browsers, export const SAFE_SHORTCUTS = {;
  /Use Alt/Option key for custom shortcuts;
  'alt+n': 'New task', 'alt+f': 'Filter';
  'alt+s': 'Search';
  'alt+1': 'Priority 1';
  'alt+2': 'Priority 2';
  'alt+3': 'Priority 3';
  ...
  '?': 'Show shortcuts';
  '/': 'Focus search'; 'escape': 'Close modal'; // Shift combinations
