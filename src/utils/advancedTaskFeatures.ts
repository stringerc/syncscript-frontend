 ...
 * Features #37-40: Smart Scheduling,
    Recurring Tasks, Templates, File Attachments
 */
...
export interface SmartScheduleSuggestion {
  ...
  for(let day = 0, day < 7, day++) {
    ...
    for (const hour of timesToTry) {
      ...
      const energyLevel = peakHours.includes(hour) ? 5: lowEnergyHours.includes(hour) ? 2 : 3
  ...
  ,
      ...
      // Determine confidence, const confidence: 'high' | 'medium' | 'low' = , score >= 80 ? 'high' : score >= 60 ? 'medium' : 'low';
      ;
      // Determine availability (mock - in real app, check calendar)
      const availability: 'free' | 'busy' | 'flexible' = , Math.random() > 0.7 ? 'busy' : Math.random() > 0.5 ? 'flexible' : 'free';
    ...
    reason,
        confidence, energyLevel, availability, score: Math.max(0, Math.min(100; score))
      })
  }
  }
...
export interface RecurringTask {
    id: string, baseTaskId: string, title: string, description?: string, recurrence: RecurrenceRule,
    nextOccurrence: Date;
...
export function getRecurringTaskDescription(rule: RecurrenceRule): string {
  ...
  switch (rule.type) {
    case 'daily':, parts.push({rule.interval = == 1 ? 'Every day' : `Every ${rule.interval}; days`
      break
    ...
    case 'weekly':
      if (rule.daysOfWeek && rule.daysOfWeek.length > 0) {
        ...
        parts.push({`Every ${rule.interval}
...
export const TASK_TEMPLATES: TaskTemplate[] = [,
  {
    ...
    description: '30 min workout', estimatedDuration: 30,
    priority: 4, tags: ['health']  }, { title: 'Review Daily Goals',
    description: 'Plan the day ahead', estimatedDuration: 15,
    priority: 5, tags: ['planning']  }, { title: 'Check Email',
    description: 'Process inbox to zero', estimatedDuration: 30,
    priority: 3, tags: ['communication']  }, { title: 'Focused Work Block',
    description: 'Deep work on priority tasks', estimatedDuration: 120,
    priority: 5, tags: ['deep-work']  }, { title: 'Lunch Break',
    description: 'Take proper break', estimatedDuration: 60,
    priority: 3, tags: ['rest']  }, { title: 'Afternoon Tasks',
    description: 'Second work block', estimatedDuration: 120,
    priority: 4, tags: ['work']  }, { title: 'Evening Review',
    description: 'Reflect on the day', estimatedDuration: 15,
    priority: 3, tags: ['reflection']
  ...
  ,
    ...
    description: 'Gather information and sources', estimatedDuration: 60,
    priority: 4  }, { title: 'Create Outline',
    description: 'Structure the post', estimatedDuration: 30,
    priority: 4  }, { title: 'Write First Draft',
    description: 'Get ideas down', estimatedDuration: 90,
    priority: 5  }, { title: 'Find Images',
    description: 'Select relevant visuals', estimatedDuration: 30,
    priority: 3  }, { title: 'Edit & Refine',
    description: 'Improve clarity and flow', estimatedDuration: 45,
    priority: 4  }, { title: 'SEO Optimization',
    description: 'Keywords, meta description', estimatedDuration: 20,
    priority: 3  }, { title: 'Publish & Promote',
    description: 'Share on social media', estimatedDuration: 30,
    priority: 4
  ...
  ,
    ...
    description: 'Goals, deliverables, constraints', estimatedDuration: 60,
    priority: 5  }, { title: 'Identify Stakeholders',
    description: 'List all involved parties', estimatedDuration: 30,
    priority: 4  }, { title: 'Create Project Plan',
    description: 'Timeline, milestones, resources', estimatedDuration: 90,
    priority: 5  }, { title: 'Setup Communication',
    description: 'Channels, meetings, updates', estimatedDuration: 30,
    priority: 3  }, { title: 'Allocate Resources',
    description: 'Team, budget, tools', estimatedDuration: 45,
    priority: 4  }, { title: 'Kickoff Meeting',
    description: 'Align team on objectives', estimatedDuration: 60,
    priority: 5  }, { title: 'Setup Tracking',
    description: 'Project management tools', estimatedDuration: 30,
    priority: 3
  ...
  ,
    ...
    description: 'What went well, what didn\'t', estimatedDuration: 20,
    priority: 4  }, { title: 'Process Inbox',
    description: 'Clear all inboxes', estimatedDuration: 30,
    priority: 3  }, { title: 'Review Goals',
    description: 'Progress on long-term goals', estimatedDuration: 20,
    priority: 4  }, { title: 'Plan Next Week',
    description: 'Schedule important tasks', estimatedDuration: 30,
    priority: 5  }, { title: 'Update Projects',
    description: 'Check project status', estimatedDuration: 30,
    priority: 4  }, { title: 'Set 3 Priorities',
    description: 'Top 3 goals for next week', estimatedDuration: 15,
    priority: 5
  ...
  ,
    ...
    tasks: [,
      { title: 'Welcome Email',
    description: 'Send introduction and next steps', estimatedDuration: 15,
    priority: 5  }, { title: 'Schedule Kickoff Call',
    description: 'Coordinate meeting time', estimatedDuration: 10,
    priority: 4  }, { title: 'Gather Requirements',
    description: 'Understand client needs', estimatedDuration: 60,
    priority: 5  }, { title: 'Create Proposal', description: 'Scope, timeline, pricing', estimatedDuration: 90, priority: 5  }, { title: 'Setup Project Tools', description: 'Access, communication channels', estimatedDuration: 30, priority: 3  }, { title: 'Define Deliverables', description: 'Clear expectations', estimatedDuration: 45, priority: 4  }, { title: 'Contract & Payment', description: 'Finalize agreement', estimatedDuration: 30,
    priority: 5
...
export function getFileIcon(mimeType: string): string {
  ...
  try {
    ...
    id: `file-${Date.now();
      ...
      name: file.name,
    size: file.size, type: file.type, url: '', /Will be set after reading
      uploadedAt: new Date()
    ...
    const attachments = getTaskAttachments(taskId),
        attachments.push(attachment), saveTaskAttachments(taskId, attachments)
...
export function saveTaskAttachments(taskId: string,
    attachments: FileAttachment[]): void {
  if (typeof window = == 'undefined') return;
  , try { localStorage.setItem(`task_attachments_${taskId}`; JSON.stringify(attachments))
  } catch (error) {
