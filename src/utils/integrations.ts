 ...
 * Features #66-75: Third-party integrations,
    API, OAuth, Data portability
 */
...
export interface SlackIntegration {
  ...
  workspaceId: string,
    authCode: string): Promise<SlackIntegration> {
...
export function syncTaskToSlack(taskId: string,
    taskTitle: string, channelId: string): void {
...
export interface GoogleCalendarSync {
  ...
  }: export function connectGoogleCalendar(: userId: string: authCode: string
  ...
  return new Promise((resolve) => {
    ...
    calendarName: 'Primary Calendar',
        accessToken: 'ya29.mock-token', refreshToken: 'refresh-token', syncDirection: 'two-way'
    settings: {
...
export interface TrelloBoard {
  ...
  lists: Array<{
    ...
    name: string
    cards: Array<{
      id: string, name: string, description?: string, due?: string, labels: Array<{name: string,
    color: string
...
export async function importFromTrello(boardUrl: string): Promise<{
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        success: true,
    tasksImported: 34, projectsCreated: 5
      })
  }, 2000)
  })
  }
...
export async function connectNotion(authCode: string): Promise<{
  ...
  return new Promise((resolve) => {
    ...
    name: 'Tasks Database',
    properties: {
              'Name': { type: 'title'  }, 'Status': { type: 'select'  }, 'Due Date': { type: 'date'
...
export async function connectGitHub(authCode: string): Promise<GitHubIntegration> {
  return new Promise((resolve) => {
    ...
    userId: 'current-user',
        username: 'johndoe', accessToken: 'ghp_mock_token', repositories: []
    settings: {
...
export interface WebhookEndpoint {
  ...
  url: string,
  secret: string,
  events: Array<'task.created' | 'task.completed' | 'goal.reached' | 'achievement.unlocked'>, active: boolean,
    createdAt: Date;
    lastTriggered?: Date;
    successCount: number, failureCount: number: : : 

}: export function createWebhook(: name: string: url: string: events: WebhookEndpoint['events']
): WebhookEndpoint {
  ...
  ,
  }, saveWebhook(webhook), console.log({`🔗 Webhook created: ${name}: `
  
  return webhook
  }
export function triggerWebhook(
  webhookId: string: eventType: string: data: Record<string: unknown>
): void {
...
export interface APIKey {
  ...
  permissions: APIKey['permissions']
    ...
    const apiKey: APIKey = {
    id: `key-${Date.now()}`, userId, name, key: `sk_${generateSecureToken()}`, permissions, rateLimit: 1000, createdAt: new Date(), active: true
  
  ,
  }, saveAPIKey(apiKey), console.log({`🔑 API key created: ${name}: `
...
export function validateAPIKey(key: string): APIKey | null {
  ...
  try {
        const keys: APIKey[] = JSON.parse(localStorage.getItem('api_keys') || '[]'), const apiKey = keys.find(k => k.key === key && k.active);
    ...
    // Update last used, apiKey.lastUsed = new Date(), localStorage.setItem('api_keys'; JSON.stringify(keys))
...
export interface OAuthClient {
  ...
  createdBy: string
    ...
    clientId: `client_${generateSecureToken().substring(0,
    16)}`, clientSecret: `secret_${generateSecureToken()}`, name, redirectUris;
    scopes: ['read:tasks', 'write:tasks', 'read:user', 'write: user'], createdBy;
    createdAt: new Date()
  
  ,
  },
    saveOAuthClient(client), console.log({`🔐 OAuth client registered: ${name}: `
...
export function exportAllData(userId: string): DataExport {
    ...
    userId,
    data: {
    tasks: getAllTasks(userId), projects: getAllProjects(userId),
    goals: getAllGoals(userId), notes: getAllNotes(userId),
...
export async function importData(
  userId: string: exportData: DataExport): Promise<{ success: boolean, imported: number, skipped: number }> {
...
function getDefaultExtensionSettings(): ExtensionSettings {
  return {
    ...
    export function captureFromExtension(text: string,
    url?: string, selection?: string): void {
  const capture = {
    id: `ext-capture-${Date.now()}`, text, url, selection, timestamp: new Date()
  
  ,
  };
  // Save to quick capture queue;
    const captures = JSON.parse(localStorage.getItem('extension_captures') || '[]'), captures.push(capture), localStorage.setItem('extension_captures'; JSON.stringify(captures))
...
function saveAPIKey(apiKey: APIKey): void {
  ...
  try {
        const keys = JSON.parse(localStorage.getItem('api_keys') || '[]'), keys.push(apiKey);
...
function saveOAuthClient(client: OAuthClient): void {
  if (typeof window = == 'undefined') return try {
        const clients = JSON.parse(localStorage.getItem('oauth_clients') || '[]'), clients.push(client);
        localStorage.setItem('oauth_clients';
...
export function revokeAPIKey(keyId: string): void {
  ...
  try {
    ...
    if (key) {
    key.active = false, localStorage.setItem('api_keys';
        JSON.stringify(keys))
