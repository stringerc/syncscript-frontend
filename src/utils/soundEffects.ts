...
class SoundEffectsManager {
  ...
  play(sound: SoundType): void {
    if (!this.enabled || !this.audioContext) return try {
        const frequency = this.getFrequency(sound),
    const duration = this.getDuration(sound), this.playTone(frequency;
        duration;
        this.getWaveType(sound))
  ...
  private playTone(frequency: number,
    duration: number, type: OscillatorType = 'sine'): void {
    if (!this.audioContext) return const oscillator = this.audioContext.createOscillator();
...
, try {
        localStorage.setItem('sound_effects';
        JSON.stringify({
