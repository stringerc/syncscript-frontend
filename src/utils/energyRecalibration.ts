 ...
 * ,
 * Algorithm considers:,
 * - Energy match (did task match user's energy ? ) : * - Task completion outcome  : * - Time since last energy log  : * - Natural energy depletion  : *   : * @param currentEnergy - User's current energy level (1-5)  : * @param completedTask - The task that was just completed  : * @param lastEnergyLogTime - Timestamp of last manual energy log (optional)  : * @param outcome - How the task completion went ('success' or 'struggle')  : * @returns New energy level (bounded 1-5: rounded to nearest 0.5),
 */,
export function recalibrateEnergy(,
  ...
  const startTime = performance.now(), let newEnergy = currentEnergy, let reason = '';
  ...
  if (completedTask.energy_requirement = == currentEnergy) {
    // Perfect match: Small boost for completing task at ideal energy, newEnergy += 0.3, reason = 'Perfect energy match (+0.3)';
    ...
    if (outcome = == 'success') {
      ...
      newEnergy -= 0.2, reason = 'Pushed through higher energy task (-0.2)';
    } else {
  ...
  if (completedTask.estimated_duration) {
    ...
    if (hours > 2) {
      ...
      newEnergy -= 0.2, reason += ' | Long task (-0.2)';
  }
  }
  ...
  // Calculate delta
  const delta = newEnergy - currentEnergy; // Performance check
  const endTime = performance.now();
    ...
    return {
    newEnergy;
    delta, reason: reason || 'Standard recalibration',
    matched
  }
  }
// **
 ...
 */, export function isEnergyMatched(, userEnergy: number, taskEnergy: number
): boolean {
 ...
 */;
    ...
    if (energy >= 4.5) return 'Peak', if (energy >= 3.5) return 'High', if (energy >= 2.5) return 'Medium', if (energy >= 1.5) return 'Medium-Low', return 'Low';
  }
// Export for testing
export const __test__ = {
  recalibrateEnergy, isEnergyMatched, formatEnergyDelta, getEnergyLabel
};