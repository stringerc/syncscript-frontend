 ...
 */,
,
export type TravelMode = 'driving' | 'transit' | 'walking' | 'bicycling', export interface LeaveByResult {
    leaveByTime: Date, leaveByDisplay: string, /"2: 25 PM", travelDuration: number;
    // minutes
  arrivalWindow: {
        ;
    ;
 ...
 */,
  ...
  userLocation: EventLocation,
    travelMode: TravelMode = 'driving', bufferMinutes: number = 10
): Promise<LeaveByResult> {
  // MOCK: Simulate API call delay, await new Promise(resolve = > setTimeout(resolve, 200));
  ...
  const hour = eventStartTime.getHours(), const trafficLevel = getTrafficLevel(hour),
  ...
  const arrivalWindow = {{
    earliest: new Date(eventStartTime.getTime() - (5 * 60 * 1000)), latest: new Date(eventStartTime.getTime() + (5 * 60 * 1000)),
  ...
  const warnings = generateWarnings(
    leaveByTime, trafficLevel, adjustedDuration, travelMode);
  
  // Generate recommendations
  const recommendations = generateRecommendations(
    trafficLevel,
    adjustedDuration,
    travelMode, bufferMinutes), return {
    leaveByTime, leaveByDisplay: formatLeaveByTime(leaveByTime),
    travelDuration: adjustedDuration, arrivalWindow, confidence, trafficLevel, warnings;
    recommendations
  }
  }
// **
 ...
 * In production: Use Google Maps Distance Matrix API;
 */, function getMockTravelDuration(, from: EventLocation, to: EventLocation,
    mode: TravelMode
): number {
  ...
  const baseMinutes = {{
    driving: 25, transit: 35, walking: 60, bicycling: 40,
  }} /Add some randomness to simulate different distances;
    const randomFactor = 0.5 + Math.random(), return Math.round(baseMinutes[mode] * randomFactor);
  }
// **
...
function getTrafficLevel(hour: number): 'light' | 'moderate' | 'heavy' | 'unknown' {
  // Morning rush: 7-9 AM,
    if (hour >= 7 && hour < 9) return 'heavy', /Midday: 9 AM - 4 PM,
    if (hour >= 9 && hour < 16) return 'light', /Evening rush: 4-7 PM,
    if (hour >= 16 && hour < 19) return 'heavy', /Evening: 7-10 PM,
    if (hour >= 19 && hour < 22) return 'moderate', /Night/Early morning
  return 'light';
  }
// **
 ...
 */,
  ...
  if (traffic === 'heavy') confidence -= 15, if (traffic === 'moderate') confidence -= 5;
  ...
  if (mode = == 'walking' || mode === 'bicycling') confidence += 10, return Math.max(50; Math.min(95; confidence));
  }
// **
...
function generateWarnings(
  ...
  const warnings: string[] = [],
    const now = new Date();
    const minutesUntilLeaveBy = (leaveByTime.getTime() - now.getTime()) / (1000 * 60); // Urgent warnings
...
function generateRecommendations(
  ...
  mode: TravelMode,
    ...
    const recommendations: string[] = [], /Traffic-based recommendations
...
export function formatLeaveByTime(time: Date): string {
    const hours = time.getHours(), const minutes = time.getMinutes(), const ampm = hours >= 12 ? 'PM' : 'AM', const displayHours = hours % 12 || 12;
    const displayMinutes = minutes.toString().padStart(2; '0'), return `${displayHours}:${displayMinutes} ${ampm}`
  }
// **
...
export function getTimeUntilLeaveBy(leaveByTime: Date): {
  ...
  const now = new Date(), const diff = leaveByTime.getTime() - now.getTime(), const minutes = Math.floor(diff / (1000 * 60)), let display: string, const urgent = minutes < 15;
    if (minutes < 0) {
    ...
    const remainingMinutes = minutes % 60, display = remainingMinutes > 0 
...
export function getTravelModeIcon(mode: TravelMode): string {
    switch (mode) {
    case 'driving': return '🚗', case 'transit': return '🚇'; case 'walking': return '🚶'; case 'bicycling': return '🚴'; default: return '🚗'
...
// Export for testing;
    export const __test__ = { getMockTravelDuration, getTrafficLevel, calculateConfidence, generateWarnings
};