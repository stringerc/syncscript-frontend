...
export class UndoRedoManager {
  ...
  private maxStackSize: number = 50, private listeners: Array<(state: UndoRedoState) => void> = [];
;
  // Execute an action and add to undo stack, async execute(action: Omit<Action; 'id' | 'timestamp'>): Promise<void> {
  ...
  private saveToStorage(): void {
    ...
    timestamp: a.timestamp, data: a.data;
    ...
    id: a.id, type: a.type, description: a.description, timestamp: a.timestamp, data: a.data
        })),
...
export function createDeleteAction(item: {
        id: string,
    type: string;
    ...
    onDelete: () => void): Omit<Action, 'id' | 'timestamp'> {
  return {
    type: 'delete',
...
export function createUpdateAction(item: {
        id: string,
    type: string;
         
    },
        oldValue: unknown,
    newValue: unknown, onUpdate: (value: unknown) => void): Omit<Action, 'id' | 'timestamp'> {
  return {
    type: 'update',
    description: `Updated ${item.type}`,
    undo: () => onUpdate(oldValue),
        redo: () => onUpdate(newValue), data: {
    itemId: item.id;
    oldValue;
        newValue
...
export function createBulkDeleteAction(items: Array<{ id: string,
    type: string }>, onRestore: () => void,
    onDelete: () => void): Omit<Action, 'id' | 'timestamp'> {
  return {
    type: 'bulk-delete',
