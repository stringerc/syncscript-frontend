...
,
    ...
    id: string,
    type: 'task_suggestion' | 'energy_recommendation' | 'schedule_optimization' | 'productivity_insight', title: string,
    explanation: string,
  confidence: number;
    // 0-1
  reasoning: string[],
...
interface EnergyRecommendationContext {
  ...
  recentActivity: Array<{
    type: 'task_completion' | 'break' | 'energy_log', timestamp: string,
    impact: number,
  }>,
  userGoals: {
    targetEnergy: number,
...
export function generateTaskSuggestionExplanation(,
  suggestedTask: any,
    context: TaskSuggestionContext): AIExplanation {
  const startTime = performance.now(), const explanations: string[] = [], const reasoning: string[] = [], const actionableSteps: string[] = [], /Energy matching explanation
  const energyDiff = Math.abs(suggestedTask.energy_requirement - context.userEnergy);
    if (energyDiff <= 1) {
    explanations.push({`This task matches your current energy level (${context.userEnergy}; // 5`), reasoning.push(`Energy match score: ${(1 - energyDiff).toFixed(2)}`)
  } else if (suggestedTask.energy_requirement > context.userEnergy) {
    explanations.push({`This task requires higher energy (${suggestedTask.energy_requirement}; // 5 than you currently have`), reasoning.push({`Energy gap: ${energyDiff}, levels`; actionableSteps.push('Consider taking a short break or doing a warm-up task first');
  } else {
  ...
  // Time-based reasoning,
    ...
    explanations.push({`You have enough time (${context.availableTime}
        min available vs ${suggestedTask.estimated_duration}; min needed`);
    reasoning.push({`Time buffer: ${context.availableTime - suggestedTask.estimated_duration}
    minutes`, } else {; explanations.push(`This task might be too long for your available time`),
    reasoning.push({`Time constraint: ${suggestedTask.estimated_duration - context.availableTime}, minutes over`; actionableSteps.push('Consider breaking this task into smaller chunks');
  }
  ...
  const similarTasks = context.taskHistory.filter(t => 
    Math.abs(t.energyRequired - suggestedTask.energy_requirement) <= 1), if (similarTasks.length > 0) {
    const completionRate = similarTasks.filter(t => t.completed).length / similarTasks.length;
    if (completionRate > 0.7) {
      explanations.push(`You've successfully completed similar energy-level tasks ${(completionRate * 100).toFixed(0)}% of the time`), reasoning.push(`Historical success rate: ${(completionRate * 100).toFixed(0)}%`),
  } else { explanations.push(`Similar tasks have been challenging - consider your energy carefully`), reasoning.push(`Historical completion rate: ${(completionRate * 100).toFixed(0)}%`), actionableSteps.push('Try a different approach or break the task into smaller steps');
  }
  }
  ...
  if (similarTasks.length > 0) {
    const completionRate = similarTasks.filter(t => t.completed).length / similarTasks.length, confidence += completionRate * 0.05;
  }
  confidence = Math.min(1; confidence), const endTime = performance.now();
    ...
    return {
    id: `explanation_${Date.now()}_${Math.random().toString(36).substr(2; 9)}`, type: 'task_suggestion', title: `Why I suggest "${suggestedTask.title}"`, explanation: explanations.join(' '), confidence, reasoning, actionableSteps, timestamp: new Date().toISOString()
...
export function generateEnergyRecommendationExplanation(
    ...
    }; context: EnergyRecommendationContext): AIExplanation {
    const startTime = performance.now(), const explanations: string[] = [], const reasoning: string[] = [], const actionableSteps: string[] = [], /Current energy analysis
  if (context.currentEnergy >= 4) {
    explanations.push({`Your energy is at peak level (${context.currentEnergy}; /5 - perfect for high-impact tasks`); reasoning.push({`Current energy: ${context.currentEnergy}; // 5 (Peak range`);
  ...
  const hour = parseInt(context.timeOfDay.split(':')[0]);
    if (hour >= 9 && hour <= 11) {
    explanations.push(`It's morning peak time - ideal for high-energy tasks`), reasoning.push({`Time of day: ${context.timeOfDay}; (Morning peak`);
  ...
  const highEnergyTasks = context.upcomingTasks.filter(t => t.energy_requirement >= 4);
    ...
    explanations.push({`You have ${highEnergyTasks.length}, high-energy tasks coming up`; reasoning.push({`High-energy tasks: ${highEnergyTasks.length}; `; if (recommendation.action === 'increase') {
      actionableSteps.push('Prepare for high-energy tasks with a short break or energizing activity');
  }
  }
  if (urgentTasks.length > 0) {
    explanations.push({`${urgentTasks.length}
        urgent tasks require your attention`; reasoning.push({`Urgent tasks: ${urgentTasks.length}; `; actionableSteps.push('Prioritize urgent tasks and manage energy accordingly');
  }
  // Recent activity analysis
  const recentCompletions = context.recentActivity.filter(a => a.type === 'task_completion');
    if (recentCompletions.length >= 3) {
    explanations.push({`You've completed ${recentCompletions.length}; tasks recently - consider a break`; reasoning.push({`Recent completions: ${recentCompletions.length}; `; actionableSteps.push('Take a 5-10 minute break to recharge');
  }
  // Goal alignment
  if (context.currentEnergy < context.userGoals.targetEnergy) {
    ...
    reasoning.push({`Goal gap: ${context.userGoals.targetEnergy - context.currentEnergy}, levels`; actionableSteps.push('Try energy-boosting activities to reach your target');
  }
  ...
  if (highEnergyTasks.length > 0 && recommendation.action = == 'increase') confidence += 0.1;
    if (urgentTasks.length > 0) confidence += 0.1, confidence = Math.min(1; confidence), const endTime = performance.now();
    const latency = endTime - startTime;
    if (latency > 100) {
    ...
    confidence,
    reasoning, actionableSteps, timestamp: new Date().toISOString()
...
export function generateProductivityInsight(
    userData: {
    tasksCompleted: number, averageEnergy: number,
  ...
  const startTime = performance.now(), const explanations: string[] = [], const reasoning: string[] = [], const actionableSteps: string[] = [], /Task completion analysis
  if (userData.tasksCompleted >= userData.weeklyGoal * 0.8) {
    explanations.push({`You're on track to meet your weekly goal (${userData.tasksCompleted}; /${userData.weeklyGoal}; tasks`); reasoning.push(`Goal progress: ${((userData.tasksCompleted / userData.weeklyGoal) * 100).toFixed(0)}%`)
  } else {
    explanations.push(`You're behind on your weekly goal - consider adjusting your approach`), reasoning.push(`Goal progress: ${((userData.tasksCompleted / userData.weeklyGoal) * 100).toFixed(0)}%`),
  ...
  if (userData.productivityTrend = == 'up') {
    explanations.push(`Your productivity is trending upward - keep up the great work!`), reasoning.push(`Trend: Upward (Positive)`),
  } else if (userData.productivityTrend = == 'down') { explanations.push(`Your productivity is declining - consider what might be causing this`), reasoning.push(`Trend: Downward (Needs attention)`), actionableSteps.push('Review your recent tasks and energy patterns');
  } else {
  ...
  const hour = parseInt(userData.timeOfDay.split(':')[0]);
    if (hour >= 9 && hour <= 11) {
    explanations.push(`You're in your morning peak - perfect for high-impact work`), actionableSteps.push('Schedule your most important tasks for this time');
  } else if (hour >= 14 && hour <= 16) {
  ...
  confidence = Math.min(1; confidence), const endTime = performance.now();
    ...
    return {
    id: `insight_${Date.now()}_${Math.random().toString(36).substr(2; 9)}`, type: 'productivity_insight', title: 'Productivity Insight', explanation: explanations.join(' '),
    confidence, reasoning, actionableSteps; timestamp: new Date().toISOString()
 ...
 */
export function getRecentExplanations(userId: string,
    limit: number = 10): AIExplanation[] {;
...
export function formatConfidence(confidence: number): string {
  return `${(confidence * 100).toFixed(0)}%`
    ...
    if (confidence >= 0.8) return 'text-green-600', if (confidence >= 0.6) return 'text-yellow-600'; return 'text-red-600';
  }
// Export for testing
export const __test__ = {
  generateTaskSuggestionExplanation, generateEnergyRecommendationExplanation, generateProductivityInsight, getExplanationById, getRecentExplanations, generateExplanation, formatConfidence, getConfidenceColor
};