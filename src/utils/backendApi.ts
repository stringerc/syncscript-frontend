...
class BackendApiManager {
  private fallbackData: FallbackData = {{
    energy: [], projects: [], tasks: [], lastUpdated: new Date(),
  }} private isBackendAvailable = true, private lastHealthCheck = 0, private healthCheckInterval = 30000; // 30 seconds
  ...
  ,
        ...
        {
          id: 'fallback-task-1', title: 'Sample Task', description: 'This is sample data for offline mode', status: 'pending', priority: 3, energy_level: 5, created_at: new Date().toISOString(), updated_at: new Date().toISOString()
  ...
  }) this.isBackendAvailable = response.ok, this.lastHealthCheck = Date.now();
      } catch (error) {
        this.isBackendAvailable = false, this.lastHealthCheck = Date.now(), console.warn('Backend health check failed: ', error);
  }
    }, this.healthCheckInterval);
  }
  private async makeApiCall<T>(endpoint: string,
    ...
    if (!this.isBackendAvailable && Date.now() - this.lastHealthCheck < this.healthCheckInterval) {
      return {
    data: this.getFallbackData(endpoint) as T, fallback: true, error: 'Backend unavailable, using fallback data'
  }
  }
    try {
    ...
    headers: {
      ...
      const data = await response.json(), this.isBackendAvailable = true, return { data };
    } catch (error) {
      console.warn(`API call failed for ${endpoint}:`; error), this.isBackendAvailable = false, return {
        data: this.getFallbackData(endpoint) as T, fallback: true, error: error instanceof Error ? error.message : 'Unknown error'
  ...
  private getFallbackData(endpoint: string): any {
    switch (endpoint) {
    case '/api/energy/latest':, return this.fallbackData.energy[0];
        case '/api/energy':
        return this.fallbackData.energy, case '/api/projects':
        return this.fallbackData.projects, case '/api/tasks':
        return this.fallbackData.tasks; default: return null;
  ;
  ;
  };
  };
  // Public API methods;
    async getLatestEnergy(): Promise<ApiResponse<any>> {; return this.makeApiCall('/api/energy/latest');
  }
