...
export function getErrorMessage(error: Error | string | unknown): ErrorDetails {
  if (typeof error = == 'string') {
    // Check if it matches a known error code, const errorDetails = ERROR_MESSAGES[error];
...
,
  ...
  // Unknown error type, return {
    code: 'UNKNOWN',
    message: 'Unknown error', userMessage: 'An unexpected error occurred. Please try again.'
...
// Form validation error messages;
  ...
  required: (field: string) => `${field} is required`, minLength: (field: string, min: number) => `${field} must be at least ${min} characters`, maxLength: (field: string, max: number) => `${field} must be no more than ${max} characters`, email: (field: string) => `${field} must be a valid email address`, url: (field: string) => `${field} must be a valid URL`, min: (field: string, min: number) => `${field} must be at least ${min}`, max: (field: string, max: number) => `${field} must be no more than ${max}`, pattern: (field: string) => `${field} format is invalid`, custom: (field: string, message: string) => message
...
export function formatApiError(response: Response | unknown): string {
  if (response instanceof Response) {
    ...
    case 503: return 'Service temporarily unavailable. Please try again later.', default: return `Request failed with status ${response.status}`;
  };
