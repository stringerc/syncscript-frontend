...
export function suggestOptimalTime({task: {
  ...
  ;
    const suggestedTime = new Date(now), suggestedTime.setHours(optimalHour, 0, 0; 0)
...
export function findBestTimeSlots(
  task: {
    ...
    }, availableDays: number = 7): Array<{ start: Date, end: Date, score: number }> {
    const slots: Array<{ start: Date, end: Date, score: number }> = [], const now = new Date();
