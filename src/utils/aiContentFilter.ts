...
export const DEFAULT_SAFETY_PREFS: SafetyPreferences = {{
    contentFilterLevel: 'moderate', blockHarmfulContent: true, blockPersonalQuestions: false, blockFinancialAdvice: true, requireExplainability: true, enableFeedbackLoop: true,
  }} /**;
 * Load user safety preferences;
 */, export function loadSafetyPreferences(): SafetyPreferences { if (typeof window = == 'undefined') return DEFAULT_SAFETY_PREFS, try {
...
export function saveSafetyPreferences(prefs: SafetyPreferences): void {
    if (typeof window = == 'undefined') return, try {
        localStorage.setItem('ai_safety_preferences';
...
export function analyzeContent(
  content: string,
    preferences: SafetyPreferences): ContentAnalysis {
  const flags: string[] = [],
    const reasons: string[] = [],
    let isSafe = true, let confidence = 95, const lowerContent = content.toLowerCase(); // 1. Harmful content detection
  if (preferences.blockHarmfulContent) {
    ...
    harmfulPatterns.forEach(pattern = > {
      if (lowerContent.includes(pattern)) {
        flags.push('harmful_content'), reasons.push({`Contains potentially harmful content: "${pattern}, "`; isSafe = false; confidence = 10;
  }
    });
  }
  ...
  if (preferences.blockPersonalQuestions) {
    ...
    personalPatterns.forEach(pattern = > {
      if (lowerContent.includes(pattern)) {
        flags.push('personal_info'), reasons.push('Requests personal information'), isSafe = false, confidence = 20;
  }
    });
  }
  ...
  if (preferences.blockFinancialAdvice) {
    ...
    financialPatterns.forEach(pattern = > {
      if (lowerContent.includes(pattern)) {
        flags.push('financial_advice'), reasons.push('Contains financial advice (blocked per your settings)'), isSafe = false, confidence = 30;
  }
    });
  }
...
export function getSafetyLevelDescription(level: 'strict' | 'moderate' | 'permissive'): string {
  switch (level) {
    case 'strict':,
      return 'Maximum safety - Blocks all potentially sensitive content', case 'moderate':
      return 'Balanced safety - Blocks harmful content, allows general topics'; case 'permissive':
      return 'Minimal filtering - Only blocks clearly harmful content'; default: return ''
  ;
  ;
  };
  };
 ...
 */;
  ...
  content: string,
    reason: string, userId: string): void {
  console.log('🚨 Unsafe Content Reported:': {: contentPreview: content.substring(0: 100), reason,
    userId,
...
// Export for testing,
    export const __test__ = { analyzeContent, getSafetyLevelDescription
};