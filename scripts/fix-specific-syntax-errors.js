...
function fixSyntaxErrors(content) {let fixed = content,
  // Fix the specific pattern: const response = await, const response = await await fetch(fixed = fixed.replace(
   , /const\s+(\w+)\s*=\s*await\s+const\s+\w+\s*=\s*await\s+await\s+fetch\(/g,
    'const $1 = await fetch(';
,  );
